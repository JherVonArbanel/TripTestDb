...
CREATE Function [dbo].[ufn_GetValidResponsesForMultiCityInternational]  
...
AS  
 ...
 DECLARE @subRequestTables AS TABLE (airSubRequestKey   INT)
	...
	SELECT  AirSubRequestKey From AirSubRequest WITH (NOLOCK) where airRequestKey =@airRequestKey and airSubRequestLegIndex = -1  
     ...
     --set @gdsSourcekey = (Select gdssourcekey from Airresponse where airresponsekey = @selectedResponseKey)
		...
		select @flightNumberOne= flightnumber ,@airlineNameOne=airlines --, @airLegBookingClasses =airLegBookingClasses  
		 from NormalizedAirResponses WITH (NOLOCK) where airresponsekey = @selectedResponseKey  and airLegnumber = 1  
		...
		if (( ( select gdsSourcekey from airresponse WITH (NOLOCK) where airresponsekey =@selectedResponseKey ) <> 9 ) or (( select gdsSourcekey from airresponse WITH (NOLOCK) where airresponsekey =@selectedResponseKey ) <> 2))
		begin 
		
			set @airlines =( select top 1 airlines  from NormalizedAirResponses WITH (NOLOCK) where airResponsekey =@selectedResponseKey and airLegnumber = 1 )  
			set @airLegConnections = (select top 1  airlegConnections from NormalizedAirResponses WITH (NOLOCK) where airResponsekey =@selectedResponseKey and airLegnumber = 1 )  
		--	set @airLegBookingClasses  = (select top 1  airLegBookingClasses from NormalizedAirResponses WITH (NOLOCK) where airResponsekey =@selectedResponseKey and airLegnumber = 1 )  
			insert into @ResultTable (airresponseKey)  
			(  
			select Airresponsekey from NormalizedAirResponses n  WITH (NOLOCK)
			inner join airsubrequest s WITH (NOLOCK) on n.airsubrequestkey=s.airSubrequestkey   
		...
		--  set @airLegBookingClasses  = (select top 1  airLegBookingClasses from NormalizedAirResponses where airResponsekey =@selectedResponseKey and airLegnumber = 1 )  
			...
			inner join airsubrequest s WITH (NOLOCK) on n.airsubrequestkey=s.airSubrequestkey
			INNER JOIN  @subRequestTables tmpS on S.airsubrequestkey = tmps.airSubRequestKey 
			where airrequestkey  = @airrequestkey  and airLegnumber = 1 and flightnumber =  @flightnumberOne and airlines = @airlineNameOne      
			--and airlegBookingClasses = @airlegBookingClasses   
			)
		end 
		end   
  ...
  if  @airLegnumber = 3 
 			...
 			END 
 			 select @flightNumberOne= flightnumber ,@airlineNameOne=airlines--,@airLegBookingClasses=airLegBookingClasses   
 			 from NormalizedAirResponses WITH (NOLOCK) where airresponsekey = @selectedResponseKey  and airLegnumber = 1
	 select @flightNumberTwo= flightnumber ,@airlineNameTwo=airlines --,@airLegSecondBookingClasses= airLegBookingClasses  
	 from NormalizedAirResponses WITH (NOLOCK)  where airresponsekey = @selectedResponseKeySecond and    airLegnumber = 2
	   insert into @ResultTable (airresponseKey)
			 (
			select airresponsekey  from NormalizedAirResponses  n  WITH (NOLOCK)
  inner join airsubrequest s WITH (NOLOCK) on n.airsubrequestkey=s.airSubrequestkey  
...
intersect 
  ...
  inner join airsubrequest s WITH (NOLOCK) on n.airsubrequestkey=s.airSubrequestkey 
  INNER JOIN  @subRequestTables tmpS on S.airsubrequestkey = tmps.airSubRequestKey 
   where airrequestkey  = @airrequestkey  and airLegnumber = 2  and flightnumber =  @flightnumberTwo and airlines = @airlineNameTwo  
   --and airLegBookingClasses = @airLegSecondBookingClasses
	 ...
	 from NormalizedAirResponses WITH (NOLOCK) where airresponsekey = @selectedResponseKeySecond  and airLegnumber = 2
		 ...
		 select @flightNumberThree= flightnumber ,@airlineNameThree=airlines --,@airLegThirdBookingClasses=airLegBookingClasses  
		 from NormalizedAirResponses WITH (NOLOCK) where airresponsekey = @selectedResponseKeyThird  and airLegnumber = 3
  ...
  INNER JOIN  @subRequestTables tmpS on S.airsubrequestkey = tmps.airSubRequestKey 
   where airrequestkey  = @airrequestkey  and airLegnumber = 3  and flightnumber =  @flightnumberThree and airlines = @airlineNameThree --and airLegBookingClasses =@airLegThirdBookingClasses  
	 ...
	 from NormalizedAirResponses WITH (NOLOCK) where airresponsekey = @selectedResponseKeySecond  and airLegnumber = 2
		 ...
		 select @flightNumberFour= flightnumber ,@airlineNameFour=airlines   --,@airLegFourthBookingClasses=airLegBookingClasses 
		 from NormalizedAirResponses WITH (NOLOCK) where airresponsekey = @selectedResponseKeyFourth  and airLegnumber = 4
 ...
 intersect 
  ...
  where airrequestkey  = @airrequestkey  and airLegnumber = 4  and flightnumber =  @flightnumberFour  and airlines = @airlineNameFour --and airLegBookingClasses = @airLegFourthBookingClasses 
	 ...
	 from NormalizedAirResponses WITH (NOLOCK) where airresponsekey = @selectedResponseKeySecond  and airLegnumber = 2
		 ...
		 select @flightNumberFive = flightnumber ,@airlineNameFive=airlines  -- ,@airLegFifthBookingClasses=airLegBookingClasses 
		 from NormalizedAirResponses WITH (NOLOCK) where airresponsekey = @selectedResponseKeyFifth  and airLegnumber = 5
