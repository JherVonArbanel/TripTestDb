...
CREATE Function [dbo].[ufn_GetValidResponsesForMultiCityInternational1]  
...
AS  
 ...
 DECLARE @subRequestTables AS TABLE (airSubRequestKey   INT)
	...
	SELECT  AirSubRequestKey From AirSubRequest WITH (NOLOCK) where airRequestKey =@airRequestKey and airSubRequestLegIndex = -1  
     ...
     --set @gdsSourcekey = (Select gdssourcekey from Airresponse where airresponsekey = @selectedResponseKey)
		...
		--  set @airLegBookingClasses  = (select top 1  airLegBookingClasses from NormalizedAirResponses where airResponsekey =@selectedResponseKey and airLegnumber = 1 )  
			...
			select @flightNumberOne= flightnumber ,@airlineNameOne=airlines  ,@airLegBookingClasses =airLegBookingClasses  from NormalizedAirResponses WITH (NOLOCK) where airresponsekey = @selectedResponseKey  and airLegnumber = 1
			insert into @ResultTable (airresponseKey)
			(
			select airresponsekey  from NormalizedAirResponses  n  WITH (NOLOCK)
			inner join airsubrequest s WITH (NOLOCK) on n.airsubrequestkey=s.airSubrequestkey
  ...
  if  @airLegnumber = 3 
 			...
 			END 
 			 select @flightNumberOne= flightnumber ,@airlineNameOne=airlines,@airLegBookingClasses=airLegBookingClasses   from NormalizedAirResponses WITH (NOLOCK) where airresponsekey = @selectedResponseKey  and airLegnumber = 1
	 select @flightNumberTwo= flightnumber ,@airlineNameTwo=airlines ,@airLegSecondBookingClasses= airLegBookingClasses  from NormalizedAirResponses WITH (NOLOCK)  where airresponsekey = @selectedResponseKeySecond and    airLegnumber = 2
	   insert into @ResultTable (airresponseKey)
			 (
			select airresponsekey  from NormalizedAirResponses  n  WITH (NOLOCK)
  inner join airsubrequest s WITH (NOLOCK) on n.airsubrequestkey=s.airSubrequestkey  
  ...
  INNER JOIN  @subRequestTables tmpS on S.airsubrequestkey = tmps.airSubRequestKey 
   where airrequestkey  = @airrequestkey  and airLegnumber = 2  and flightnumber =  @flightnumberTwo and airlines = @airlineNameTwo  and airLegBookingClasses = @airLegSecondBookingClasses
 			...
 			END 
 			 ...
 			 select @flightNumberOne= flightnumber ,@airlineNameOne=airlines ,@airLegBookingClasses=airLegBookingClasses  from NormalizedAirResponses WITH (NOLOCK) where airresponsekey = @selectedResponseKey  and airLegnumber = 1
	 select @flightNumberTwo= flightnumber ,@airlineNameTwo=airlines,@airLegSecondBookingClasses=airLegBookingClasses   from NormalizedAirResponses WITH (NOLOCK) where airresponsekey = @selectedResponseKeySecond  and airLegnumber = 2
		 select @flightNumberThree= flightnumber ,@airlineNameThree=airlines ,@airLegThirdBookingClasses=airLegBookingClasses  from NormalizedAirResponses WITH (NOLOCK) where airresponsekey = @selectedResponseKeyThird  and airLegnumber = 3
   insert into @ResultTable (airresponseKey)
			 (
			select airresponsekey  from NormalizedAirResponses n  WITH (NOLOCK)
  inner join airsubrequest s WITH (NOLOCK) on n.airsubrequestkey=s.airSubrequestkey  
 ...
 intersect 
  ...
  INNER JOIN  @subRequestTables tmpS on S.airsubrequestkey = tmps.airSubRequestKey 
   where airrequestkey  = @airrequestkey  and airLegnumber = 3  and flightnumber =  @flightnumberThree and airlines = @airlineNameThree and airLegBookingClasses =@airLegThirdBookingClasses  
	 ...
	 select @flightNumberTwo= flightnumber ,@airlineNameTwo=airlines ,@airLegSecondBookingClasses=airLegBookingClasses  from NormalizedAirResponses WITH (NOLOCK) where airresponsekey = @selectedResponseKeySecond  and airLegnumber = 2
		 select @flightNumberThree= flightnumber ,@airlineNameThree=airlines ,@airLegThirdBookingClasses=airLegBookingClasses  from NormalizedAirResponses  WITH (NOLOCK) where airresponsekey = @selectedResponseKeyThird  and airLegnumber = 3
		 select @flightNumberFour= flightnumber ,@airlineNameFour=airlines   ,@airLegFourthBookingClasses=airLegBookingClasses from NormalizedAirResponses WITH (NOLOCK) where airresponsekey = @selectedResponseKeyFourth  and airLegnumber = 4
   insert into @ResultTable (airresponseKey)
			 (
			select airresponsekey  from NormalizedAirResponses n WITH (NOLOCK)  
  inner join airsubrequest s WITH (NOLOCK) on n.airsubrequestkey=s.airSubrequestkey  
 ...
 intersect 
  ...
  where airrequestkey  = @airrequestkey  and airLegnumber = 4  and flightnumber =  @flightnumberFour  and airlines = @airlineNameFour and airLegBookingClasses = @airLegFourthBookingClasses 
	 ...
	 select @flightNumberTwo= flightnumber ,@airlineNameTwo=airlines ,@airLegSecondBookingClasses=airLegBookingClasses  from NormalizedAirResponses WITH (NOLOCK) where airresponsekey = @selectedResponseKeySecond  and airLegnumber = 2
		 select @flightNumberThree= flightnumber ,@airlineNameThree=airlines ,@airLegThirdBookingClasses=airLegBookingClasses  from NormalizedAirResponses WITH (NOLOCK) where airresponsekey = @selectedResponseKeyThird  and airLegnumber = 3
		 select @flightNumberFour= flightnumber ,@airlineNameFour=airlines   ,@airLegFourthBookingClasses=airLegBookingClasses from NormalizedAirResponses WITH (NOLOCK) where airresponsekey = @selectedResponseKeyFourth  and airLegnumber = 4
		 		 select @flightNumberFive = flightnumber ,@airlineNameFive=airlines   ,@airLegFifthBookingClasses=airLegBookingClasses from NormalizedAirResponses WITH (NOLOCK) where airresponsekey = @selectedResponseKeyFifth  and airLegnumber = 5
