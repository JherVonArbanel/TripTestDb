...
CREATE VIEW [dbo].[vw_TripHotelResponse]  
AS  
...
FROM         HotelContent.dbo.Hotels AS HT WITH (NOLOCK) 
	...
	RIGHT OUTER JOIN dbo.TripHotelResponse AS HR WITH (NOLOCK) ON SH.SupplierHotelId = HR.supplierHotelKey ON HT.HotelId = SH.HotelId 
		 AND SH.supplierHotelId = ISNULL(HR.HotelId, SH.supplierHotelId)
	...
	INNER JOIN  dbo.Trip WITH (NOLOCK) ON HR.tripKey = dbo.Trip.tripKey  
WHERE     (ISNULL(HR.isDeleted, 0) = 0)    AND dbo.Trip.tripStatusKey <> 17 --ANd HR.TripHotelResponseKey =400449
...
Begin DesignProperties = 
   Begin PaneConfigurations = 
      Begin PaneConfiguration = 0
         NumPanes = 4
         Configuration = "(H (1[22] 4[11] 2[48] 3) )"
      End
   ...
   Begin DiagramPane = 
      ...
      Begin Tables = 
         ...
         Begin Table = "Trip"
            ...
            DisplayFlags = 280
            TopColumn = 0
        ', 'SCHEMA', N'dbo', 'VIEW', N'vw_TripHotelResponse', NULL, NULL
GO
...
DECLARE @xp int
SELECT @xp=2
EXEC sp_addextendedproperty N'MS_DiagramPaneCount', @xp, 'SCHEMA', N'dbo', 'VIEW', N'vw_TripHotelResponse', NULL, NULL
GO
