...
CREATE VIEW [dbo].[vw_TripCarResponse_Audit]  
AS  
SELECT     trip.tripKey, tripName, trip.userKey, TripCarResponse.recordLocator, trip.endDate, trip.startDate, trip.tripStatusKey, dbo.TripCarResponse.carResponseKey,   
                      ...
                      ,dbo.TripCarResponse.PickupLocationInfo  
FROM         CarContent.dbo.CarCompanies WITH (NOLOCK) INNER JOIN  
                      ...
                      dbo.Trip WITH (NOLOCK) ON (dbo.TripCarResponse.tripKey = dbo.Trip.tripKey or ( dbo.TripCarResponse.tripguidkey = (case when dbo.Trip.tripPurchasedKey is not null then dbo.Trip.tripPurchasedKey else dbo.Trip.tripsavedkey end )  and (dbo.TripCarResponse.tripKey is null or dbo.TripCarResponse.tripKey=0 )) )  
...
FROM         TripCarResponse TR WITH (NOLOCK) INNER JOIN  
                      ...
                      Trip WITH (NOLOCK) ON (TR.tripKey = Trip.tripKey OR (TR.tripGUIDKey = (case when Trip.tripPurchasedKey is not null then Trip.tripPurchasedKey else Trip.tripsavedkey end ) and (tr.tripKey is null or tr.tripKey =0 )))  
                      LEFT OUTER JOIN  
...
FROM         TripCarResponse TR WITH (NOLOCK) INNER JOIN  
                      CarContent.dbo.CarCompanies WITH (NOLOCK) ON CarContent.dbo.CarCompanies.CarCompanyCode = TR.carVendorKey INNER JOIN  
                      Trip WITH (NOLOCK) ON (TR.tripKey = Trip.tripKey OR (TR.tripGUIDKey = (case when Trip.tripPurchasedKey is not null then Trip.tripPurchasedKey else Trip.tripsavedkey end ) and (tr.tripKey is null or tr.tripKey =0 ))) LEFT OUTER JOIN  
...
SELECT     trip.tripKey, tripName, trip.userKey, TR.recordLocator, trip.endDate, trip.startDate, trip.tripStatusKey, TR.carResponseKey, TR.confirmationNumber, TR.carVendorKey,   
                      ...
                      ,TR.OperationTimeStart,TR.OperationTimeEnd,TR.PickupLocationInfo  
FROM         TripCarResponse TR WITH (NOLOCK) INNER JOIN  
                      CarContent.dbo.CarCompanies WITH (NOLOCK) ON CarContent.dbo.CarCompanies.CarCompanyCode = TR.carVendorKey INNER JOIN  
                      Trip WITH (NOLOCK) ON (TR.tripKey = Trip.tripKey OR (TR.tripGUIDKey = (case when Trip.tripPurchasedKey is not null then Trip.tripPurchasedKey else Trip.tripsavedkey end ) and (tr.tripKey is null or tr.tripKey =0 )))LEFT OUTER JOIN  
