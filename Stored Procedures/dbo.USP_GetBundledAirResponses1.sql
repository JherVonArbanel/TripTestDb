...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[USP_GetBundledAirResponses1]
(
...
---creating TABLE variable for container for flitered result ..
 ...
 print (cast(getdate() AS time))
	...
	(
		...
		INSERT @tempOneWayResponses (airOneResponsekey,airOnePriceBase,airSegmentFlightNumber,airSegmentMarketingAirlineCode,airsubRequestkey,airOnePriceTax   ,cabinClass ,otherLegPrice  ,airlegnumber   )
									
 									 SELECT resp.AirResponsekey, airPriceBase ,nresp.flightNumber ,nresp.airlines,resp.airSubRequestKey,airPriceTax ,nresp.cabinclass,(case when @isTotalPriceSort = 0 then isnull(@airPriceForAnotherLeg,0)else ( isnull(@airPriceForAnotherLeg,0) + isnull(@airPriceTaxForAnotherLeg,0))  end),nresp.airLegNumber 
...
END 
	...
	(
		...
		(
			...
			resp.gdsSourceKey, result .airOnePriceTax ,result.airsubRequestkey ,result .cabinClass  ,otherLegPrice,otherLegTax  ,result.airlegnumber 
			FROM 
