...
CREATE PROCEDURE [dbo].[USP_GetTripSavedDealCart]              
(          
 @tripKey int,
 @tripDate date,
 @userId int = 0 
...
( isnull(tripSeniorBase,0) * isnull(@seniorCount,0) ) + (isnull(tripYouthBase,0)*isnull(@youthCount,0) ) + (isnull(tripInfantBase,0)*isnull(@infantCount,0) )+ (isnull(tripInfantwithSeatBase,0)*isnull(@infantWithSeatCount,0) )  )+(( isnull(tripAdulttax,0) * isnull(@adultCount,0) ) + (isnull(tripChildtax,0)*isnull(@childCount,0) ) + 
( isnull(tripSeniortax,0) * isnull(@seniorCount,0) ) + (isnull(tripYouthtax,0)*isnull(@youthCount,0) ) + (isnull(tripInfanttax,0)*isnull(@infantCount,0) )+ (isnull(tripInfantwithSeattax,0)*isnull(@infantWithSeatCount,0) )  ) )- isnull(@originalAirPrice ,0))) as Savings,TAP.*,@airDealDate as dealDate,
...
PRINT  CONVERT(VARCHAR(30),GETDATE(),113) 
select @dealKey =d.dealKey  FROM @deals D WHERE  [componentType]= 4
DECLARE @hotelResponseKey AS UNIQUEIDENTIFIER 
DECLARE @hotelDealDate AS DateTime 
SELECT @hotelResponseKey =responseKey ,@hotelDealDate = creationDate  FROM TripSavedDeals WITH(NOLOCK) WHERE TripSavedDealKey =@dealKey 
select top 1 @tripKey AS tripKey, vw.*,  Convert(decimal ( 18,2) , (hotelTotalPrice  )) -  (@originalHotelPrice )  as Savings ,@hotelDealDate as dealDate , 1 AS TripCreationPath
 from --TripSavedDeals   TND   WITH(NOLOCK) INNER JOIN 
  vw_tripHotelResponseDetails vw --on TND.TripSavedDealKey  = @dealKey  AND TND.ResponseKey = vw.hotelResponseKey 
--LEFT OUTER JOIN 
--HotelResponseDetail HD on TND.responseDetailKey =HD.hotelResponseDetailKey 
...
BEGIN 
select distinct T.tripKey, vw.*  , 0 as Savings , @date as dealDate from vw_tripCarResponseDetails vw INNER JOIN Trip T WITH (NOLOCK) on vw.tripGUIDKey = T.tripPurchasedKey Where T.tripPurchasedKey = @purchasekey   and T.tripStatusKey <> 17
END 
ELSE 
BEGIN 
...
SELECT @carResponsekey = responsekey,@dealDate = creationDate FROM TripSavedDeals WHERE TripSavedDealKey =@dealKey
select   @tripKey AS tripkey, vw.*  ,  convert(decimal ( 18,2) ,(isnull((minRate * NoOfDays ) + searchCarTax   ,0)- isnull((@originalCarPrice) ,0))) as Savings ,@dealDate as dealDate  
  from  --TripSavedDeals TND  WITH(NOLOCK) INNER JOIN 
