...
Exec [USP_GetFollowingDetailsByUserId] 561138 , 'followers',560799    
 Exec [USP_GetFollowingDetailsByUserId] 561138 , 'following',560799  
 */  
-- =============================================          
    
CREATE PROCEDURE [dbo].[USP_GetFollowingDetailsByUserId]          
 @UserId INT,     
 @Type VARCHAR(50),        
...
BEGIN          
  ...
  SET NOCOUNT ON -- added to prevent extra result sets from          
	...
	Begin     
		...
		desinationImageUrl    
		  ...
		  SELECT --DISTINCT 
			...
			, ISNULL(AP.originAirportCode,'') originAirportCode, ISNULL(D.CityName, '') CityName,UF.DestinationImageUrlForFollowing  
		  FROM Loyalty..UserFollowers UF     
			INNER JOIN [Vault]..[User] U ON U.userKey = UF.UserId     
			LEFT OUTER JOIN [Loyalty]..[UserMap] UM ON UF.UserId = UM.UserId    
			LEFT OUTER JOIN [Vault].[dbo].[AirPreference] AP ON UF.UserId = AP.userKey    
	...
	Begin 
		...
		SELECT UF.FollowerId, U.userFirstName, U.userLastName, UM.BadgeName, UM.ImageURL, AP.originAirportCode, D.CityName, UF.DestinationImageUrlForFollowers    
		FROM Loyalty..UserFollowers UF     
