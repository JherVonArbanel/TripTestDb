...
-- =============================================
CREATE PROCEDURE [dbo].[SavePurchaseTrip_TripPassenger_Insert]
...
BEGIN
	...
	INSERT INTO TripPassengerInfo (TripKey, TripHistoryKey, PassengerKey, PassengerTypeKey, IsPrimaryPassenger, TripRequestKey, AdditionalRequest,  
					PassengerEmailID, PassengerFirstName, PassengerLastName, PassengerLocale, PassengerTitle, CellPhone,PassengerGender, PassengerBirthDate, 
					TravelReferenceNo, IsExcludePricingInfo, ReimbursementAddressId, ArrangerEmailCSV, IsArrangerEmailWithPricing, IsArrangerEmailWithoutPricing,IROPPassengerKey)
	...
	SELECT @tripId, 
	  ...
	  TripPassengerInfo.value('(CellPhone/text())[1]','VARCHAR(200)') AS CellPhone,
	  ...
	  TripPassengerInfo.value('(ArrangerEmailCSV/text())[1]','VARCHAR(500)') AS ArrangerEmailCSV,
	  TripPassengerInfo.value('(IsArrangerEmailWithPricing/text())[1]','BIT') AS IsArrangerEmailWithPricing,
	  TripPassengerInfo.value('(IsArrangerEmailWithoutPricing/text())[1]','BIT') AS IsArrangerEmailWithoutPricing,
	  iif(TripPassengerInfo.value('(PassengerIROPKey/text())[1]','int')>0,TripPassengerInfo.value('(PassengerIROPKey/text())[1]','int'),null) AS IROPPassengerKey
	...
	End
	---------CoWorker - Kinjal----------
	INSERT INTO TripCoworker(TripPassengerInfoKey,FirstName,LastName,Email,IsSendMail,UserKey)		
	SELECT O.TripPassengerInfoKey,
		   TripPassengerCoworker.value('(FirstName/text())[1]','NVARCHAR(50)'),
		   TripPassengerCoworker.value('(LastName/text())[1]','NVARCHAR(50)'),
		   TripPassengerCoworker.value('(EmailAddress/text())[1]','NVARCHAR(50)'),
		   TripPassengerCoworker.value('(IsSendMail/text())[1]','BIT'),
		   TripPassengerCoworker.value('(UserKey/text())[1]','INT')
		FROM @xmldata.nodes('/TripPassenger/TripPassengerInfos/TripPassengerInfo/TripPassengerCoworker/Coworker')AS TEMPTABLE(TripPassengerCoworker)
		inner join @output O on O.PassengerKey = TripPassengerCoworker.value('(TripPassengerInfoKey/text())[1]','int') 	
	
	---------CoWorker - Kinjal----------
	---------Rebook IROP Passenger - Kinjal -------
	--declare @temp varchar(100)=
	--(select top 1  iif(TripPassengerInfo.value('(PassengerIROPKey/text())[1]','int')>0,TripPassengerInfo.value('(PassengerIROPKey/text())[1]','int'),null) AS IROPPassengerKey
	--FROM @xmldata.nodes('/TripPassenger/TripPassengerInfos/TripPassengerInfo')AS TEMPTABLE(TripPassengerInfo))
	--if @temp>0
	--update Vault..IROP_TravelerInfo	set TripKey=@tripId
	--where IROPTravelerKey=@temp
	---------Rebook IROP Passenger - Kinjal -------
