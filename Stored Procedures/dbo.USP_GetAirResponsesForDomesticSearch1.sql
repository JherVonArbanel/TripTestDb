...
CREATE PROCEDURE [dbo].[USP_GetAirResponsesForDomesticSearch1]
	(  
	...
	DECLARE @HighFareTotal AS FLOAT = 0, @LowFareThreshold AS FLOAT = 0, @IsLowFareThreshold AS BIT = 0, @LowestPrice AS FLOAT = 0;
	...
	IF (@isInternationalTrip = 0)
	   SELECT TOP 1 @MaxFareTotal = domFareTol, @HighFareTotal = domHighFareTol,@LowFareThreshold = LowFareThreshold, @IsLowFareThreshold = isLowFareThreshold  FROM @tblGetPolicyDetailsForAir
	ELSE
	   SELECT TOP 1 @MaxFareTotal = intlFareTol, @HighFareTotal = domHighFareTol,@LowFareThreshold = LowFareThreshold, @IsLowFareThreshold = isLowFareThreshold  FROM @tblGetPolicyDetailsForAir
				...
				BEGIN    
				   ...
				   ELSE
					   BEGIN
							...
							AND EXISTS(SELECT 1 FROM @cabinUniq WHERE airLegBrandName = lower(N.airLegBrandName)AND isRefundable = resp.refundable AND gdsSourceKey = resp.gdsSourceKey)
							UNION ALL
							SELECT 
							gdsSourceKey,airPriceBAse,airPriceTax,airPriceBaseSenior,airPriceTaxSenior,airPriceBaseChildren,airPriceTaxChildren ,airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth ,airPriceTaxYouth ,AirPriceBaseTotal,AirPriceTaxTotal,airPriceBaseDisplay,airPriceTaxDisplay        ,resp.airResponseKey,airPriceBaseInfantWithSeat,airPriceTaxInfantWithSeat,N.airLegBrandName,resp.refundable
							...
							WHERE airSubRequestLegIndex > 1 
							and N.airResponseKey not in ( select airresponsekey  from @tempResponseToRemove ) 
							AND EXISTS(SELECT 1 FROM @cabinUniq WHERE airLegBrandName = lower(N.airLegBrandName) 
