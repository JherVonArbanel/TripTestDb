...
GO
		 ...
		 CREATE PROCEDURE [dbo].[USP_Report_TransactionCounter_Tuned_NeedRecCheck]     (        
		--DECLARE  
		 @pageNo As int = 1,      
		 @pageSize As int = 25,      
...
--SELECT  @pageNo=1,@PageSize=25,@isCurrentYear=1,@drillDown=1,@userID=0,@dateRangeType=N'bookdate',@fromDate='2013-05-01 00:00:00',
--@toDate='2013-09-30 23:59:59',@airline=NULL,@eventCode=NULL,@formOfPayment=NULL,@country=NULL,@fareType=NULL,@BookType=0,@sortField=NULL,
...
-------------------- Added By Gopal ---------------------------------
--SELECT GETDATE() AS [0_1]

		IF OBJECT_ID('tempdb..#tblCur') IS NOT NULL
			DROP TABLE #tblCur
		...
		(     
			...
			exchangeRateAmount  FLOAT NULL
		)     

		INSERT INTO #tblCur (CurrencyKey, currencyCode,exchangeRateDate)
		SELECT E.currencyKey, C.currencyCode, MAX(E.exchangeRateDate)
		FROM vault.dbo.ExchangeRate E, vault.dbo.Currency C where e.currencyKey = c.currencyKey
		GROUP BY E.currencyKey, C.currencyCode 
		order by C.currencyCode 

		UPDATE t SET exchangeRateAmount = isnull(E.exchangeRateAmount,0)
		FROM #tblCur t
			INNER JOIN vault.dbo.ExchangeRate E ON t.CurrencyKey = E.currencyKey AND t.exchangeRateDate = E.exchangeRateDate 

		--Select Price, totalPrice,actualAirPrice,PassengerLocale,* from #tripItinerary  I
		--left outer join #tblCur CU on CU.currencyCode = I.PassengerLocale
		--where PassengerLocale <> 'USD' -- and @exchangeRateAmount <> 0
				  
--SELECT GETDATE() AS [0_2]
-------------------- Added By Gopal continue.... ---------------------------------
...
--SELECT GETDATE() AS [3]
		...
		where trip.tripStatusKey <> 17 and seg.isDeleted <> 1  --and trip.userKey = 0     
		and      
		(    
...
--SELECT GETDATE() AS [5]
		...
		Insert into #tripItinerary (tripkey, PassengerName, Booked, Travel, City, Vendor, VendorCode, AdvPurch, agentStatus, Price, appliedDiscount, 
			meetingCode, tripStatus, fareType, bookingClass, farebasisCode, creditCardVendorCode, recordlocator, countryCode, tripType
		,currencyCode, envUdidValue, cashUdidValue, totalprice, carriers, carriersName
		,PassengerLocale, actualAirPrice 
		)    
...
(
	case  when Pax.PassengerLocale <> 'USD' Then 
		isnull(TAR.actualAirPrice, 0) / ISNULL(C.exchangeRateAmount, 1)  
	else     
...
(
	case  when Pax.PassengerLocale <> 'USD' Then 
		...
		from Trip T    
			...
			left outer join #tblCur C ON Pax.PassengerLocale = C.currencyCode     
...
--SELECT GETDATE() AS [6_0]



--DECLARE @strQ VARCHAR(MAX) = ''

--SET @strQ = 'select  T.tripKey,isnull(Pax.PassengerLastName+''/''+Pax.PassengerFirstName,'''') as ''''PassengerName'''',T.CreatedDate as ''Booked'',T.startDate as ''Travel'',     
--		isnull(TSD.City,''''),isnull(TSD.Vendor,''''),isnull(TSD.VendorCode,''''),    
--		isnull(DATEDIFF(DAY,T.CreatedDate,T.startDate),0) AS ''AdvPurch'', T.isOnlineBooking as agentStatus,  
...
--		T.tripKey = E.tripkey     
--		where    
--		T.tripStatusKey <> 17 AND T.tripStatusKey <> 1 AND  
--		 (    
--		 (case when Lower(ISNULL(@dateRangeType,''bookdate''))=Lower(''bookdate'') then T.CreatedDate else T.startDate end)     
--		between ISNULL(@fromDate,''1753-01-01 00:00:00.000'') and ISNULL(@toDate, ''9999-12-31 23:59:59.997'')    
--		OR (case when Lower(ISNULL(@dateRangeType,''bookdate''))=Lower(''bookdate'') then T.CreatedDate else T.endDate  end)     
...
----SELECT @orderBy = 		      
----		case when @sortField = 'ConventionCode' and @sortDirection ='Descending' then  ' ltrim(T.meetingCodeKey) DESC '      
----			when @sortField = 'ConventionCode' and @sortDirection ='Ascending' then   ' ltrim(T.meetingCodeKey) ASC ' END 
----SELECT @orderBy = 		      
----		case when @sortField = 'Booked' and @sortDirection ='Descending' then   ' T.CreatedDate  desc '     
----			when @sortField = 'Booked' and @sortDirection ='Ascending' then   ' T.CreatedDate  asc ' END
----SELECT @orderBy = 		      
----		case when @sortField = 'Travel' and @sortDirection ='Descending' then  '  T.startDate  desc '  
----			when @sortField = 'Travel' and @sortDirection ='Ascending' then   ' T.startDate asc ' END
----SELECT @orderBy = 		      
----		case when @sortField = 'City' and @sortDirection ='Descending' then   ' TSD.City  desc '     
----			 when @sortField = 'City' and @sortDirection ='Ascending' then   ' TSD.City   asc ' END    
----SELECT @orderBy = 		      
----		case when @sortField = 'Vendor' and @sortDirection ='Descending' then  '  TSD.Vendor  desc '  
----			 when @sortField = 'Vendor' and @sortDirection ='Ascending' then   ' TSD.Vendor  asc ' END
----SELECT @orderBy = 		      
----		case when @sortField = 'Adv' and @sortDirection ='Descending' then    ' isnull(DATEDIFF(DAY,T.CreatedDate,T.startDate),0)   desc ' 
----			 when @sortField = 'Adv' and @sortDirection ='Ascending' then  ' isnull(DATEDIFF(DAY,T.CreatedDate,T.startDate),0)  asc ' END
----SELECT @orderBy = 		      
----		case when @sortField = 'Status' and @sortDirection ='Descending' then  '  case   when T.tripRequestKey   > 0  then 1 else 0 end  desc' 
----			 when @sortField = 'Status' and @sortDirection ='Ascending' then   ' case   when T.tripRequestKey   > 0  then 1 else 0 end  asc ' END    
----SELECT @orderBy = 		      
----		case when @sortField = 'Price' and @sortDirection ='Descending' then   ' isnull(TAR.actualAirPrice,0) desc ' 
----			 when @sortField = 'Price' and @sortDirection ='Ascending' then  '  isnull(TAR.actualAirPrice ,0)  asc ' END 
----SELECT @orderBy = case when @sortField = '' and @sortDirection ='Descending' then   ' T.startDate desc ' END

--SELECT @orderBy = 
--		CASE WHEN @sortField = 'ConventionCode' THEN	
--				CASE WHEN @sortDirection = 'Descending' THEN ' ltrim(T.meetingCodeKey) DESC ' ELSE ' ltrim(T.meetingCodeKey) ASC ' END
--			WHEN @sortField = 'Booked' THEN
--				CASE WHEN @sortDirection ='Descending' then ' T.CreatedDate  desc ' ELSE ' T.CreatedDate  asc ' END
--			WHEN @sortField = 'Travel' THEN 
--				CASE WHEN @sortDirection ='Descending' then  '  T.startDate  desc ' ELSE ' T.startDate asc ' END
--			WHEN @sortField = 'City' THEN
--				CASE WHEN @sortDirection ='Descending' then   ' TSD.City  desc ' ELSE ' TSD.City   asc ' END 
--			WHEN @sortField = 'Vendor' THEN	
--				CASE WHEN @sortDirection ='Descending' then  '  TSD.Vendor  desc ' ELSE ' TSD.Vendor  asc ' END
--			WHEN @sortField = 'Adv' THEN
--				CASE WHEN @sortDirection ='Descending' then    ' isnull(DATEDIFF(DAY,T.CreatedDate,T.startDate),0)   desc ' ELSE ' isnull(DATEDIFF(DAY,T.CreatedDate,T.startDate),0)  asc ' END
--			WHEN @sortField = 'Status' THEN
--				CASE WHEN @sortDirection ='Descending' then  '  case when T.tripRequestKey   > 0  then 1 else 0 end  desc' ELSE ' case   when T.tripRequestKey   > 0  then 1 else 0 end  asc ' END 
--			WHEN @sortField = 'Price' THEN 
--				CASE WHEN @sortDirection ='Descending' then   ' isnull(TAR.actualAirPrice,0) desc ' ELSE '  isnull(TAR.actualAirPrice ,0)  asc ' END 
--			ELSE	
--				CASE WHEN @sortField = '' Or @sortField IS NULL THEN 
--					CASE WHEN @sortDirection = 'Descending' THEN ' T.startDate desc ' ELSE ' T.startDate asc ' END
--				END
--			END
...
--PRINT 'Printing @strQ --> ' 
--PRINT @strQ 
--PRINT 'Printing @orderBy --> ' 
--PRINT @orderBy

--SELECT GETDATE() AS [6_2]
