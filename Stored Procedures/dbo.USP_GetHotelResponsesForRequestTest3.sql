...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[USP_GetHotelResponsesForRequestTest3]
( @hotelRequestKey  int ,
...
SET @sqlString = '(Select * from vw_sabreHotelResponseTest where supplierId=''hotelsCom'' and hotelId Not in (Select HotelId from vw_sabreHotelResponseTest where supplierId=''sabre'' and ' + @shortString + ') and ' + @shortString + ') Union all (Select * from vw_sabreHotelResponseTest where  supplierId=''sabre''  and  ' + @shortString + ') order by preferenceorder ,minRate,distance,Rating,HotelName asc'
...
SET @sqlString = '(Select * from vw_sabreHotelResponseTest where supplierId=''hotelsCom'' and hotelId Not in (Select HotelId from vw_sabreHotelResponseTest where supplierId=''sabre'' and ' + @shortString + ') and ' + @shortString + ') Union all (Select * from vw_sabreHotelResponseTest where  supplierId=''sabre''  and  ' + @shortString + ') order by ' + CONVERT(varchar, @sortColumn) + ' Asc'
...
insert into @hotelResponseResult (hotelResponseKey,supplierHotelKey,hotelRequestKey,supplierId,minRate,HotelName,Rating,RatingType,ChainCode,HotelId,Latitude,Longitude,Address1,CityName,StateCode,CountryCode,ZipCode,PhoneNumber,FaxNumber,CityCode,distance,checkInDate,checkOutDate,HotelDescription,ChainName,minRateTax,ImageURL) 
EXEC(@sqlString)
--SELECT * FROM vw_sabreHotelResponseTest where  hotelRequestKey=@hotelRequestKey and HotelName <> '' and Rating in ( select * from ufn_CSVToTable ( @hotelRatings ))and  minRate between   @minPrice and  @maxPrice and distance between  @mindistance and @maxdistance  
...
begin 
insert into @hotelResponseResult (hotelResponseKey,supplierHotelKey,hotelRequestKey,supplierId,minRate,HotelName,Rating,RatingType,ChainCode,HotelId,Latitude,Longitude,Address1,CityName,StateCode,CountryCode,ZipCode,PhoneNumber,FaxNumber,CityCode,distance,checkInDate,checkOutDate,HotelDescription,ChainName,minRateTax,ImageURL) 
SELECT * FROM vw_sabreHotelResponseTest where  hotelRequestKey=@hotelRequestKey and HotelName <> '' and Rating in ( select * from ufn_CSVToTable ( @hotelRatings ))and  minRate between   @minPrice and  @maxPrice and distance between  @mindistance and @maxdistance   and ChainCode = case when @chainCode ='' then ChainCode else @chainCode end  and HotelId 
in
(select distinct Hotelid from [HotelContent].[dbo].HotelAmenities where AmenityId in (select * from ufn_CSVToTable (@hotelAmenities) ) group by HotelId 
having
COUNT(hotelid)=( select COUNT(*) from ufn_CSVToTable ( @hotelAmenities)) 
...
Select MIN (minRate)as LowestPrice ,MAX (minRate)as HighestPrice From vw_sabreHotelResponseTest where  hotelRequestKey=@hotelRequestKey and HotelName <> ''

Select  Distinct MIN ( minRate) As BestPrice,Rating as Rating From  vw_sabreHotelResponseTest where hotelRequestKey=@hotelRequestKey group by Rating order by Rating 

Select MIN (distance)as Minimumdistance ,MAX (distance)as Maximumdistance From  vw_sabreHotelResponseTest where hotelRequestKey=@hotelRequestKey 


/***** Matrix for all brands as per distance ****/

 
 select min(minrate) as minRate ,chaincode ,ChainName, 0 as mindistance ,2 as Maxdistance,Rating  from  vw_sabreHotelResponseTest where hotelRequestKey=@hotelRequestKey and Rating in ( select * from vault.dbo.ufn_CSVToTable ( @hotelRatings )) and distance between 0 and 2  group by chaincode ,chainname ,Rating 
   union 
   select min(minrate) as minRate  ,chaincode ,ChainName, 2 as mindistance ,5 as Maxdistance,Rating  from  vw_sabreHotelResponseTest where hotelRequestKey=@hotelRequestKey and Rating in ( select * from vault.dbo.ufn_CSVToTable ( @hotelRatings )) and distance > 2 and distance <5   group by chaincode ,chainname  ,Rating
   union 
 select min(minrate) as minRate  ,chaincode ,ChainName, 5 as mindistance ,10 as Maxdistance ,Rating from  vw_sabreHotelResponseTest where hotelRequestKey=@hotelRequestKey and Rating in ( select * from vault.dbo.ufn_CSVToTable ( @hotelRatings )) and   distance > 5   group by chaincode ,chainname  ,Rating
 
 select COUNT(*)as NoOfHotels,'0-2' as distance  from  vw_sabreHotelResponseTest where hotelRequestKey=@hotelRequestKey and Rating in ( select * from vault.dbo.ufn_CSVToTable ( @hotelRatings )) and distance between 0 and 2  
 union
 select COUNT(*)as NoOfHotels,'2-5' as distance  from  vw_sabreHotelResponseTest where hotelRequestKey=@hotelRequestKey and Rating in ( select * from vault.dbo.ufn_CSVToTable ( @hotelRatings )) and distance > 2 and distance <5
 union
 select COUNT(*)as NoOfHotels,'>5' as distance  from  vw_sabreHotelResponseTest where hotelRequestKey=@hotelRequestKey and Rating in ( select * from vault.dbo.ufn_CSVToTable ( @hotelRatings )) and   distance > 5 
