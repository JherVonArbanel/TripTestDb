...
CREATE PROCEDURE [dbo].[usp_GetTripsITS_Irop_ng_20161123] 
--DECLARE
...
--		RowID   INT,                          
	...
	BEGIN 
		...
		BEGIN
			...
			SELECT --ROW_NUMBER() OVER (ORDER BY Trip.startDate), 
				...
				,CASE WHEN Trip.StartDate < @GMT_Today_4_AM AND Trip.tripStatusKey IN (2,3)
				Then 'Past'
				WHEN Trip.StartDate >= @GMT_Today_4_AM AND Trip.tripStatusKey IN (2,3) 
				THEN 'Purchased'
	...
	SELECT ROW_NUMBER() OVER (ORDER BY carresponsekey), 'car' AS TYPE, t.tripKey, t.tripName, u.userFirstName, u.userLastName, u.userKey
		...
		, '' as HotelRating, 0 as  DiscountFare, 0 
	FROM #tbl_vw_TRipCarResponse t WITH (NOLOCK) 
		--INNER JOIN TripCarResponse res WITH (NOLOCK) ON t.tripKey 
	...
	SELECT ROW_NUMBER() OVER (ORDER BY hotelresponsekey), 'hotel' AS TYPE, t.tripkey, t.tripName, u.userFirstName, u.userLastName, u.userKey
		...
		, t.CityName, t.StateCode, t.CountryCode, t.Rating AS HotelRating, 0 AS DiscountFare, 0 
