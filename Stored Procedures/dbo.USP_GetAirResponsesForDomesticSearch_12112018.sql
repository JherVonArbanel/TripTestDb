...
CREATE PROCEDURE [dbo].[USP_GetAirResponsesForDomesticSearch_12112018](  
--DECLARE
...
-- SELECT @airSubRequestKey=194536,@airRequestTypeKey=1,@SuperSetairLines=N'',@allowedOperatingAirlines=N'',@airLines=N'',@price=2147483647,@pageNo=0,@pageSize=30,@NoOfStops=N'-1',@drillDownLevel=N'0',@gdsSourcekey=2,@minTakeOffDate='2018-08-09 00:00:00',@maxTakeOffDate='2020-11-09 00:00:00',@minLandingDate='2018-08-09 00:00:00',@maxLandingDate='2020-11-09 00:00:00',@isIgnoreAirlineFilter=N'False',@isTotalPriceSort=N'True',@excludeAirline=N'WN',@siteKey=9,@matrixView=1,@maxNoofstops=2,@MaxFareTotal=0,@UserKey=739,@UserGroupKey=234,@CompanyKey=335,@CutOffSalesPriorDepartureInMinutes=35,@isMultiBrand=1,@isCabinUniquification=1,@EventId=0,@changeOverTimeinMinutes=60,@changeOverTimeinMinutes_AlternateAirport=180,@IsDomesticRegionTravel=1,@IsSameDayReturnOWAllowed=1
	...
	(
		...
		[ThirdPartySessionId] [nvarchar](100),
	 CONSTRAINT [PK_AirSubRequest_1] PRIMARY KEY CLUSTERED 
	(
		[airSubRequestKey] 
	)
	) ON [PRIMARY]
	...
	--ALTER TABLE #AirResponse ADD PRIMARY KEY (airResponseID)
	...
	--ALTER TABLE #Airsegments ADD PRIMARY KEY (airSegmentKey)
	...
	SELECT * FROM AirResponse WITH(NOLOCK) WHERE airSubRequestKey IN (SELECT airSubRequestKey FROM #AirSubRequest)
	...
	SELECT * FROM AirSegments WITH(NOLOCK) WHERE airResponseKey in (SELECT airResponseKey FROM #AirResponse)
...
CREATE NONCLUSTERED INDEX [IDX_airResponseMultiBrandKey_airResponseKey] ON #NormalizedAirResponsesMultiBrand ([airresponseMultiBrandkey])
	...
	IF(@airBundledRequest <> 0 AND @airBundledRequest IS NOT NULL)
		SELECT @isSameDaySearch = CASE WHEN DATEDIFF(MINUTE,CONVERT(date,airRequestArrivalDate)
			,CONVERT(date,airRequestDepartureDate)) = 0 THEN 1 ELSE 0 END 
		FROM #AirSubRequest WITH(NOLOCK) where airSubRequestKey = @airBundledRequest

	IF(@isSameDaySearch = 0 AND @IsDomesticRegionTravel = 0)
	BEGIN
		SELECT @isSameDaySearch = CASE WHEN DATEDIFF(MINUTE,CONVERT(date,airRequestArrivalDate)
			,CONVERT(date,airRequestDepartureDate)) <= 2 THEN 1 ELSE 0 END 
		FROM #AirSubRequest WITH(NOLOCK) where airSubRequestKey = @airBundledRequest
	END
	...
	BEGIN
		...
		SELECT policyDetailKey,		policyKey,				policyName,					farePolicyAmt, 				domFareTol,						domHighFareTol, 
			   ...
			   AdvancePurchasePrice, IsBasicUnselectable,	ApplyBasicUnselectable ,	IsFlagBasicUnselectable,	IsSuppressAirline 
				...
				BEGIN 
					...
					BEGIN
						...
						BEGIN
							...
							order by (airPriceBaseDisplay + airPriceTaxDisplay) asc
						END
						--select * from @SELECTedResponseMultiBrand
...
END
	...
	BEGIN
		...
		WHERE airresponseMultiBrandkey IN (SELECT A.airresponseMultiBrandkey 
									FROM #AdditionalFares A 
									WHERE ROUND(A.TotalPriceToDisplay,2) > ROUND(@AdvancePurchasePrice,2)
									AND DATEDIFF(dd,getdate(),@TripFromDate) < ROUND(@AdvancePurchaseDays,2))
		END
	END
END
	...
	SELECT airresponseMultiBrandkey,airLegBrandName,TotalPriceToDisplay,childresponsekey,isRefundable,ReasonCode,airResBookDesigCode, IsSuppressed
	FROM #AdditionalFares A
