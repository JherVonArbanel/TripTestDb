...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[USP_GetBundledAirResponses_Ashima_24012017]
...
BEGIN
 ...
 SET NOCOUNT ON   
 SELECT @airSubRequestKey=6305,@airRequestTypeKey=1,@SuperSetairLines=N'',@allowedOperatingAirlines=N'',@airLines=N'',@price=2147483647,@pageNo=0,@pageSize=30,@NoOfStops=N'-1',@drillDownLevel=N'0',@minTakeOffDate='2017-10-17 00:00:00',@maxTakeOffDate='2020-01-17 00:00:00',@minLandingDate='2017-10-17 00:00:00',@maxLandingDate='2020-01-17 00:00:00',@isIgnoreAirlineFilter=N'False',@isTotalPriceSort=N'True',@excludeAirline=N'WN',@siteKey=70,@CutOffSalesPriorDepartureInMinutes=35,@MaxFareTotal=0
...
--  STEP3-CREATE TEMP VARIABLE FOR AirSegments 
 ...
 CREATE NONCLUSTERED INDEX IX_airresponsekey ON #AirSegments(airresponsekey,airLegNumber)
 CREATE NONCLUSTERED INDEX IX_airSegmentMarketingAirlineCode ON #AirSegments(airSegmentMarketingAirlineCode)
	...
	IF OBJECT_ID('TEMPDB..#tempResponseToRemove') IS NOT NULL
	DROP TABLE #tempResponseToRemove
	CREATE TABLE #tempResponseToRemove (airresponsekey uniqueidentifier)   
	CREATE NONCLUSTERED INDEX IX_airresponsekey ON #tempResponseToRemove(airresponsekey)
	-- declare Tables
	IF OBJECT_ID('TEMPDB..#tblAirlinesGroup') IS NOT NULL
	DROP TABLE #tblAirlinesGroup
	CREATE TABLE #tblAirlinesGroup (marketingAirline varchar(10),operatingAirline varchar(10), groupKey int)
	CREATE NONCLUSTERED INDEX IX_groupKey on #tblAirlinesGroup(groupKey,marketingAirline,operatingAirline)
	
	IF OBJECT_ID('TEMPDB..#tblSuperAirlines') IS NOT NULL
	DROP TABLE #tblSuperAirlines
	CREATE TABLE #tblSuperAirlines( marketingAirline varchar(10))
	CREATE NONCLUSTERED INDEX IX_marketingAirline on #tblSuperAirlines(marketingAirline)
	
	IF OBJECT_ID('TEMPDB..#tblOperatingAirlines') IS NOT NULL
	DROP TABLE #tblOperatingAirlines
	CREATE TABLE #tblOperatingAirlines(operatingAirline VARCHAR(10))
	CREATE NONCLUSTERED INDEX IX_operatingAirline ON #tblOperatingAirlines(operatingAirline)
...
--SETP5 :GET ALL RESPONSES TO REMOVE BASED ON FILTERS 
	...
	BEGIN
		...
		BEGIN
			...
			FROM AirSegments S WITH(NOLOCK) 
	...
	BEGIN 
	  ...
	  IF exists (SELECT  s.airResponseKey FROM AirSegments s WITH(NOLOCK) 
	  INNER JOIN AirResponse resp WITH(NOLOCK) on s.airResponseKey =resp.airResponseKey   
	  INNER JOIN AirSubRequest subReq WITH(NOLOCK) on resp.airSubRequestKey =subReq.airSubRequestKey  
	  WHERE airRequestKey = @airRequestKey and airSegmentMarketingAirlineCode IN (SELECT * FROM @tblExcludedAirlines)) 
	  BEGIN
	...
	BEGIN 
		...
		IF EXISTS(SELECT DISTINCT s.airResponseKey FROM AirSegments s WITH(NOLOCK) 
		INNER JOIN AirResponse resp WITH(NOLOCK) on s.airResponseKey =resp.airResponseKey   
		INNER JOIN AirSubRequest subReq WITH(NOLOCK) on resp.airSubRequestKey =subReq.airSubRequestKey  
		WHERE airRequestKey = @airRequestKey 
		AND ((S.airSegmentDepartureAirport IN (SELECT * FROM @tblExcludedAirport)) OR (S.airSegmentArrivalAirport IN (SELECT * FROM @tblExcludedAirport))) ) 
 ...
 DECLARE @tmpAirline  TABLE   
  ...
  airLineCode VARCHAR (200)   
	...
	BEGIN   
		...
		SELECT DISTINCT seg1.airSegmentMarketingAirlineCode FROM AirSegments seg1
...
CREATE NONCLUSTERED INDEX IX_airresponsekey ON #airResponseResultset(airresponsekey)
CREATE NONCLUSTERED INDEX IX_INC_airResponseKey ON 	#airResponseResultset(airPrice,totalcost)
include(airResponseKey)
CREATE NONCLUSTERED INDEX IX_INC_airresponsekey_1 ON #airResponseResultset(airresponsekey)
include(airPriceBaseDisplay,actualTakeOffDateForLeg,MarketingAirlineName) 
 ...
 (  
  ...
  airOnePriceTaxInfantWithSeat float
	...
	CREATE NONCLUSTERED INDEX IX_airOneResponsekey  ON #tempOneWayResponses(airOneResponsekey)
	INCLUDE(airOneBaseDisplay,airOneTaxDisplay,airsubRequestkey,cabinClass,airlegnumber)
...
(  
  ...
  otherlegPrice float ,otherlegtax float ,airlegnumber int   
	...
	(SELECT seg.airresponsekey,result.airOneBaseDisplay ,
	CASE WHEN COUNT(seg.airresponsekey )-1 > 1 THEN 1 ELSE  COUNT(seg.airresponsekey )-1 END ,
	COUNT(distinct seg.airSegmentMarketingAirlineCode ),MIN(airSegmentDepartureDate ) ,
...
(  
   ...
   END 
	...
	SELECT (case when @isTotalPriceSort = 0 then MIN (airPriceBaseDisplay)  else MIN (totalCost ) end ) AS LowestPrice ,  (case when @isTotalPriceSort = 0 then MAX (airPriceBaseDisplay)  else MAX (totalCost ) end ) AS HighestPrice 
	FROM #airResponseResultset  result1   
	...
	if EXISTS ( SELECT COUNT (*) FROM @tmpAirline)    
	BEGIN   
