...
/***********************************             
 ...
 -- exec USP_GetHotelResponsesForRequest2 60447,'','1,2,3,4,5','','','','','','',1,10,'','DFW',0,0,1,0,0   
        
 **********************************/            
CREATE PROCEDURE [dbo].[USP_GetHotelResponsesForRequest2]  
(   
--declare   
	@hotelRequestKey  INT,  
...
--SELECT GetDate() AS [1]
 ...
 --print '1'  
   ...
   (
        ...
        richMediaUrl,hotelSequence, offerName, primaryOffertext, secondaryOffertext, linktoPage, inStripOfferImage, customHotelImageUrl,cmsHotelName,lowRate,highRate, realRating, rowNumber , IsPromo , PromoDescription , AverageBaseCost 
	)
   (    
		SELECT HRD.hotelResponseKey,HRD.supplierHotelKey,VW.hotelRequestKey,HRD.SupplierId,VW.minRate,HT.HotelName,HT.Rating,HT.RatingType,
			HT.ChainCode,VW.HotelId,HT.Latitude,HT.Longitude,HT.Address1,HT.CityName,HT.StateCode,HT.CountryCode,HT.ZipCode,'','',HT.CityCode, 
			'',--ISNULL(CL.CountryName,'') as CountryName,
			0,--ISNULL(AH.Distance,0) as Distance,
			null,null,
			'',--HD.HotelDescription,
			HC.ChainName,0,
			HI.SupplierImageURL,HRD.preferenceOrder,HRD.corporateCode,'','',HT.reviewRating as tripAdvisorRating ,'','',''
			...
			CH.H1Title As cmsHotelName, HRD.lowRate, HRD.highRate ,HT.Rating,
			ROW_NUMBER() OVER(PARTITION BY vw.HotelId,vw.hotelRequestKey,vw.minRate ORDER BY HRD.SupplierId ASC) AS rowNumber,
			isnull(HRD.IsPromoTrue,0) as IsPromoTrue,Isnull(HRD.PromoDescription,'') as PromoDescription ,Isnull(HRD.AverageBaseRate,0) as   AverageBaseRate		
		...
		inner join @vw VW ON vw.HotelId = HT.HotelId AND VW.HotelRequestKey =@hotelRequestKey  
		...
		--Left outer join HotelContent.dbo.AirportHotels AS AH WITH(NOLOCK) ON AH.HotelId = VW.HotelId AND AH.AirportCode = HT.CityCode        
		--LEFT OUTER JOIN [vault].[dbo].CountryLookup CL WITH(NOLOCK) ON HT.CountryCode = CL.CountryCode  
		--Left outer join HotelContent.dbo.HotelDescriptions AS HD WITH(NOLOCK) ON HD.HotelId = VW.HotelId         
		Left outer join HotelContent.dbo.HotelChains AS HC WITH(NOLOCK) ON HC.ChainCode = HT.ChainCode         
		LEFT OUTER JOIN [CMS].[dbo].[CustomHotelImages] AS CHI WITH(NOLOCK) ON CHI.HotelId = VW.HotelId AND CHI.OrderId = 1    
		LEFT OUTER JOIN [CMS].[dbo].[CustomHotels] AS CH WITH(NOLOCK) ON CH.HotelId = VW.HotelId     
		LEFT OUTER JOIN     
		(    
		...
		)	mer ON VW.HotelId = mer.HotelVendorMatch OR (mer.HotelChainMatch <> '0' AND HT.ChainCode = ISNULL(HT.ChainCode,mer.HotelChainMatch))    
		INNER JOIN dbo.HotelResponse AS HRD WITH(NOLOCK) ON HRD.hotelRequestKey = VW.hotelRequestKey AND  HRD.HotelId = VW.HotelId AND HRD.minRate = VW.minRate        

		--FROM @vw VW         
		--	INNER JOIN HotelContent.dbo.Hotels AS HT WITH(NOLOCK) ON HT.HotelId = VW.HotelId    
		--	LEFT OUTER JOIN [CMS].[dbo].[CustomHotelGroupMapping] HGM WITH(NOLOCK) ON HGM.HotelId = HT.HotelId AND HGM.HotelGroupId = @hotelGroupId    
		--	Left outer join HotelContent.dbo.AirportHotels AS AH WITH(NOLOCK) ON AH.HotelId = VW.HotelId AND AH.AirportCode = HT.CityCode        
		--	LEFT OUTER JOIN [vault].[dbo].CountryLookup CL WITH(NOLOCK) ON HT.CountryCode = CL.CountryCode  
		--	Left outer join HotelContent.dbo.HotelImages_Exterior AS HI WITH(NOLOCK) ON HI.HotelId = VW.HotelId  AND  HI.ImageType = 'Exterior'      
		--	Left outer join HotelContent.dbo.HotelDescriptions AS HD WITH(NOLOCK) ON HD.HotelId = VW.HotelId         
		--	Left outer join HotelContent.dbo.HotelChains AS HC WITH(NOLOCK) ON HC.ChainCode = HT.ChainCode         
		--	INNER JOIN dbo.HotelResponse AS HRD WITH(NOLOCK) ON HRD.hotelRequestKey = VW.hotelRequestKey AND  HRD.HotelId = VW.HotelId AND HRD.minRate = VW.minRate        
		--	LEFT OUTER JOIN [CMS].[dbo].[CustomHotelImages] AS CHI WITH(NOLOCK) ON CHI.HotelId = VW.HotelId AND CHI.OrderId = 1    
		...
		--WHERE VW.HotelRequestKey =@hotelRequestKey  
...
BEGIN  
 --Select GETDATE() [9]  
 SELECT  Distinct MIN ( minRate) AS BestPrice,Rating AS Rating 
...
--SELECT GetDate() AS [29]
  ...
  SELECT RM.RegionId, RM.HotelId From HotelContent..RegionHotelIDMapping RM WITH(NOLOCK) 
...
Declare @RHM Table (RegionId int, HotelId int)  
Insert Into @RHM (RegionId,HotelId)   
 SELECT  RM.RegionId  
   ,RM.HotelId   
 From HotelContent..RegionHotelIDMapping RM WITH(NOLOCK)  
   INNER JOIN @hotelResponseResult HT ON  RM.HotelId = HT.HotelId  
