...
CREATE PROCEDURE [dbo].[usp_GetTripsITS_Irop_ng_20161010] 
--DECLARE
...
--SELECT @SiteGUID=N'1F33CD4E-BCCC-466C-A618-2C5A36AB5B93',@CompanyGUID=N'4CB134CD-6C48-40AF-BBD2-2EA1F0802B5B',@UserGUID=N'5568CD20-A7F3-4535-BF1B-AB7E66863F72',@PageName=N'mytrips',@PNR=N'',@UDIDandValue=NULL
...
*/
	...
	DECLARE  @GMTNOW DATETIME = DATEADD(HH, 12, GETDATE())
		,@GMT_Today_4_AM DATETIME 
	SET @GMT_Today_4_AM = DATEADD(HOUR, -12, GETDATE())
	...
	BEGIN 
		...
		BEGIN
			...
			SELECT --ROW_NUMBER() OVER (ORDER BY Trip.startDate), 
				...
				, CASE	WHEN Trip.StartDate < @GMT_Today_4_AM AND Trip.tripStatusKey IN (2,3) Then 'Past'
						WHEN Trip.StartDate >= @GMT_Today_4_AM AND Trip.tripStatusKey IN (2,3) THEN 'Purchased'
	...
	SELECT trip.tripKey, trip.tripName, trip.userKey, TR.recordLocator, trip.endDate, trip.startDate, trip.tripStatusKey, TR.actualCarPrice
		...
		, TR.NoOfDays, '' As CityName, '' AS StateCode, '' AS CountryCode, '' As HotelRating, 0 AS DiscountFare
	...
	SELECT ROW_NUMBER() OVER (ORDER BY tripAirsegmentkey) segmentOrder, 'air' AS TYPE, trip.tripKey, trip.tripName, u.userFirstName
		...
		, '' as StateCode, '' as CountryCode, '' as HotelRating, ISNULL(resp.discountedBaseFare,0) as DiscountFare 
	FROM Trip WITH (NOLOCK) 
	...
	SELECT ROW_NUMBER() OVER (ORDER BY carresponsekey), 'car' AS TYPE, t.tripKey, t.tripName, u.userFirstName, u.userLastName, u.userKey
		, t.recordLocator, t.endDate, t.startDate, t.tripStatusKey, actualCarPrice, actualCarTax, carVendorKey, carCompanyName, carLocationCode
		, carLocationCode, NULL, PickUpdate, dropOutDate, SippCodeClass, NULL AS Ratingtype, t.carResponseKey, t.recordLocator, t.siteKey
	...
	SELECT ROW_NUMBER() OVER (ORDER BY hotelresponsekey), 'hotel' AS TYPE, t.tripkey, t.tripName, u.userFirstName, u.userLastName, u.userKey
		...
		, hotelname, cityname + ',' + StateCode, cityname + ',' + StateCode, NULL, checkindate, checkoutdate, NULL, Ratingtype
		, t .hotelResponseKey, t .recordLocator, t .siteKey, t .createdDate, t .tripRequestKey , '' As VehicleCompanyName ,0 as NoofDays
		, t.CityName, t.StateCode, t.CountryCode, t.Rating AS HotelRating, 0 AS DiscountFare 
...
--ALTER TABLE #tmpTrip ADD TotalCost FLOAT
	...
	BEGIN
		...
		SELECT	 Tripkey
				, CompanyUDIDNumber
				, ISNULL(PassengerUDIDValue, '') + ISNULL(CompanyUDIDOptionText, '')
