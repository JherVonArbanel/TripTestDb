...
GO
/*
exec USP_GetMyTrip  562934,51 ,1,1000,0,1,0,'',''

exec USP_GetMyTrip  570467,70 ,1,1000,0,1,0,'',',purchased,traveled,held'

exec USP_GetMyTrip  562934,51 ,1,1000,0,0,0,'','purchased'

exec USP_GetMyTrip   562934,51 ,1,1000,0,0,0,'','traveled'

exec USP_GetMyTrip  562934,51 ,1,1000,0,0,0,'','purchased,traveled'

 
*/
CREATE PROCEDURE [dbo].[USP_GetMyTrip]                
 @UserKey int,                
 @SiteKey int,              
 @FromIndex INT = 1,                          
 @ToIndex INT = 10 ,              
...
ELSE               
BEGIN              
	INSERT INTO @crowdType(crowdtype) VALUES ('purchased'),('cancelled'),('purchased expired'),('cancelled expired'),('held'),('pending approval')
END               
 ...
 TC.Component as tripComponents ,                     
 ...
 FROM [Trip] T WITH(NOLOCK)                            
  ...
  inner join TripComponentLookUp TC  WITH (NOLOCK) ON TC.componentTypeKey = T.tripComponentType  and TC.TripComponentVal  = TC.componentTypeKey             
  WHERE T.siteKey = @SiteKey AND T.userKey = @UserKey 
               
	 	-- select *  from @me  order by 2 desc

	--SELECT DATEADD(hour, -24, GETDATE()) ;
	--IF(@tripType ='held')
	if CHARINDEX('held',@tripType) > 0
	BEGIN
		DELETE FROM @me WHERE (createddate < DATEADD(hour, -24, GETDATE())) and tripStatusKey=7
	END
 ...
 UPDATE TD               
  ...
  (CASE WHEN startDate < GETDATE() THEN 'following crowd expired' ELSE 'following crowd' END)               
   ...
   UPDATE TD SET TD.tripFilterType = 'purchased'              
   FROM @me TD WHERE (TD.tripStatusKey = 2 OR TD.tripStatusKey = 1 OR TD.tripStatusKey = 15 OR TD.tripStatusKey = 5)              
   
   UPDATE TD SET TD.tripFilterType = 'held' ,  TD.type = 'held'               
   FROM @me TD WHERE (TD.tripStatusKey = 7)              
                 
   UPDATE TD               
    SET TD.isexpired = 
	(CASE WHEN               
    (CASE WHEN (TD.tripStatusKey = 2 OR TD.tripStatusKey = 1 OR TD.tripStatusKey = 15 OR TD.tripStatusKey = 5) 
	THEN dateadd(HOUR, 18, enddate)              
    ELSE startDate END) < GETDATE() THEN 1 ELSE 0 END)              
   FROM @me TD                
           
  UPDATE TD SET  TD.tripFilterType = 'traveled'              
    FROM @me TD WHERE tripFilterType = 'purchased' and isexpired = 1              

  UPDATE TD SET TD.tripFilterType = 'held' ,  TD.type = 'held'               
   FROM @me TD WHERE (TD.tripStatusKey = 7) 

   UPDATE TD SET TD.tripFilterType = 'pending approval' ,TD.type = 'pending approval'              
   FROM @me TD WHERE (TD.tripStatusKey = 8)
...
-----------------------------------------------------------------------------------              
 ...
 ELSE              
  ...
  ELSE          
	...
	--  select *  from @finalme  order by id
