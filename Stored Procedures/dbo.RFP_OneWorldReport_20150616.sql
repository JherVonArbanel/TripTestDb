...
CREATE procedure [dbo].[RFP_OneWorldReport_20150616]
(    
 @isOrganiser bit,  
 @userKey int,  
 @currency varchar(5),  
...
BEGIN    
 ...
 BEGIN  
  ...
  ISNULL(SUM(case when (T.tripStatusKey in (2,12,3) ) then 
	(
		--case when T.tripstatuskey = 12 then TTI.TotalFare * ExRate.ExchangeRate  else (TAR.actualAirPrice) * ExRate.ExchangeRate end
 (
	Case when t.tripStatusKey = 12 then  isnull(TTI.TotalFare,0) * (CASE WHEN TAR.CurrencyCodeKey = 'USD' THEN ExRate.ExchangeRate ELSE CASE WHEN (tm.currencyCode = ExRate.currencyCode) THEN 1 ELSE ExRate.ExchangeRate / tm.exchangeRateAmount END END)
	else isnull(TAR.actualAirPrice,0) * (CASE WHEN TAR.CurrencyCodeKey = 'USD' THEN ExRate.ExchangeRate ELSE CASE WHEN (tm.currencyCode = ExRate.currencyCode) THEN 1 ELSE ExRate.ExchangeRate / tm.exchangeRateAmount END END) end 
 ) 
 ...
 (
	...
	FROM vault.dbo.ExchangeRate E 
		LEFT OUTER JOIN vault.dbo.Currency C ON e.currencyKey = c.currencyKey 
	WHERE exchangeRateDate = (SELECT  MAX(exchangeRateDate ) FROM vault.dbo.ExchangeRate)
 ) tm ON tm.currencyCode = TAR.CurrencyCodeKey 
