...
-- exec USP_GetHashTagBucketListForUser 560799
-- =============================================
CREATE PROCEDURE [dbo].[USP_GetHashTagBucketListForUser]  
	(@userKey INT = 0 )
AS
BEGIN
	...
	SET NOCOUNT ON;
	DECLARE @tbl TABLE (TripTo VARCHAR(50), ImageURL VARCHAR(5000),  CityName VARCHAR(150), CityIdFromLookUp INT DEFAULT 0 , Hashtag varchar(150),  TotalSavings FLOAT DEFAULT(0), UserSelected bit DEFAULT 0, PrefVacationId INT DEFAULT 0, IsCustom bit default 0 )
	
	-- get all destination having totalsavings 
	insert into @tbl(TripTo, TotalSavings, ImageURL)
	SELECT TD.tripTo,
			SUM
		(
		...
		) , MAX(T.DestinationSmallImageURL)
    ...
    UPDATE t SET CityName = AL.CityName, Hashtag =  LOWER(REPLACE(AL.CityName ,' ','')), t.CityIdFromLookUp = CL.cityKey
	FROM @tbl t 
		INNER JOIN trip..AirportLookup AL WITH (NOLOCK) ON t.TripTo = AL.AirportCode 
		LEFT JOIN vault.dbo.CityLookup CL ON AL.CityName = CL.CityName 
	...
	SELECT *  FROM @tbl WHERE TotalSavings > 0  AND CityIdFromLookUp IS NOT NULL
END
GO
