...
Create PROCEDURE [dbo].[usp_GetTripDetailsForMultipax_20180608] (                          
--DECLARE    
	@tripID   int   , 
	@tripRequestID Int = 0 
) AS                              
...
---------------------------------------------     
 ...
 SELECT DISTINCT T.tripKey, t.tripRequestKey, segments.tripAirSegmentKey, segments.airSegmentKey,     
  ...
  legs.ValidatingCarrier as LegValidatingCarrier, segments.airSegmentBrandName, segments.IsChangeTripSeg, AirResp.redeemPoints,AirResp.redeemAuthNumber    
 ...
 FROM TripAirSegments  segments  WITH(NOLOCK)     
  ...
  INNER JOIN Trip t WITH(NOLOCK) ON AirResp.tripGUIDKey  = @TripPurchasedKey
 ...
 FROM vw_TripHotelResponseDetails hotel WITH(NOLOCK)     
  INNER JOIN trip t WITH(NOLOCK) ON (hotel.tripGUIDKey = @TripPurchasedKey) AND T.tripStatusKey <>17     
  INNER JOIN @tblTrip tt on tt.tripKey = t.tripKey AND T.tripStatusKey <>17     
 ...
 FROM vw_TripCarResponseDetails TD          
  INNER JOIN dbo.Trip T WITH (NOLOCK) ON (TD.tripGUIDKey = @TripPurchasedKey) AND T.tripStatusKey <>17   --TD.tripKey = T.tripKey    
  INNER JOIN @tblTrip tt ON tt.tripKey  = T.tripKey AND T.tripStatusKey <> 17     
  INNER JOIN vault.dbo.GDSSourceLookup GL WITH(NOLOCK) ON GL.GDSName = TD.SupplierId     
 ...
 SELECT T.tripKey, TD.recordLocator, TD.carResponseKey, confirmationNumber, carVendorKey, supplierId, carCategoryCode, carLocationCode    
  ...
  , TD.IsChangeTripCar    
 FROM vw_TripCarResponseDetails TD     
  INNER JOIN dbo.Trip T WITH (NOLOCK) ON (TD.tripguidkey =@TripPurchasedKey) and T.tripStatusKey <>17     
  INNER JOIN @tblTrip tt ON tt.tripKey  = T.tripKey AND T.tripStatusKey <>17     
  INNER JOIN vault.dbo.GDSSourceLookup GL WITH(NOLOCK) ON GL.GDSName = TD.SupplierId     
 ...
 INNER JOIN trip t WITH(NOLOCK) ON (cruise.tripGuidkey = @TripPurchasedKey )AND T.tripStatusKey <> 17     
 INNER JOIN @tblTrip tt ON tt.tripKey = t.tripKey and T.tripStatusKey <>17     
 ORDER BY t.tripKey     
 ...
 FROM TripAirLegPassengerInfo TLP  WITH(NOLOCK)     
  ...
  INNER JOIN Trip T WITH(NOLOCK) ON TA.tripGUIDKey = @TripPurchasedKey
 ...
 FROM TripAirSegmentPassengerInfo TSP  WITH(NOLOCK)     
  ...
  INNER JOIN Trip T ON (TA.tripGUIDKey =@TripPurchasedKey)     
  INNER JOIN @tblTrip Tbl ON t.tripKey = tbl.tripKey AND T.tripStatusKey <> 17     
 ...
 INNER JOIN TripHotelResponse TH WITH(NOLOCK) ON t.tripPurchasedKey =@TripPurchasedKey   
