...
CREATE PROCEDURE [dbo].[usp_GetTripsITS_Irop_ng_Manifest_inviteCand]
--DECLARE
...
PRINT 'CompanyKey --->' + CONVERT(VARCHAR, @CompanyKey)
	...
	(                          
		...
		, InviteType		NVARCHAR(50)
		--, AttendeeNotes		NVARCHAR(MAX)
		--, OrganizerNotes	NVARCHAR(MAX)
		--, AttendeeNotesModifiedOn DateTime
		--, OrganizerNotesModifiedOn DateTime
		--, AttendeeNotesModifiedBy int
		--, OrganizerNotesModifiedBy int
	)              
	...
	*/
	...
	WHERE	
		...
		AND ISNULL(MET.MeetingTypeKey,0) <> 0
	END
	ELSE
	BEGIN -- For Manifest of Invite Candidate
	-- pagename = today, future, past
		INSERT INTO #trps
		SELECT T.tripKey, CASE WHEN T.startDate > GETDATE() THEN 'Up Coming' ELSE 'Past' END AS PageName
	FROM trip T WITH(NOLOCK)               
		 INNER JOIN Trip..TripPassengerInfo U WITH(NOLOCK) ON T.tripKey =  U.TripKey  
	...
	WHERE	
		...
		AND ISNULL(MET.CompanyKey,0) = @CompanyKey
		AND ISNULL(MET.MeetingTypeKey,0) <> 0
		--AND T.startDate >= CASE WHEN @StartDate <> '' THEN @StartDate END
		--AND T.endDate <= CASE WHEN @EndDate <> '' THEN @EndDate END
	END
...
--SELECT 'Delete it', * FROM #trpsDepartureSegment
	...
	BEGIN
		...
		SELECT Trip.tripKey, Trip.TripRequestKey, Trip.tripName, Trip.userKey, Trip.recordLocator, Trip.startDate, Trip.endDate
			...
			, S.tripStatusName AS [Status], T.PageName, typ.MeetingType
		FROM #trps T 
			...
			LEFT OUTER JOIN Vault.dbo.MeetingType typ ON met.MeetingTypeKey = typ.MeetingTypeKey
	 ...
	 BEGIN
		...
		*/
		...
		SELECT trip.tripKey, trip.TripRequestKey, trip.tripName, trip.TravelerName, trip.userKey, trip.recordLocator, trip.startDate startDate
			...
			, Arr.Origin, Arr.LastLeg, Arr.Dest, Arr.Flight, Arr.[Time], Arr.[Type], trip.PageName, trip.InviteType
			, trpNotes.OrganizerNotes,trpNotes.AttendeeNotes, trpNotes.AttendeeNotesLastModifiedOn,trpNotes.OrganizerNotesLastModifiedOn
			, usr.userFirstName +' ' + usr.userLastName AS AttendeeNotesLastModifiedBy,  usr.userFirstName +' ' + usr.userLastName AS OrganizerNotesLastModifiedBy
		FROM #tmpTrip trip 
			...
			LEFT OUTER JOIN Trip.dbo.TripNotes trpNotes ON trip.tripKey = trpNotes.TripKey
			LEFT OUTER JOIN Vault.dbo.[User] usr WITH (NOLOCK) ON (trpNotes.OrganizerNotesLastModifiedBy = usr.userKey OR trpNotes.AttendeeNotesLastModifiedBy = usr.userKey)
--WHERE trip.tripKey = 244587
		...
		SELECT trip.tripKey, trip.TripRequestKey, trip.tripName, trip.TravelerName, trip.userKey, trip.recordLocator, trip.startDate startDate
			...
			, trip.IsArrangerBookForGuest
			, trip.MeetingListName EventName, trip.Traveler, trip.Invited, trip.[Status]
			, Dep.Origin, Dep.LastLeg, Dep.Dest, Dep.Flight, Dep.[Time], Dep.[Type], trip.PageName, trip.InviteType
			, trpNotes.OrganizerNotes,trpNotes.AttendeeNotes, trpNotes.AttendeeNotesLastModifiedOn,trpNotes.OrganizerNotesLastModifiedOn
			, usr.userFirstName +' ' + usr.userLastName AS AttendeeNotesLastModifiedBy,  usr.userFirstName +' ' + usr.userLastName AS OrganizerNotesLastModifiedBy
		FROM #tmpTrip trip 
			LEFT OUTER JOIN #tbl_vw_TRipCarResponse Car ON Car.tripKey=trip.tripKey 
