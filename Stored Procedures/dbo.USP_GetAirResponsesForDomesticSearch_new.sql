...
SET ANSI_NULLS ON
GO
   CREATE PROCEDURE [dbo].[USP_GetAirResponsesForDomesticSearch_new]
	(  
	...
	DECLARE @airRequestClass as VARCHAR(50)
	...
	END 
	SET @airRequestClass = (SELECT LTRIM(RTRIM(airRequestClassName)) FROM AirRequestClassLookup WHERE airRequestClassKey = (SELECT airRequestClassKey FROM TripRequest_air WHERE airRequestKey =@airRequestKey ))
	...
	--Hemali - start
	DECLARE @airEconomyPriceForAnotherLeg AS FLOAT  = 0 
    DECLARE @airPremiumEconomyPriceForAnotherLeg AS FLOAT  = 0 
	DECLARE @airBusinessPriceForAnotherLeg AS FLOAT  = 0 
	DECLARE @airPremiumBusinessPriceForAnotherLeg AS FLOAT  = 0 
	DECLARE @airFirstPriceForAnotherLeg AS FLOAT  = 0 
	DECLARE @airPremiumFirstPriceForAnotherLeg AS FLOAT  = 0 
	
	DECLARE @airEconomyTaxForAnotherLeg AS FLOAT  = 0 
    DECLARE @airPremiumEconomyTaxForAnotherLeg AS FLOAT  = 0 
	DECLARE @airBusinessTaxForAnotherLeg AS FLOAT  = 0 
	DECLARE @airPremiumBusinessTaxForAnotherLeg AS FLOAT  = 0 
	DECLARE @airFirstTaxForAnotherLeg AS FLOAT  = 0 
	DECLARE @airPremiumFirstTaxForAnotherLeg AS FLOAT  = 0 
	--Hemali - End
	...
	BEGIN   
			IF ( @airLines = '' or @airLines = 'Multiple Airlines' )   
				BEGIN   
					...
					-- Hemali start - Find lowest fare for Multiple Cabin
					
					    Select top 1 @airEconomyPriceForAnotherLeg = ISNULL(airEconSaverPrice,0),@airEconomyTaxForAnotherLeg = ISNULL(airEconSaverTax,0)
						from airresponse  Resp WITH (NOLOCK)
					    inner join AirSubRequest subReq WITH(NOLOCK) on resp.airSubRequestKey = subReq.airSubRequestKey   
					    WHERE airRequestKey  = @airRequestKey  and airSubRequestLegIndex > 1 
					    and resp.gdsSourceKey = 2
					    and airResponseKey not in ( select airresponsekey  from @tempResponseToRemove )   
						AND airSuperSaverPrice is not null AND airEconSaverPrice != 0
						AND airSuperSaverTax is not null AND airEconSaverTax != 0
						order by (airEconSaverPrice + airEconSaverTax) asc
						
						Select top 1 @airPremiumEconomyPriceForAnotherLeg = ISNULL(airEconFlexPrice,0),@airPremiumEconomyTaxForAnotherLeg = ISNULL(airEconFlexTax,0)
						from airresponse  Resp WITH (NOLOCK)
					    inner join AirSubRequest subReq WITH(NOLOCK) on resp.airSubRequestKey = subReq.airSubRequestKey   
					    WHERE airRequestKey  = @airRequestKey  and airSubRequestLegIndex > 1 
					    and resp.gdsSourceKey = 2
						...
						order by (airEconFlexPrice + airEconFlexTax) asc
						
						Select top 1 @airBusinessPriceForAnotherLeg = ISNULL(airCorporatePrice,0),@airBusinessTaxForAnotherLeg = ISNULL(airCorporateTax,0)
						from airresponse  Resp WITH (NOLOCK)
					    inner join AirSubRequest subReq WITH(NOLOCK) on resp.airSubRequestKey = subReq.airSubRequestKey   
					    WHERE airRequestKey  = @airRequestKey  and airSubRequestLegIndex > 1 
					    and resp.gdsSourceKey = 2
					    and airResponseKey not in ( select airresponsekey  from @tempResponseToRemove )   
						AND airCorporatePrice is not null AND airCorporatePrice != 0
						AND airCorporateTax is not null AND airCorporateTax != 0
						order by (airCorporatePrice + airCorporateTax) asc
						
						Select top 1 @airPremiumBusinessPriceForAnotherLeg = ISNULL(airEconUpgradePrice,0),@airPremiumBusinessTaxForAnotherLeg = ISNULL(airEconUpgradeTax,0)
						from airresponse  Resp WITH (NOLOCK)
					    inner join AirSubRequest subReq WITH(NOLOCK) on resp.airSubRequestKey = subReq.airSubRequestKey   
					    WHERE airRequestKey  = @airRequestKey  and airSubRequestLegIndex > 1 
					    and resp.gdsSourceKey = 2
					    and airResponseKey not in ( select airresponsekey  from @tempResponseToRemove )   
						AND airEconUpgradePrice is not null AND airEconUpgradePrice != 0
						AND airEconUpgradeTax is not null AND airEconUpgradeTax != 0
						order by (airEconUpgradePrice + airEconUpgradeTax) asc
						
						Select top 1 @airFirstPriceForAnotherLeg = ISNULL(airSuperSaverPrice,0),@airFirstTaxForAnotherLeg = ISNULL(airSuperSaverTax,0)
						from airresponse  Resp WITH (NOLOCK)
					    inner join AirSubRequest subReq WITH(NOLOCK) on resp.airSubRequestKey = subReq.airSubRequestKey   
					    WHERE airRequestKey  = @airRequestKey  and airSubRequestLegIndex > 1 
					    and resp.gdsSourceKey = 2
						...
						Select top 1 @airPremiumFirstPriceForAnotherLeg = ISNULL(airFirstFlexPrice,0),@airPremiumFirstTaxForAnotherLeg = ISNULL(airFirstFlexTax,0)
						from airresponse  Resp WITH (NOLOCK)
					    inner join AirSubRequest subReq WITH(NOLOCK) on resp.airSubRequestKey = subReq.airSubRequestKey   
					    WHERE airRequestKey  = @airRequestKey  and airSubRequestLegIndex > 1 
					    and resp.gdsSourceKey = 2
	...
	BEGIN  
		/**airLEg > 1 **/  
			...
			BEGIN  
				...
				BEGIN  
					--selectedResponse is OW response
					INSERT @SELECTedResponse (responsekey,fareType  ) values (@SELECTedResponseKey ,@SELECTedFareType)  
				END  
				ELSE   
				BEGIN  
                   --selectedResponse is RT Response - insert corre OW option based on flight number & airline of selectedresponse
  ...
  inner join AirSubRequest r WITH(NOLOCK) on n.airsubrequestkey = r.airSubRequestKey  WHERE airRequestKey = @airRequestKey AND airlegnumber =1
   and  airSubRequestLegIndex = 1 and flightNumber =(SELECT flightnumber FROM NormalizedAirResponses WHERE airresponsekey = @SELECTedResponseKey  and airLegNumber =1 
			...
			--hemali start -  Gettting bucket price of selected response
			SELECT @airEconomyPriceForAnotherLeg = ISNULL(airEconSaverPrice,0),@airEconomyTaxForAnotherLeg = ISNULL(airEconSaverTax,0)
			,@airPremiumEconomyPriceForAnotherLeg = ISNULL(airEconFlexPrice,0),@airPremiumEconomyTaxForAnotherLeg = ISNULL(airEconFlexTax,0)
			,@airBusinessPriceForAnotherLeg = ISNULL(airCorporatePrice,0),@airBusinessTaxForAnotherLeg = ISNULL(airCorporateTax,0)
			,@airPremiumBusinessPriceForAnotherLeg = ISNULL(airEconUpgradePrice,0),@airPremiumBusinessTaxForAnotherLeg = ISNULL(airEconUpgradeTax,0)
			,@airFirstPriceForAnotherLeg = ISNULL(airSuperSaverPrice,0),@airFirstTaxForAnotherLeg = ISNULL(airSuperSaverTax,0)
			,@airPremiumFirstPriceForAnotherLeg = ISNULL(airFirstFlexPrice,0),@airPremiumFirstTaxForAnotherLeg = ISNULL(airFirstFlexTax,0)
			FROM AirResponse resp  WITH(NOLOCK)
			inner join @SELECTedResponse SELECTed   
			on resp .airResponseKey = SELECTed .responsekey 
            --hemali end
	...
	economyRefundable bit,
	premiumEconomyRefundable bit,
	businessRefundable bit,
	premiumBusinessRefundable bit,
	firstRefundable bit,
	premiumFirstRefundable bit,
	airsegmentClassEconomy	varchar(20),
	airsegmentClassPremiumEconomy	varchar(20),
	airsegmentClassBusinessCabin	varchar(20),
	airsegmentClassPremiumBusinessCabin	varchar(20),
	airsegmentClassFirstCabin	varchar(20),
	airsegmentClassPremiumFirstCabin	varchar(20),
	)  
	...
	airOneEconomyPriceBase float, 
	airOneEconomyPriceTax float,
	airOnePremiumEconomyPriceBase float,
	airOnePremiumEconomyPriceTax float,
	airOneBusinessPriceBase float,
	airOneBusinessPriceTax float,
	airOnePremiumBusinessPriceBase float,
	airOnePremiumBusinessPriceTax float,
	airOneFirstPriceBase float,
	airOneFirstPriceTax float,
	airOnePremiumFirstPriceBase float,
	airOnePremiumFirstPriceTax float
	)      
	...
	IF ( @airRequestTypeKey = 1)   
	BEGIN   
		--RT Bundles - Leg1 details
		INSERT #AllOneWayResponses (airOneResponsekey,airOnePriceBase,airOnePriceBaseSenior ,airOnePriceTaxSenior, airOnePriceBaseChildren ,airOnePriceTaxChildren ,airOnePriceBaseInfant, airOnePriceTaxInfant,airOnePriceBaseYouth, airOnePriceTaxYouth, airOnePriceBaseTotal, airOnePriceTaxTotal,airOnePriceBaseDisplay, airOnePriceTaxDisplay,airSegmentFlightNumber,airSegmentMarketingAirlineCode,airsubRequestkey,airOnePriceTax ,airpriceTotal ,cabinclass  ,legConnections,	airOneEconomyPriceBase , airOneEconomyPriceTax , airOnePremiumEconomyPriceBase ,airOnePremiumEconomyPriceTax ,airOneBusinessPriceBase ,airOneBusinessPriceTax ,
			airOnePremiumBusinessPriceBase ,airOnePremiumBusinessPriceTax ,	airOneFirstPriceBase ,	airOneFirstPriceTax ,	airOnePremiumFirstPriceBase ,	airOnePremiumFirstPriceTax)  
		...
		airEconSaverPrice,airEconSaverTax,airEconFlexPrice,airEconFlexTax,airCorporatePrice,airCorporateTax,airEconUpgradePrice,airEconUpgradeTax,airSuperSaverPrice,airSuperSaverTax,airFirstFlexPrice,airFirstFlexTax
	...
	IF ( @airRequestTypeKey = 1 )   
	BEGIN   
	    --One way calls - inbound
	    IF ((@airRequestClass='BUSINESS') OR (@airRequestClass='FIRST'))
	    BEGIN
			INSERT #AllOneWayResponses (airOneResponsekey,airOnePriceBase,airOnePriceBaseSenior,airOnePriceTaxSenior,airOnePriceBaseChildren,airOnePriceTaxChildren,airOnePriceBaseInfant,airOnePriceTaxInfant,airOnePriceBaseYouth,airOnePriceTaxYouth,airOnePriceBaseTotal,airOnePriceTaxTotal,airOnePriceBaseDisplay, airOnePriceTaxDisplay,airSegmentFlightNumber,airSegmentMarketingAirlineCode,airsubRequestkey,airOnePriceTax ,airpriceTotal,otherLegprice,cabinclass  ,otherlegtax ,legConnections,	airOneEconomyPriceBase , airOneEconomyPriceTax , airOnePremiumEconomyPriceBase ,airOnePremiumEconomyPriceTax ,airOneBusinessPriceBase ,airOneBusinessPriceTax ,
			airOnePremiumBusinessPriceBase ,airOnePremiumBusinessPriceTax ,	airOneFirstPriceBase ,	airOneFirstPriceTax ,	airOnePremiumFirstPriceBase ,	airOnePremiumFirstPriceTax )  
			...
			--hemali start
			,Case WHEN (ISNull(airEconSaverPrice,0) > 0 AND @airEconomyPriceForAnotherLeg > 0) OR @airRequestType = 1 then (airEconSaverPrice + @airEconomyPriceForAnotherLeg)  else 0 end,
			Case WHEN (ISNull(airEconSaverTax,0) > 0 AND @airEconomyTaxForAnotherLeg > 0) OR @airRequestType = 1 then (airEconSaverTax + @airEconomyTaxForAnotherLeg)  else 0 end,
      		Case WHEN (ISNull(airEconFlexPrice,0) > 0 AND @airPremiumEconomyPriceForAnotherLeg > 0) OR @airRequestType = 1 then (airEconFlexPrice + @airPremiumEconomyPriceForAnotherLeg)  else 0 end,
			Case WHEN (ISNull(airEconFlexTax,0) > 0 AND @airPremiumEconomyTaxForAnotherLeg > 0) OR @airRequestType = 1 then (airEconFlexTax + @airPremiumEconomyTaxForAnotherLeg)  else 0 end,
			Case WHEN (ISNull(airCorporatePrice,0) > 0 AND @airBusinessPriceForAnotherLeg > 0) OR @airRequestType = 1 then (airCorporatePrice + @airBusinessPriceForAnotherLeg)  else 0 end ,
			Case WHEN (ISNull(airCorporateTax,0) > 0 AND @airBusinessTaxForAnotherLeg > 0) OR @airRequestType = 1 then (airCorporateTax + @airBusinessTaxForAnotherLeg)  else 0 end ,
			Case WHEN (ISNull(airEconUpgradePrice,0) > 0 AND @airPremiumBusinessPriceForAnotherLeg > 0) OR @airRequestType = 1 then (airEconUpgradePrice + @airPremiumBusinessPriceForAnotherLeg)  else 0 end ,
			Case WHEN (ISNull(airEconUpgradeTax,0) > 0 AND @airPremiumBusinessTaxForAnotherLeg > 0) OR @airRequestType = 1 then (airEconUpgradeTax + @airPremiumBusinessTaxForAnotherLeg)  else 0 end ,
			Case WHEN (ISNull(airSuperSaverPrice,0) > 0 AND @airFirstPriceForAnotherLeg > 0) OR @airRequestType = 1 then (airSuperSaverPrice + @airFirstPriceForAnotherLeg)  else 0 end ,
			Case WHEN (ISNull(airSuperSaverTax,0) > 0 AND @airFirstTaxForAnotherLeg > 0) OR @airRequestType = 1 then (airSuperSaverTax + @airFirstTaxForAnotherLeg)  else 0 end,
			Case WHEN (ISNull(airFirstFlexPrice,0) > 0 AND @airPremiumFirstPriceForAnotherLeg > 0) OR @airRequestType = 1 then (airFirstFlexPrice + @airPremiumFirstPriceForAnotherLeg)  else 0 end ,
			Case WHEN (ISNull(airFirstFlexTax,0) > 0 AND @airPremiumFirstTaxForAnotherLeg > 0) OR @airRequestType = 1 then (airFirstFlexTax + @airPremiumFirstTaxForAnotherLeg)  else 0 end
			--hemali end
			...
			and ISNULL(resp.gdsSourceKey,2) = (Case WHEN @gdssourcekey = 0 THEN ISNULL(resp.gdsSourceKey,2) ELSE @gdssourcekey END )  
			--Added by Hemali
			and resp.airResponseKey not in (select airResponseKey from trip..Airsegments where airResponseKey = resp.airResponseKey and airsegmentCabin <> @airRequestClass) END
	    ELSE
	    BEGIN
			INSERT #AllOneWayResponses (airOneResponsekey,airOnePriceBase,airOnePriceBaseSenior,airOnePriceTaxSenior,airOnePriceBaseChildren,airOnePriceTaxChildren,airOnePriceBaseInfant,airOnePriceTaxInfant,airOnePriceBaseYouth,airOnePriceTaxYouth,airOnePriceBaseTotal,airOnePriceTaxTotal,airOnePriceBaseDisplay, airOnePriceTaxDisplay,airSegmentFlightNumber,airSegmentMarketingAirlineCode,airsubRequestkey,airOnePriceTax ,airpriceTotal,otherLegprice,cabinclass  ,otherlegtax ,legConnections,	airOneEconomyPriceBase , airOneEconomyPriceTax , airOnePremiumEconomyPriceBase ,airOnePremiumEconomyPriceTax ,airOneBusinessPriceBase ,airOneBusinessPriceTax ,
			airOnePremiumBusinessPriceBase ,airOnePremiumBusinessPriceTax ,	airOneFirstPriceBase ,	airOneFirstPriceTax ,	airOnePremiumFirstPriceBase ,	airOnePremiumFirstPriceTax )  
	...
	,airSuperSaverTax,airEconSaverTax,airFirstFlexTax,airCorporateTax,airEconFlexTax,airEconUpgradeTax, economyRefundable,premiumEconomyRefundable,businessRefundable,premiumBusinessRefundable,firstRefundable,premiumFirstRefundable,airsegmentClassEconomy,airsegmentClassPremiumEconomy,airsegmentClassBusinessCabin,airsegmentClassPremiumBusinessCabin,airsegmentClassFirstCabin,airsegmentClassPremiumFirstCabin)
	...
	economyRefundable,premiumEconomyRefundable,businessRefundable,premiumBusinessRefundable,firstRefundable,premiumFirstRefundable,
	airSegmentClassEconSaver as airsegmentClassEconomy,airSegmentClassEconFlex as airsegmentClassPremiumEconomy,airSegmentClassCorporate as airsegmentClassBusinessCabin,
	airSegmentClassEconUpgrade as airsegmentClassPremiumBusinessCabin,airSegmentClassSuperSaver as airsegmentClassFirstCabin,airSegmentClassFirstFlex as airsegmentClassPremiumFirstCabin
