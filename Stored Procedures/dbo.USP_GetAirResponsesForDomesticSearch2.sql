...
CREATE PROCEDURE [dbo].[USP_GetAirResponsesForDomesticSearch2]
(  
 ...
 -- END   
	...
	BEGIN  
		...
		--begin   
			...
			INSERT @SELECTedResponse (responsekey,fareType )   (SELECT TOP 1  airResponseKey,@SELECTedFareType  FROM  NormalizedAirResponses n WITH(NOLOCK)inner join AirSubRequest r WITH(NOLOCK) on n.airsubrequestkey = r.airSubRequestKey  WHERE airRequestKey = @airRequestKey and  airSubRequestLegIndex = 1 and flightNumber =(SELECT flightnumber FROM NormalizedAirResponses WHERE airresponsekey = @SELECTedResponseKey  and airLegNumber =1 )   AND AIRLINES = (SELECT airlines FROM NormalizedAirResponses WHERE airresponsekey = @SELECTedResponseKey and airLegNumber =1 ) )  
		--END   
		--ELSE   
		--begin    
...
--  having MIN(airSegmentDepartureDate ) > ISNULL ( DATEADD (HH,1, @selectedDate ) , DATEADD(D, -1,GETDATE() ))  
 ...
 --INNER JOIN 
 --(
 --SELECT  MIN(rowNumber) minRowIndex ,MAX(rowNumber) maxRowIndex, MIN(N.legDurationInMinutes) durationInMinutes,MAX(N.legDurationInMinutes) maximumDuration,COUNT(*) totalRecords ,noOFSTOPs ,startingFlightAirline ,startingFlightNumber,( airPriceBaseTotal + airPriceTaxTotal ) Price  FROM #normalizedResultSet N 
 ...
 AirPriceBaseTotal,AirPriceTaxTotal,airPriceBaseDisplay, airPriceTaxDisplay,airSegmentOperatingAirlineCompanyShortName,otherlegAirlines ,noOfOtherlegairlines,legConnections ,legDuration,actualNoOFStops  )  
...
--UPDATE #pagingResultSet SET isSmartFare = 1 where rowNum < @firstRoundTripResponse  and  airPrice < @firstRoundTripResponsePrice 
 ...
 CD.CountryName AS DepartureAirportCountryName, CA.CountryName AS ArrivalAirportCountryName,isSmartFare ,isLowestJourneyTime
...
FOR
 ...
 --noOFFLights INT,
 --MarketingAirlineName varchar(50),
 --isSameAirline bit default 0 
 --)

 --  INSERT INTO #test (LowestPrice,NoOFSegments,airSegmentMarketingAirlineCode,noOFFLights,MarketingAirlineName)   
 --   SELECT ( case when @isTotalPriceSort = 0 then  MIN(airPrice ) else min ( airprice + airpricetax) end )AS LowestPrice ,NoOfSTOPs AS NoOFSegments ,airlineName AS airSegmentMarketingAirlineCode,COUNT(distinct air.airResponseKey ) noOFFLights ,
 --   ISNULL (ShortName,airlineName)AS MarketingAirlineName From #airResponseResultset air  
 --   LEFT OUTER JOIN AirVendorLookup vendor  WITH (NOLOCK) ON air.airlineName = vendor .AirlineCode   
 --   GROUP BY airlineName ,ShortName ,NoOfSTOPs   
 ...
 --   BEGIN
 --   UPDATE T SET isSameAirline = 1 FROM  #test T Inner join #airResponseResultset A
 ...
 --    UPDATE T SET isSameAirline = 1 FROM  #test T Inner join #airResponseResultset A
 --   ON t.airSegmentMarketingAirlineCode = A.airlineName AND T.LowestPrice = A.airprice  
 --   WHERE A.airlineName <> A.otherlegAirlines AND isSameAirline = 0 
 ...
 --   UPDATE T SET isSameAirline = 1 FROM  #test T Inner join #airResponseResultset A
	...
	--SELECT * FROM  #test T Inner join #airResponseResultset A
	--ON t.airSegmentMarketingAirlineCode =A.airlineName AND T.LowestPrice =(airprice + airpricetax)
	--WHERE A.airlineName =A.otherlegAirlines --AND noOfOtherlegairlines = 1
	-- --AND airlineName ='UA'
    
    
 --   SELECT * FROM #airResponseResultset where airlineName = otherlegAirlines 
 --    SELECT * FROM #test 

 --   UPDATE T SET isSameAirline = 0 FROM  #test T Inner join #airResponseResultset A
 --   ON t.airSegmentMarketingAirlineCode = A.airlineName AND T.LowestPrice =(airprice + airpricetax)
 --   WHERE A.airlineName <> A.otherlegAirlines AND isSameAirline = 0 
