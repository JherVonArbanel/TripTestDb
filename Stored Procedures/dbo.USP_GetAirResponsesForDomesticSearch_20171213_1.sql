...
SET ANSI_NULLS ON
GO
create PROCEDURE [dbo].[USP_GetAirResponsesForDomesticSearch_20171213_1]
	(  
			...
			IF ( @airLines = '' or @airLines = 'Multiple Airlines' )   
				BEGIN    
				   ...
				   ELSE
					   BEGIN
							...
							SELECT TOP 1 gdsSourceKey,airPriceBAse,airPriceTax,airPriceBaseSenior,airPriceTaxSenior,airPriceBaseChildren,airPriceTaxChildren ,airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth ,airPriceTaxYouth ,AirPriceBaseTotal,AirPriceTaxTotal,airPriceBaseDisplay,airPriceTaxDisplay        ,A.airResponseKey,airPriceBaseInfantWithSeat,airPriceTaxInfantWithSeat,A.airLegBrandName,refundable
							FROM
							(
							...
							AND resp.refundable = 0 AND lower(N.airLegBrandName) = 'basic'
							UNION ALL
							...
							ISNULL(resp.gdsSourceKey,2) =( Case WHEN @gdssourcekey = 0 THEN ISNULL(resp.gdsSourceKey ,2) ELSE @gdssourcekey END )  
							and N.airResponseKey not in ( select airresponsekey  from @tempResponseToRemove ) 
							AND resp.refundable = 0 AND lower(N.airLegBrandName) = 'basic' ) A
							...
							AND ISNULL(resp.gdsSourceKey,2) =( Case WHEN @gdssourcekey = 0 THEN ISNULL(resp.gdsSourceKey ,2) ELSE @gdssourcekey END )  
							AND N.airResponseKey not in ( select airresponsekey  from @tempResponseToRemove ) 
							AND resp.refundable = 0 AND lower(N.airLegBrandName) = 'main'
							UNION ALL
							SELECT gdsSourceKey,airPriceBAse,airPriceTax,airPriceBaseSenior,airPriceTaxSenior,airPriceBaseChildren,airPriceTaxChildren ,airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth ,airPriceTaxYouth ,AirPriceBaseTotal,AirPriceTaxTotal,airPriceBaseDisplay,airPriceTaxDisplay        ,resp.airResponseKey,airPriceBaseInfantWithSeat,airPriceTaxInfantWithSeat,N.airLegBrandName,resp.refundable
							FROM #NormalizedAirResponsesMultiBrand N 
							INNER JOIN #AirResponseMultiBrand resp WITH (NOLOCK) ON N.airresponseMultiBrandkey = resp.airresponseMultiBrandKey
							...
							ISNULL(resp.gdsSourceKey,2) =( Case WHEN @gdssourcekey = 0 THEN ISNULL(resp.gdsSourceKey ,2) ELSE @gdssourcekey END )  
							and N.airResponseKey not in ( select airresponsekey  from @tempResponseToRemove ) 
							AND resp.refundable = 0 AND lower(N.airLegBrandName) = 'main' ) A
							ORDER BY airpriceBase + airPriceTax ASC
							...
							AND ISNULL(resp.gdsSourceKey,2) =( Case WHEN @gdssourcekey = 0 THEN ISNULL(resp.gdsSourceKey ,2) ELSE @gdssourcekey END )  
							AND N.airResponseKey not in ( select airresponsekey  from @tempResponseToRemove ) 
							AND resp.refundable = 1 AND lower(N.airLegBrandName) = 'main'
							UNION ALL
							SELECT gdsSourceKey,airPriceBAse,airPriceTax,airPriceBaseSenior,airPriceTaxSenior,airPriceBaseChildren,airPriceTaxChildren ,airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth ,airPriceTaxYouth ,AirPriceBaseTotal,AirPriceTaxTotal,airPriceBaseDisplay,airPriceTaxDisplay        ,resp.airResponseKey,airPriceBaseInfantWithSeat,airPriceTaxInfantWithSeat,N.airLegBrandName,resp.refundable
							FROM #NormalizedAirResponsesMultiBrand N 
							INNER JOIN #AirResponseMultiBrand resp WITH (NOLOCK) ON N.airresponseMultiBrandkey = resp.airresponseMultiBrandKey
							...
							ISNULL(resp.gdsSourceKey,2) =( Case WHEN @gdssourcekey = 0 THEN ISNULL(resp.gdsSourceKey ,2) ELSE @gdssourcekey END )  
							and N.airResponseKey not in ( select airresponsekey  from @tempResponseToRemove ) 
							AND resp.refundable = 1 AND lower(N.airLegBrandName) = 'main' ) A
							ORDER BY airpriceBase + airPriceTax ASC
							...
							AND ISNULL(resp.gdsSourceKey,2) =( Case WHEN @gdssourcekey = 0 THEN ISNULL(resp.gdsSourceKey ,2) ELSE @gdssourcekey END )  
							AND N.airResponseKey not in ( select airresponsekey  from @tempResponseToRemove ) 
							AND resp.refundable = 0 AND lower(N.airLegBrandName) = 'first'
							UNION ALL
							SELECT gdsSourceKey,airPriceBAse,airPriceTax,airPriceBaseSenior,airPriceTaxSenior,airPriceBaseChildren,airPriceTaxChildren ,airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth ,airPriceTaxYouth ,AirPriceBaseTotal,AirPriceTaxTotal,airPriceBaseDisplay,airPriceTaxDisplay        ,resp.airResponseKey,airPriceBaseInfantWithSeat,airPriceTaxInfantWithSeat,N.airLegBrandName, resp.refundable
							FROM #NormalizedAirResponsesMultiBrand N 
							INNER JOIN #AirResponseMultiBrand resp WITH (NOLOCK) ON N.airresponseMultiBrandkey = resp.airresponseMultiBrandKey
							...
							ISNULL(resp.gdsSourceKey,2) =( Case WHEN @gdssourcekey = 0 THEN ISNULL(resp.gdsSourceKey ,2) ELSE @gdssourcekey END )  
							and N.airResponseKey not in ( select airresponsekey  from @tempResponseToRemove ) 
							AND resp.refundable = 0 AND lower(N.airLegBrandName) = 'first' ) A
							ORDER BY airpriceBase + airPriceTax ASC
							...
							AND ISNULL(resp.gdsSourceKey,2) =( Case WHEN @gdssourcekey = 0 THEN ISNULL(resp.gdsSourceKey ,2) ELSE @gdssourcekey END )  
							AND N.airResponseKey not in ( select airresponsekey  from @tempResponseToRemove ) 
							AND resp.refundable = 1 AND lower(N.airLegBrandName) = 'first'
							UNION ALL
							SELECT gdsSourceKey,airPriceBAse,airPriceTax,airPriceBaseSenior,airPriceTaxSenior,airPriceBaseChildren,airPriceTaxChildren ,airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth ,airPriceTaxYouth ,AirPriceBaseTotal,AirPriceTaxTotal,airPriceBaseDisplay,airPriceTaxDisplay        ,resp.airResponseKey,airPriceBaseInfantWithSeat,airPriceTaxInfantWithSeat,N.airLegBrandName,resp.refundable
							FROM #NormalizedAirResponsesMultiBrand N 
							INNER JOIN #AirResponseMultiBrand resp WITH (NOLOCK) ON N.airresponseMultiBrandkey = resp.airresponseMultiBrandKey
							...
							ISNULL(resp.gdsSourceKey,2) =( Case WHEN @gdssourcekey = 0 THEN ISNULL(resp.gdsSourceKey ,2) ELSE @gdssourcekey END )  
							and N.airResponseKey not in ( select airresponsekey  from @tempResponseToRemove ) 
							AND resp.refundable = 1 AND lower(N.airLegBrandName) = 'first' ) A
							ORDER BY airpriceBase + airPriceTax ASC
							
							--Business NonRefundable
							INSERT @multiLegPrice (gdsSourceKey,airPriceBase,airPriceTax,airPriceBaseSenior,airPriceTaxSenior,airPriceBaseChildren,airPriceTaxChildren,airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth,airPriceTaxYouth,AirPriceBaseTotal,AirPriceTaxTotal,airPriceBaseDisplay,airPriceTaxDisplay,airresponsekey,airPriceBaseInfantWithSeat,airPriceTaxInfantWithSeat,airLegBrandName,isRefundable)  
							SELECT TOP 1 gdsSourceKey,airPriceBAse,airPriceTax,airPriceBaseSenior,airPriceTaxSenior,airPriceBaseChildren,airPriceTaxChildren ,airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth ,airPriceTaxYouth ,AirPriceBaseTotal,AirPriceTaxTotal,airPriceBaseDisplay,airPriceTaxDisplay        ,A.airResponseKey,airPriceBaseInfantWithSeat,airPriceTaxInfantWithSeat,A.airLegBrandName,refundable
							FROM
							(
							...
							AND ISNULL(resp.gdsSourceKey,2) =( Case WHEN @gdssourcekey = 0 THEN ISNULL(resp.gdsSourceKey ,2) ELSE @gdssourcekey END )  
							AND N.airResponseKey not in ( select airresponsekey  from @tempResponseToRemove ) 
							AND resp.refundable = 0 AND lower(N.airLegBrandName) = 'business'
							UNION ALL
							SELECT gdsSourceKey,airPriceBAse,airPriceTax,airPriceBaseSenior,airPriceTaxSenior,airPriceBaseChildren,airPriceTaxChildren ,airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth ,airPriceTaxYouth ,AirPriceBaseTotal,AirPriceTaxTotal,airPriceBaseDisplay,airPriceTaxDisplay        ,resp.airResponseKey,airPriceBaseInfantWithSeat,airPriceTaxInfantWithSeat,N.airLegBrandName,resp.refundable
							FROM #NormalizedAirResponsesMultiBrand N 
							INNER JOIN #AirResponseMultiBrand resp WITH (NOLOCK) ON N.airresponseMultiBrandkey = resp.airresponseMultiBrandKey
							...
							ISNULL(resp.gdsSourceKey,2) =( Case WHEN @gdssourcekey = 0 THEN ISNULL(resp.gdsSourceKey ,2) ELSE @gdssourcekey END )  
							and N.airResponseKey not in ( select airresponsekey  from @tempResponseToRemove ) 
							AND resp.refundable = 0 AND lower(N.airLegBrandName) = 'business' ) A
							ORDER BY airpriceBase + airPriceTax ASC
							...
							AND ISNULL(resp.gdsSourceKey,2) =( Case WHEN @gdssourcekey = 0 THEN ISNULL(resp.gdsSourceKey ,2) ELSE @gdssourcekey END )  
							AND N.airResponseKey not in ( select airresponsekey  from @tempResponseToRemove ) 
							AND resp.refundable = 1 AND lower(N.airLegBrandName) = 'business'
							UNION ALL
							SELECT gdsSourceKey,airPriceBAse,airPriceTax,airPriceBaseSenior,airPriceTaxSenior,airPriceBaseChildren,airPriceTaxChildren ,airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth ,airPriceTaxYouth ,AirPriceBaseTotal,AirPriceTaxTotal,airPriceBaseDisplay,airPriceTaxDisplay        ,resp.airResponseKey,airPriceBaseInfantWithSeat,airPriceTaxInfantWithSeat,N.airLegBrandName,resp.refundable
							FROM #NormalizedAirResponsesMultiBrand N 
							INNER JOIN #AirResponseMultiBrand resp WITH (NOLOCK) ON N.airresponseMultiBrandkey = resp.airresponseMultiBrandKey
							...
							ISNULL(resp.gdsSourceKey,2) =( Case WHEN @gdssourcekey = 0 THEN ISNULL(resp.gdsSourceKey ,2) ELSE @gdssourcekey END )  
							and N.airResponseKey not in ( select airresponsekey  from @tempResponseToRemove ) 
							AND resp.refundable = 1 AND lower(N.airLegBrandName) = 'business' ) A
							ORDER BY airpriceBase + airPriceTax ASC
							...
							--Business Refundable - Business Select Fare
							INSERT @multiLegPrice (gdsSourceKey,airPriceBase,airPriceTax,airPriceBaseSenior,airPriceTaxSenior,airPriceBaseChildren,airPriceTaxChildren,airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth,airPriceTaxYouth,AirPriceBaseTotal,AirPriceTaxTotal,airPriceBaseDisplay,airPriceTaxDisplay,airresponsekey,airPriceBaseInfantWithSeat,airPriceTaxInfantWithSeat,airLegBrandName,isRefundable)  
							SELECT TOP 1 gdsSourceKey,airPriceBAse,airPriceTax,airPriceBaseSenior,airPriceTaxSenior,airPriceBaseChildren,airPriceTaxChildren ,airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth ,airPriceTaxYouth ,AirPriceBaseTotal,AirPriceTaxTotal,airPriceBaseDisplay,airPriceTaxDisplay        ,A.airResponseKey,airPriceBaseInfantWithSeat,airPriceTaxInfantWithSeat,A.airLegBrandName,refundable
							FROM
							(
							...
							AND resp.gdsSourceKey = 12
							AND N.airResponseKey not in ( select airresponsekey  from @tempResponseToRemove ) 
							AND resp.refundable = 1 AND lower(N.airLegBrandName) = 'select'
							UNION ALL
							SELECT gdsSourceKey,airPriceBAse,airPriceTax,airPriceBaseSenior,airPriceTaxSenior,airPriceBaseChildren,airPriceTaxChildren ,airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth ,airPriceTaxYouth ,AirPriceBaseTotal,AirPriceTaxTotal,airPriceBaseDisplay,airPriceTaxDisplay        ,resp.airResponseKey,airPriceBaseInfantWithSeat,airPriceTaxInfantWithSeat,N.airLegBrandName,resp.refundable
							FROM #NormalizedAirResponsesMultiBrand N 
							INNER JOIN #AirResponseMultiBrand resp WITH (NOLOCK) ON N.airresponseMultiBrandkey = resp.airresponseMultiBrandKey
							...
							AND resp.gdsSourceKey = 12
							and N.airResponseKey not in ( select airresponsekey  from @tempResponseToRemove ) 
							AND resp.refundable = 1 AND lower(N.airLegBrandName) = 'select' ) A
							ORDER BY airpriceBase + airPriceTax ASC
				...
				BEGIN  
				  ...
				  BEGIN
						...
						--Business Refundable - select business
						INSERT @multiLegPrice (gdsSourceKey,airPriceBase,airPriceTax,airPriceBaseSenior,airPriceTaxSenior,airPriceBaseChildren,airPriceTaxChildren,airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth,airPriceTaxYouth,AirPriceBaseTotal,AirPriceTaxTotal,airPriceBaseDisplay,airPriceTaxDisplay,airresponsekey,airPriceBaseInfantWithSeat,airPriceTaxInfantWithSeat,airLegBrandName,isRefundable)  
						SELECT TOP 1 gdsSourceKey,airPriceBAse,airPriceTax,airPriceBaseSenior,airPriceTaxSenior,airPriceBaseChildren,airPriceTaxChildren ,airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth ,airPriceTaxYouth ,AirPriceBaseTotal,AirPriceTaxTotal,airPriceBaseDisplay,airPriceTaxDisplay        ,A.airResponseKey,airPriceBaseInfantWithSeat,airPriceTaxInfantWithSeat,A.airLegBrandName,refundable
						FROM
						(
				...
				SET @airPriceForAnotherLeg = (SELECT TOP 1 Airpricebase FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceTaxForAnotherLeg = (SELECT TOP 1 airPriceTax FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceSeniorForAnotherLeg = (SELECT TOP 1 AirpricebaseSenior FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceTaxSeniorForAnotherLeg = (SELECT TOP 1 airPriceTaxSenior FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceChildrenForAnotherLeg = (SELECT TOP 1 AirpricebaseChildren FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceTaxChildrenForAnotherLeg = (SELECT TOP 1 airPriceTaxChildren FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceInfantForAnotherLeg = (SELECT TOP 1 AirpricebaseInfant FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceTaxInfantForAnotherLeg = (SELECT TOP 1 airPriceTaxInfant FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceYouthForAnotherLeg = (SELECT TOP 1 AirpricebaseYouth FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceTaxYouthForAnotherLeg = (SELECT TOP 1 airPriceTaxYouth FROM @multiLegPrice where gdsSourceKey = 2 )
				SET @airPriceTotalForAnotherLeg = (SELECT TOP 1 AirPriceBaseTotal FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceTaxTotalForAnotherLeg = (SELECT TOP 1 AirPriceTaxTotal FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceDisplayForAnotherLeg = (SELECT TOP 1 AirpricebaseDisplay FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceTaxDisplayForAnotherLeg = (SELECT TOP 1 airPriceTaxDisplay FROM @multiLegPrice where gdsSourceKey = 2 )
				SET @airPriceInfantWithSeatForAnotherLeg = (SELECT TOP 1 airPriceBaseInfantWithSeat FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceTaxInfantWithSeatForAnotherLeg = (SELECT TOP 1 airPriceTaxInfantWithSeat FROM @multiLegPrice where gdsSourceKey = 2 )
...
--return
	...
	BEGIN   
		...
		BEGIN
			...
			BEGIN
				INSERT #AllOneWayResponses (airOneResponsekey,airOnePriceBase,airOnePriceBaseSenior,airOnePriceTaxSenior,airOnePriceBaseChildren,airOnePriceTaxChildren,airOnePriceBaseInfant,airOnePriceTaxInfant,airOnePriceBaseYouth,airOnePriceTaxYouth,airOnePriceBaseTotal,airOnePriceTaxTotal,airOnePriceBaseDisplay, airOnePriceTaxDisplay,airSegmentFlightNumber,airSegmentMarketingAirlineCode,airsubRequestkey,airOnePriceTax ,airpriceTotal,otherLegprice,cabinclass  ,otherlegtax ,legConnections ,airOnePriceBaseInfantWithSeat,airOnePriceTaxInfantWithSeat,agentwareQueryID,agentwareItineraryID,airLegBrandName,gdsSourceKey,childResponsekey,isRefundable)  
				SELECT resp.airresponsekey,(resp.airPriceBase + ISNULL(M.airPriceBase,(SELECT TOP 1 Airpricebase FROM @multiLegPrice where gdsSourceKey = 12))  ),
				(resp.airPriceBaseSenior + ISNULL(M.airPriceBaseSenior,(SELECT TOP 1 AirpricebaseSenior FROM @multiLegPrice where gdsSourceKey = 12)) ),
				(resp.airPriceTaxSenior + ISNULL(M.airPriceTaxSenior,(SELECT TOP 1 airPriceTaxSenior FROM @multiLegPrice where gdsSourceKey = 12))),
				(resp.airPriceBaseChildren + ISNULL(M.airPriceBaseChildren,(SELECT TOP 1 AirpricebaseChildren FROM @multiLegPrice where gdsSourceKey = 12)) ),
				(resp.airPriceTaxChildren + ISNULL(M.airPriceTaxChildren,(SELECT TOP 1 airPriceTaxChildren FROM @multiLegPrice where gdsSourceKey = 12))),
				(resp.airPriceBaseInfant + ISNULL(M.airPriceBaseInfant,(SELECT TOP 1 AirpricebaseInfant FROM @multiLegPrice where gdsSourceKey = 12)) ),
				(resp.airPriceTaxInfant + ISNULL(M.airPriceTaxInfant,(SELECT TOP 1 airPriceTaxInfant FROM @multiLegPrice where gdsSourceKey = 12))),
				(resp.airPriceBaseYouth + ISNULL(M.airPriceBaseYouth,(SELECT TOP 1 AirpricebaseYouth FROM @multiLegPrice where gdsSourceKey = 12)) ),
				(resp.airPriceTaxYouth + ISNULL(M.airPriceTaxYouth,(SELECT TOP 1 airPriceTaxYouth FROM @multiLegPrice where gdsSourceKey = 12))),
				(resp.AirPriceBaseTotal + ISNULL(M.AirPriceBaseTotal,(SELECT TOP 1 AirPriceBaseTotal FROM @multiLegPrice where gdsSourceKey = 12)) ),
				(resp.airPriceTaxTotal + ISNULL(M.AirPriceTaxTotal,(SELECT TOP 1 AirPriceTaxTotal FROM @multiLegPrice where gdsSourceKey = 12))),
				(resp.airPriceBaseDisplay + ISNULL(M.airPriceBaseDisplay,(SELECT TOP 1 AirpricebaseDisplay FROM @multiLegPrice where gdsSourceKey = 12)) ),
				(resp.airPriceTaxDisplay + ISNULL(M.airPriceTaxDisplay,(SELECT TOP 1 airPriceTaxDisplay FROM @multiLegPrice where gdsSourceKey = 12))),
				flightNumber,airlines,resp .airSubRequestKey ,
				(resp.airPriceTax + ISNULL(M.airPriceTax,( SELECT TOP 1 airpriceTax  FROM @multiLegPrice where gdsSourceKey = 12))),
				(resp.airPriceBase + ISNULL(M.airPriceBase,(SELECT TOP 1 Airpricebase FROM @multiLegPrice where gdsSourceKey = 12))  )+(resp.airPriceTax + ISNULL(M.airPriceTax,( SELECT TOP 1 airpriceTax  FROM @multiLegPrice where gdsSourceKey = 12))), 
				case when @isTotalPriceSort = 0 THEN  ISNULL(M.airPriceBase,(SELECT TOP 1 Airpricebase FROM @multiLegPrice where gdsSourceKey = 12)) else( isnull(M.airPriceBase,(SELECT TOP 1 Airpricebase FROM @multiLegPrice where gdsSourceKey = 12)) + isnull(M.airPriceTax,( SELECT SUM(airpriceTax ) FROM @multiLegPrice where gdsSourceKey = 12)) ) END ,
				N.cabinclass ,isnull(( SELECT TOP 1 airpriceTax  FROM @multiLegPrice where gdsSourceKey = 12),0)  ,
				n.airLegConnections,
				...
				BEGIN
					...
					(resp.airPriceBase + ISNULL(M.airPriceBase,(SELECT TOP 1 Airpricebase FROM @multiLegPrice where gdsSourceKey = 12))  )+(resp.airPriceTax + ISNULL(M.airPriceTax,(SELECT TOP 1 airpriceTax FROM @multiLegPrice where gdsSourceKey = 12))), 
					case when @isTotalPriceSort = 0 THEN  ISNULL((SELECT TOP 1 Airpricebase FROM @multiLegPrice where gdsSourceKey = 12),0) 
					else( isnull(M.airPriceBase,(SELECT TOP 1 Airpricebase FROM @multiLegPrice where gdsSourceKey = 12) ) + isnull(M.airPriceTax,( SELECT TOP 1 airpriceTax FROM @multiLegPrice where gdsSourceKey = 12)) ) END ,
					N.cabinclass ,	ISNULL(M.airPriceTax,( SELECT TOP 1 airpriceTax FROM @multiLegPrice where gdsSourceKey = 12))  ,
					n.airLegConnections,
					(resp.airPriceBaseInfantWithSeat + ISNULL((SELECT TOP 1 airPriceBaseInfantWithSeat FROM @multiLegPrice where gdsSourceKey = 12),0)  ),
					(resp.airPriceTaxInfantWithSeat + ISNULL((SELECT TOP 1 airPriceTaxInfantWithSeat FROM @multiLegPrice where gdsSourceKey = 12),0)),
					AirRes.agentwareQueryID,AirRes.agentwareItineraryID,n.airLegBrandName, n.airLegBookingClasses,resp.airResponseMultiBrandKey,1,ISNULL(resp.gdsSourceKey,2) ,resp.airresponsekey,resp.refundable
	     ...
	     BEGIN
	        ...
	        IF(@isMultiBrand = 1)
				BEGIN 
					...
					flightNumber,airlines,resp.airSubRequestKey,(resp.airPriceTax + ISNULL(M.airPriceBase,@airPriceForAnotherLeg)),(resp.airPriceBase + ISNULL(M.airPriceBase,@airPriceForAnotherLeg)+resp.airPriceTax + ISNULL(M.airPriceBase,@airPriceTaxForAnotherLeg)), case when @isTotalPriceSort = 0 THEN  ISNULL(M.airPriceBase,@airPriceForAnotherLeg) else( isnull(M.airPriceBase ,@airPriceForAnotherLeg) + isnull(M.airPriceTax,@airPriceTaxForAnotherLeg) ) END ,N.cabinclass ,isnull(M.airPriceTax,@airPriceTaxForAnotherLeg)  ,n.airLegConnections,
					(resp.airPriceBaseInfantWithSeat + ISNULL(M.airPriceBaseInfantWithSeat,@airPriceInfantWithSeatForAnotherLeg)),(resp.airPriceTaxInfantWithSeat + ISNULL(M.airPriceTaxInfantWithSeat,0)),n.airLegBrandName,n.airLegBookingClasses,ISNULL(resp.gdsSourceKey,2),resp.AirResponsekey,resp.refundable
					From #NormalizedAirResponses n WITH (NOLOCK)inner join #AirResponse resp WITH (NOLOCK) on n.airresponsekey = resp.airResponseKey 
					...
					(resp.airPriceBaseDisplay + ISNULL(M.airPriceBaseDisplay,@airPriceDisplayForAnotherLeg)),(resp.airPriceTaxDisplay + ISNULL(M.airPriceTaxDisplay,@airPriceTaxDisplayForAnotherLeg)),
					flightNumber,airlines,resp .airSubRequestKey ,(resp.airPriceTax + ISNULL(@airPriceTaxForAnotherLeg,0)),(resp.airPriceBase + ISNULL(@airPriceForAnotherLeg,0)  )+(resp.airPriceTax + ISNULL
					(@airPriceTaxForAnotherLeg,0)), case when @isTotalPriceSort = 0 THEN  ISNULL(M.airPriceTax,@airPriceForAnotherLeg) else( isnull(M.airPriceTax,@airPriceForAnotherLeg) + isnull(M.airPriceTax,@airPriceTaxForAnotherLeg)) END ,N.cabinclass ,isnull(M.airPriceTax,@airPriceTaxForAnotherLeg)  ,n.airLegConnections,
					...
					SELECT resp.airresponsekey, (resp.airPriceBase + ISNULL(M.airPriceBase,@airPriceForAnotherLeg)),
					(resp.airPriceBaseSenior + ISNULL(M.airPriceBaseSenior,@airPriceSeniorForAnotherLeg)  ),(resp.airPriceTaxSenior + ISNULL(M.airPriceTaxSenior,@airPriceTaxSeniorForAnotherLeg)),
					(resp.airPriceBaseChildren + ISNULL(M.airPriceBaseChildren,@airPriceChildrenForAnotherLeg)  ),(resp.airPriceTaxChildren + ISNULL(M.airPriceBaseChildren,@airPriceTaxChildrenForAnotherLeg)),
					(resp.airPriceBaseInfant + ISNULL(M.airPriceBaseInfant,@airPriceInfantForAnotherLeg)  ),(resp.airPriceTaxInfant + ISNULL(M.airPriceTaxInfant,@airPriceTaxInfantForAnotherLeg)),
					(resp.airPriceBaseYouth + ISNULL(M.airPriceBaseYouth,@airPriceYouthForAnotherLeg)  ),(resp.airPriceTaxYouth + ISNULL(M.airPriceTaxYouth,@airPriceTaxYouthForAnotherLeg)),
					(resp.AirPriceBaseTotal + ISNULL(M.AirPriceBaseTotal,@airPriceTotalForAnotherLeg)  ),(resp.AirPriceTaxTotal + ISNULL(M.AirPriceTaxTotal,@airPriceTaxTotalForAnotherLeg)),
					(resp.airPriceBaseDisplay + ISNULL(M.airPriceBaseDisplay,@airPriceDisplayForAnotherLeg)  ),(resp.airPriceTaxDisplay + ISNULL(M.airPriceBaseDisplay,@airPriceTaxDisplayForAnotherLeg)),
					...
					(M.airPriceTax,@airPriceTaxForAnotherLeg)), case when @isTotalPriceSort = 0 THEN  ISNULL(M.airPriceBase,@airPriceForAnotherLeg) else( isnull(M.airPriceBase,@airPriceForAnotherLeg) + isnull(M.airPriceBase,@airPriceTaxForAnotherLeg) ) END ,N.cabinclass ,isnull(M.airPriceTax,@airPriceTaxForAnotherLeg)  ,n.airLegConnections,
		  ...
		  --Insert Responses for AgentWare
            ...
            IF ( @airRequestType =1  ) --Print 'ravi'
	  		BEGIN 
				...
				flightNumber,airlines,resp .airSubRequestKey ,(resp.airPriceTax + ISNULL(M.airPriceTax,@airPriceTaxForAnotherLeg)),(resp.airPriceBase + ISNULL(M.airPriceBase,@airPriceForAnotherLeg)  )+(resp.airPriceTax + ISNULL
				(M.airPriceTax,@airPriceTaxForAnotherLeg)), case when @isTotalPriceSort = 0 THEN  ISNULL(M.airPriceBase,@airPriceForAnotherLeg) else( ISNULL(M.airPriceBase,@airPriceForAnotherLeg) + ISNULL(M.airPriceTax, @airPriceTaxForAnotherLeg) ) END ,N.cabinclass ,ISNULL(M.airPriceTax,@airPriceTaxForAnotherLeg)  ,n.airLegConnections,
				(resp.airPriceBaseInfantWithSeat + ISNULL(M.airPriceBaseInfantWithSeat,@airPriceInfantWithSeatForAnotherLeg)  ),(resp.airPriceTaxInfantWithSeat + ISNULL(M.airPriceTaxInfantWithSeat,@airPriceTaxInfantWithSeatForAnotherLeg)),agentwareQueryID,agentwareItineraryID,n.airLegBrandName,ISNULL(resp.gdsSourceKey,2),resp.airresponsekey,resp.refundable
				FROM NormalizedAirResponses n WITH (NOLOCK)inner join AirResponse resp WITH (NOLOCK) on n.airresponsekey = resp.airResponseKey 
				AND n.airlegNumber = @airRequestTypekey
				INNER JOIN @multiLegPrice M ON M.airLegBrandName = n.airLegBrandName AND M.isRefundable = resp.refundable  AND M.gdsSourceKey = 12 
				WHERE resp.airSubRequestKey = @airAgentWareWNRequest and airlegnumber = @airRequestTypeKey
				...
				BEGIN 
					...
					flightNumber,airlines,resp .airSubRequestKey ,(resp.airPriceTax + ISNULL(M.airPriceTax,@airPriceTaxForAnotherLeg)),(resp.airPriceBase + ISNULL(M.airPriceBase,@airPriceForAnotherLeg)  )+(resp.airPriceTax + ISNULL
					(M.airPriceTax,@airPriceTaxForAnotherLeg)), case when @isTotalPriceSort = 0 THEN  ISNULL(M.airPriceBase, @airPriceForAnotherLeg) else( ISNULL(M.airPriceBase, @airPriceForAnotherLeg ) + ISNULL(M.airPriceTax, @airPriceTaxForAnotherLeg) ) END ,N.cabinclass ,ISNULL(M.airPriceTax, @airPriceTaxForAnotherLeg)  ,n.airLegConnections,
					(resp.airPriceBaseInfantWithSeat + ISNULL(M.airPriceBaseInfantWithSeat,@airPriceInfantWithSeatForAnotherLeg)  ),(resp.airPriceTaxInfantWithSeat + ISNULL(M.airPriceTaxInfantWithSeat,@airPriceTaxInfantWithSeatForAnotherLeg)),AirRes.agentwareQueryID,AirRes.agentwareItineraryID,n.airLegBrandName, n.airLegBookingClasses,resp.airResponseMultiBrandKey,1,ISNULL(resp.gdsSourceKey,2),resp.airresponsekey,resp.refundable
					From NormalizedAirResponsesMultiBrand n WITH (NOLOCK)inner join AirResponseMultiBrand resp WITH (NOLOCK) on n.airresponseMultiBrandkey = resp.airResponseMultiBrandKey 
					INNER JOIN AirResponse AirRes  WITH (NOLOCK) on AirRes.airresponsekey = resp.airResponseKey
					AND n.airlegNumber = @airRequestTypekey
					INNER JOIN @multiLegPrice M ON M.airLegBrandName = n.airLegBrandName AND M.isRefundable = resp.refundable  AND M.gdsSourceKey = 12 
...
IF(@isMultiBrand = 1)
	...
	BEGIN
		...
		BEGIN 
			...
			From #AllOneWayResponses t 
			WHERE t.isMultiBrandFare = 1 AND (t.airsubRequestkey = @airBundledRequest OR t.airsubRequestkey = @airSubRequestKey OR t.airsubRequestkey = @airAgentWareWNRequest OR t.airsubRequestkey = @airMultiCabinRequest OR t.airsubRequestkey = @airMultiCabinBundledRequest)
  ...
  FROM #airResponseResultset    
	...
	SET @seconSubRequestKey =( SELECT airSubRequestKey  FROM #AirSubRequest  WITH (NOLOCK)  WHERE airSubRequestLegIndex = 2 )  
