...
CREATE PROCEDURE [dbo].[usp_GetROISummaryData_HD]   
(   
...
PRINT CONVERT( VARCHAR(24), GETDATE(), 121) + ' - 11.1  end tmp online adoption'  
 ...
 -------------------------------	
	...
	SET @selectClause = @selectClause + '(COUNT(*) * ' + CAST(@onlineSavings as nvarchar(7)) + ') FROM tman.dba.ta_transaction_rpt trn with (nolock) ' --Inner join @DKTable tmpDK on tmpDK.dk = trn.DK   ' 
	...
	--SELECT 4, 1, (COUNT(*) * 17) FROM tman.dba.ta_transaction_rpt trn Inner join @DKTable tmpDK on tmpDK.dk = trn.DK    WHERE (Booked_Online = 1)  and creation_date between '2014/03/01 00:00:00' AND '2014/05/31 23:59:59' AND TYPE = 'Air' 
	...
	SET @selectClause = @selectClause + '(COUNT(*) * ' + CAST(@HConlineSavings as nvarchar(7)) + ') FROM tman.dba.ta_transaction_rpt trn with (nolock) ' --Inner join @DKTable tmpDK on tmpDK.dk = trn.DK  ' 
	SET @whereClause = ' WHERE (Booked_Online = 1) and (CarHtlOnly = 1)'
	SET @whereClause = @whereClause + ' and dk IN (' + @dkList + ') '
	SET @whereClause = @whereClause + ' and creation_date between ''' + @fromDate + ''' AND ''' + @toDate + ''''
	...
	SET @selectClause = @selectClause + '(COUNT(*) * ' + CAST(@HConlineSavings as nvarchar(7)) + ') FROM tman.dba.ta_transaction_rpt ' --trn Inner join @DKTable tmpDK on tmpDK.dk = trn.DK ' 
	SET @whereClause = ' WHERE (Booked_Online = 1) and (CarHtlOnly = 1)'
	SET @whereClause = @whereClause + ' and dk IN (' + @dkList + ') '
	SET @whereClause = @whereClause + ' and creation_date between ''' + @fromDate + ''' AND ''' + @toDate + ''''
