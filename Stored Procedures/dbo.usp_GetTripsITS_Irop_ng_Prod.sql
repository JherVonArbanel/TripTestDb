...
CREATE PROCEDURE [dbo].[usp_GetTripsITS_Irop_ng_Prod] 
--DECLARE
...
--SELECT @SiteGUID=N'C13A7817-92AA-4CB0-BDA1-C4BAD3B11F0F',@CompanyGUID=N'80755600-7031-4BEB-ABA8-9DC8B8984191',@UserGUID=N'BDE50EB0-00A3-43B2-B36E-3FAAAE9DFE3F',@PageName=N'mytrips',@PNR=N'',@UDIDandValue=NULL,@StartDate=N'19-Dec-16',@EndDate=N'18-Jan-17'
...
--	,@UserGUID=N'C644172C-9129-4E13-90A4-7DE64BA8D74A',@PageName=N'mytrips',@PNR=N'',@UDIDandValue=NULL
	...
	--and	 data.value('(/Site/SiteGUID/node())[1]', 'NVARCHAR(500)') = @SiteGUID
...
--		RowID   INT,                          
	...
	/* Start Code to create temp trip table to insert common records over mytrips, activetrips and pasttrips */
	--select * from trip.dbo.trip
	CREATE TABLE #tmpCommonTrips(
					pkTripId BIGINT
					,TripRequestKey INT
					,tripName NVARCHAR(50)
					,userKey INT
	...
	INSERT INTO #tmpCommonTrips (pkTripId, TripRequestKey, tripName, userKey, siteKey, recordLocator, StartDate, EndDATE, tripStatusKey 
		, tripStatusName, agencyKey, userFirstName, userLastName, tripPurchasedKey, GroupKey, groupName, CreatedDate, PassengerLastName
		, PassengerFirstName)
	SELECT T.TripKey, T.TripRequestKey, T.tripName, T.userKey, T.siteKey, T.recordLocator, T.startDate, T.endDate, T.tripStatusKey
		, S.tripStatusName, T.agencyKey, U.userFirstName, U.userLastName, T.tripPurchasedKey, T.GroupKey, GRP.groupName, T.CreatedDate
		, TPI.PassengerLastName, TPI.PassengerFirstName
	FROM	Trip.dbo.trip T WITH(NOLOCK)               
			INNER JOIN Vault.dbo.[User] U WITH(NOLOCK) ON T.userKey =  U.UserKey AND T.tripStatusKey <> 10 AND T.tripStatusKey <> 17
			INNER JOIN Trip.dbo.TripStatusLookup S WITH (NOLOCK) ON T.tripStatusKey = S.tripStatusKey  
			LEFT OUTER JOIN vault.dbo.[Group] GRP ON T.GroupKey = GRP.groupKey
			INNER JOIN Trip.dbo.TripPassengerInfo TPI ON T.TripKey = TPI.TripKey AND TPI.IsPrimaryPassenger = 1
	WHERE	T.recordlocator IS NOT NULL AND T.recordlocator <> '' AND T.siteKey = @siteKey 
			
	/* ENDS Code to create temp trip table to insert common records over mytrips, activetrips and pasttrips */		
	
	IF @PageName = 'mytrips'                            
	BEGIN 
		INSERT INTO	#tmpTrip (tripKey, TripRequestKey, tripName, userKey, recordLocator, startDate, endDate, tripStatusKey                           
			, tripStatusName, agencyKey, userFirstName, userLastName, userLogin, tripPurchasedKey, groupKey, groupName, CreatedDate 
			, TravelerName, TotalCost 
		)
		SELECT CT.pkTripId, CT.TripRequestKey, CT.tripName, CT.userKey, CT.recordLocator, CT.startDate, CT.endDate, CT.tripStatusKey
			, CASE	WHEN CT.StartDate < @SixHrsBeforeTime --@GMT_Today_4_AM 
										AND CT.tripStatusKey IN (2,3) Then 'Past'
					WHEN CT.StartDate >= @SixHrsBeforeTime --@GMT_Today_4_AM 
										AND CT.tripStatusKey IN (2,3) THEN 'Purchased'
				ELSE CT.tripStatusName END
			, CT.agencyKey, CT.userFirstName, Ct.userLastName, LEFT(ISNULL(CT.userFirstName, ''), 1) + ' ' + CT.userLastName AS userLogin 
			, CT.tripPurchasedKey, CT.GroupKey, CT.groupName, CT.CreatedDate 
			, LTRIM(RTRIM(ISNULL(CT.PassengerLastName, '') + '/' + LEFT(ISNULL(CT.PassengerFirstName, ''), 1))), 0
		FROM #tmpCommonTrips CT 
		WHERE CT.recordLocator = CASE WHEN @PNR IS NOT NULL THEN @PNR ELSE CT.RecordLocator END 
			AND CT.siteKey = @siteKey AND CT.userKey = @userkey 
	END
	...
	BEGIN
		IF @RoleTripId <> 1
		BEGIN
			INSERT INTO	#tmpTrip (tripKey, TripRequestKey, tripName, userKey, recordLocator, startDate, endDate                             
				, tripStatusKey, tripStatusName, agencyKey, userFirstName, userLastName, userLogin, tripPurchasedKey 
				, groupKey, groupName, CreatedDate, TravelerName, TotalCost)
			SELECT CT.pkTripId, CT.TripRequestKey, CT.tripName, CT.userKey, CT.recordLocator, CT.startDate, CT.endDate
				, CT.tripStatusKey, CASE WHEN CT.tripStatusKey IN ( 2,3) THEN 'Purchased' else CT.tripStatusName END
				, CT.agencyKey, CT.userFirstName, CT.userLastName
				, LEFT(ISNULL(CT.userFirstName, ''), 1) + ' ' + CT.userLastName AS userLogin 
				, CT.tripPurchasedKey, CT.GroupKey, CT.groupName, CT.CreatedDate 
				, LTRIM(RTRIM(ISNULL(CT.PassengerLastName, '') + '/' + LEFT(ISNULL(CT.PassengerFirstName, ''), 1))), 0 
			FROM #tmpCommonTrips CT 
			WHERE CT.StartDate >= @SixHrsBeforeTime
				AND CT.recordLocator = CASE WHEN (@PNR <> '' AND @PNR IS NOT NULL) THEN @PNR ELSE CT.RecordLocator END
				AND CT.siteKey = @siteKey 					
		END
	END	
	...
	BEGIN
		...
		BEGIN
			INSERT INTO	#tmpTrip (tripKey, TripRequestKey, tripName, userKey, recordLocator, startDate, endDate                             
				...
				, groupKey, groupName, CreatedDate, TravelerName, TotalCost)
			SELECT CT.pkTripId, CT.TripRequestKey, CT.tripName, CT.userKey, CT.recordLocator, CT.startDate, CT.endDate
				, CT.tripStatusKey, CASE WHEN CT.tripStatusKey IN ( 2,3) THEN 'Past' else CT.tripStatusName END
				, CT.agencyKey, CT.userFirstName, CT.userLastName
				, LEFT(ISNULL(CT.userFirstName, ''), 1) + ' ' + CT.userLastName AS userLogin 
				, CT.tripPurchasedKey, CT.GroupKey, CT.groupName, CT.CreatedDate 
				, LTRIM(RTRIM(ISNULL(CT.PassengerLastName, '') + '/' + LEFT(ISNULL(CT.PassengerFirstName, ''), 1))), 0 
	...
	SELECT trip.tripKey, trip.tripName, trip.userKey, TripCarResponse.recordLocator, trip.endDate, trip.startDate, trip.tripStatusKey
		...
		, dbo.TripCarResponse.dropOutDate, CarContent.dbo.SippCodes.SippCodeClass, dbo.TripCarResponse.carResponseKey, Trip.siteKey
		, trip.createdDate, trip.tripRequestKey, CarContent.dbo.SabreVehicles.VehicleName, TripCarResponse.NoOfDays
	...
	FROM CarContent.dbo.CarCompanies WITH (NOLOCK) 
		INNER JOIN Trip.dbo.TripCarResponse WITH (NOLOCK) ON CarContent.dbo.CarCompanies.CarCompanyCode = Trip.dbo.TripCarResponse.carVendorKey 
	...
	SELECT trip.tripKey, trip.tripName, trip.userKey, TR.recordLocator, trip.endDate, trip.startDate, trip.tripStatusKey
		, TR.actualCarPrice, TR.actualCarTax, TR.carVendorKey, CarContent.dbo.CarCompanies.CarCompanyName, TR.carLocationCode
		, TR.PickUpdate, TR.dropOutDate, S.VehicleClass AS SippCodeClass, TR.carResponseKey, Trip.siteKey, trip.createdDate
		, trip.tripRequestKey, AV.ALVehicleName, TR.NoOfDays, '' As CityName, '' AS StateCode, '' AS CountryCode, '' As HotelRating
		, 0 AS DiscountFare, TR.RPH AS RPH
	FROM Trip.dbo.TripCarResponse TR WITH (NOLOCK) 
		...
		LEFT OUTER JOIN CarContent.dbo.ALAMOVEHICLES AV WITH (NOLOCK) ON AL.ALLocationCode = AV.ALLOCATIONCODE 
			AND AV.ALVEHICLECODE = TR.carCategoryCode 
		...
		INNER JOIN Trip.dbo.Trip WITH (NOLOCK) ON (TR.tripKey = Trip.tripKey OR (TR.tripGUIDKey = Trip.tripPurchasedKey 
			and (tr.tripKey is null or tr.tripKey =0 )))
	...
	SELECT trip.tripKey, trip.tripName, trip.userKey, TR.recordLocator, trip.endDate, trip.startDate, trip.tripStatusKey
		, TR.actualCarPrice, TR.actualCarTax, TR.carVendorKey, CarContent.dbo.CarCompanies.CarCompanyName, TR.carLocationCode
		, TR.PickUpdate, TR.dropOutDate, S.VehicleClass AS SippCodeClass, TR.carResponseKey, Trip.siteKey, trip.createdDate
		, trip.tripRequestKey, NV.ZLVehicleName, TR.NoOfDays, '' As CityName, '' AS StateCode, '' AS CountryCode, '' As HotelRating
		, 0 AS DiscountFare, TR.RPH AS RPH
	FROM TripCarResponse TR WITH (NOLOCK) 
		...
		LEFT OUTER JOIN CarContent.dbo.NationalVehicles NV WITH (NOLOCK) ON NL.ZLLocationCode = NV.ZLLOCATIONCODE 
			AND NV.ZLVEHICLECODE = TR.carCategoryCode 
		...
		INNER JOIN Trip WITH (NOLOCK) ON (TR.tripKey = Trip.tripKey OR (TR.tripGUIDKey = Trip.tripPurchasedKey 
			and (tr.tripKey is null or tr.tripKey =0 ))) 
		INNER JOIN #tmpTrip tmp ON tmp.tripKey = Trip.tripKey 
	...
	SELECT trip.tripKey, trip.tripName, trip.userKey, TR.recordLocator, trip.endDate, trip.startDate, trip.tripStatusKey
		, TR.actualCarPrice, TR.actualCarTax, TR.carVendorKey, CarContent.dbo.CarCompanies.CarCompanyName, TR.carLocationCode
		, TR.PickUpdate, TR.dropOutDate, S.VehicleClass AS SippCodeClass, TR.carResponseKey, Trip.siteKey, trip.createdDate
		, trip.tripRequestKey, ZV.ZRVehicleName, TR.NoOfDays, '' As CityName, '' AS StateCode, '' AS CountryCode, '' As HotelRating
		, 0 AS DiscountFare, TR.RPH AS RPH
	FROM TripCarResponse TR WITH (NOLOCK) 
		INNER JOIN CarContent.dbo.CarCompanies WITH (NOLOCK) ON CarContent.dbo.CarCompanies.CarCompanyCode = TR.carVendorKey AND TR.isDeleted = 0
		...
		LEFT OUTER JOIN CarContent.dbo.DollarVehicles ZV WITH (NOLOCK) ON ZL.ZRLocationCode = ZV.ZRLOCATIONCODE 
			AND ZV.ZRVEHICLECODE = TR.carCategoryCode 
	...
	SELECT trip.tripKey, trip.tripName, trip.userKey, TR.recordLocator, trip.endDate, trip.startDate, trip.tripStatusKey
		, TR.actualCarPrice, TR.actualCarTax, TR.carVendorKey, CarContent.dbo.CarCompanies.CarCompanyName, TR.carLocationCode
		, TR.PickUpdate, TR.dropOutDate, S.VehicleClass AS SippCodeClass, TR.carResponseKey, Trip.siteKey, trip.createdDate
		, trip.tripRequestKey, TV.ZTVEHICLENAME, TR.NoOfDays, '' As CityName, '' AS StateCode, '' AS CountryCode, '' As HotelRating
		, 0 AS DiscountFare, TR.RPH AS RPH
	FROM TripCarResponse TR WITH (NOLOCK) 
		INNER JOIN CarContent.dbo.CarCompanies WITH (NOLOCK) ON CarContent.dbo.CarCompanies.CarCompanyCode = TR.carVendorKey AND TR.isDeleted = 0
		...
		LEFT OUTER JOIN CarContent.dbo.ThriftyVehicles TV WITH (NOLOCK) ON TL.ZTLocationCode = TV.ZTLOCATIONCODE 
			AND TV.ZTVEHICLECODE = TR.carCategoryCode 
	...
	SELECT tripKey, tripName, userKey, recordLocator, endDate, startDate, tripStatusKey, actualHotelPrice, actualHotelTax, ChainCode
		, HotelName, CityName, StateCode, checkInDate, checkOutDate, RatingType, hotelResponseKey, siteKey, CreatedDate, tripRequestKey 
		, VehicleCompanyName, NoofDays, CountryCode, Rating, DiscountFare, RPH
	FROM 
	(
		SELECT	ROW_NUMBER() OVER(PARTITION BY SH.SupplierHotelID ORDER BY SH.AddDate DESC) RN
				, dbo.Trip.tripKey, dbo.Trip.tripName, dbo.Trip.userKey, HR.recordLocator, dbo.Trip.endDate, dbo.Trip.startDate
				, dbo.Trip.tripStatusKey, HR.actualHotelPrice, HR.actualHotelTax, HT.ChainCode, HT.HotelName, HT.CityName, HT.StateCode
				, HR.checkInDate, HR.checkOutDate, HT.RatingType, HR.hotelResponseKey, dbo.Trip.siteKey, dbo.Trip.CreatedDate
				, dbo.Trip.tripRequestKey, '' As VehicleCompanyName, 0 as NoofDays, HT.CountryCode, ISNULL(HT.Rating, 0) AS Rating
				, 0 as DiscountFare, HR.RPH as RPH   
