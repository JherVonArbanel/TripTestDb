...
GO
   ...
   CREATE PROCEDURE [dbo].[USP_GetAirResponsesForDomesticSearch_Alternate]
	(  
  ...
  inner join AirSubRequest r WITH(NOLOCK) on n.airsubrequestkey = r.airSubRequestKey  WHERE airRequestKey = @airRequestKey AND airlegnumber =1
   and  airSubRequestLegIndex = 1 and flightNumber =(SELECT flightnumber FROM NormalizedAirResponses WHERE airresponsekey = @SELECTedResponseKey  and airLegNumber =1 
	...
	SET @gdssourcekey = 2--( SELECT distinct gdssourcekey FROM AirResponse  WITH (NOLOCK)WHERE airResponseKey = @SELECTedResponseKey )           
	...
	--END   
		...
		AND resp.airSubRequestKey in (select airsubrequestkey from AirSubRequest where airRequestKey = @airRequestKey)
		...
		--INNER JOIN @valid V on r.airResponseKey = v.airResponsekey     
...
select N.airresponsekey, N.airsubrequestkey, N.flightnumber,N.airlines,N.airlegnumber,N.airlegbookingclasses,N.operatingairlines,N.airlegconnections,(A.airPriceBase+a.airPriceTax) AS Price from NormalizedAirResponses N 
inner join  airresponse A ON A.airResponseKey = N.airresponsekey 
where N.airresponsekey in (select airresponsekey FROM temptable)
AND N.airLegNumber = CASE WHEN @airRequestTypeKey > -1 THEN @airRequestTypeKey ELSE airLegNumber END   
order by N.airsubrequestkey desc, Price ASC
return
