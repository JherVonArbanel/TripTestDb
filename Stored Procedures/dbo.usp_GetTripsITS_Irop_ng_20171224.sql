...
CREATE PROCEDURE [dbo].[usp_GetTripsITS_Irop_ng_20171224] 
--DECLARE
...
PRINT 'Active Trips Else'	
	...
	INNER JOIN dbo.TripHotelResponse AS HR WITH (NOLOCK) ON SH.SupplierHotelId = HR.supplierHotelKey ON HT.HotelId = SH.HotelId 
		AND SH.SupplierFamily = HR.supplierId AND HR.isDeleted = 0 AND SH.supplierHotelId = ISNULL(HR.HotelId, SH.supplierHotelId)
