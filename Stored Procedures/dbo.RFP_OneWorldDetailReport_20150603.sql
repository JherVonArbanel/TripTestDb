...
CREATE procedure [dbo].[RFP_OneWorldDetailReport_20150603]    
(    
 @userKey int,    
 @currency varchar(5),    
 @meetingCode int,    
...
Begin      
 ...
 Case when Trip.isOnlineBooking = 1 then 'Online' else 'Offline' END as Source, (Case when trip.tripStatusKey = 12 then  isnull(TTI.TotalFare,0) * ExRate.ExchangeRate else isnull(TAR.actualAirPrice,0) * ExRate.ExchangeRate end ) as tripTotalBaseCost      
 from trip       
