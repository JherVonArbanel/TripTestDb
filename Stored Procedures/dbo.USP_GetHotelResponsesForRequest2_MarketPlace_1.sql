...
/***********************************               
 ...
 --exec USP_GetHotelResponsesForRequest2_MarketPlace @hotelRequestKey =84173,@hotelRatings=N'0,1,2,3,4,5',@pageNo=1,@pageSize=10,@cityCode=N'DFW',@hotelGroupId=134,@isMatrixRequired=0,@isNearByRegionRequired=1,@isLimitedChainList=1  
          
 **********************************/              
CREATE PROCEDURE [dbo].[USP_GetHotelResponsesForRequest2_MarketPlace_1]    
(     
--declare     
 @hotelRequestKey  INT,    
...
BEGIN  
  ...
  ,ZipCode,PhoneNumber,FaxNumber, CityCode,CountryName,distance,checkInDate,checkOutDate,HotelDescription,ChainName,minRateTax,ImageURL,       
        ...
        ,promoId, eanBarRate, touricoCalculatedBarRate, touricoNetRate, touricoCostBasisRate, marketPlaceVariableId, isNonRefundable   
 )  
   (    
  ...
  LEFT OUTER JOIN [CMS].[dbo].[CustomHotelImages] AS CHI WITH(NOLOCK) ON CHI.HotelId = FHR.HotelId AND CHI.OrderId = 1      
...
WHERE SupplierHotelKey = (SELECT B.SupplierHotelKey  FROM @hotelResponseResult B WHERE B.SupplierHotelKey = SupplierHotelKey AND B.SupplierId = 'Sabre' AND B.CorporateCode IS NOT NULL AND LTRIM(RTRIM(B.CorporateCode)) <> '')
AND (CorporateCode IS NULL  OR LTRIM(RTRIM(corporateCode)) = '')
