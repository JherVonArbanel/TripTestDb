...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[USP_GetAirResponsesForDomesticSearch_ForMultiCity]
	(  
...
--return
	...
	BEGIN
		...
		BEGIN
			...
			WHERE airLegNumber = @airRequestTypeKey AND (ARMB.airSubRequestKey = @airBundledRequest OR ARMB.airSubRequestKey = @airSubRequestKey OR ARMB.airSubRequestKey = @airPublishedFareRequest OR ARMB.airSubRequestKey = @airMultiCabinRequest OR ARMB.airSubRequestKey = @airAgentWareWNRequest)
	...
	BEGIN
		...
		WHERE airresponsekey IN (SELECT A.airResponseKey from #airResponseResultset A WHERE A.totalCost > @MaxFareTotal)
