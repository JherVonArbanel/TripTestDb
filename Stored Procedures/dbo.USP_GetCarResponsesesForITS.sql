...
CREATE PROCEDURE [dbo].[USP_GetCarResponsesesForITS]      
( @CarRequestKey int ,      
...
/*SELECT carVendorKey as carCompany ,CarCompanyname, MIN (minRate)as LowestPrice, carCategoryCode as carClassCat FROM vw_sabreCarResponse      
 ...
 SELECT carResponseDetailKey,carCompany,CarCompanyname,LowestPrice,carClassCat,carClassDes,NoOfDays,ImageName,VehicleName,carLocationCode,pickupCity,pickupLocationAddress,  
  dropoffLocationAddress,pickupDate,TotalChargeAmt,carResponseKey,passenger,baggage,minRateTax,SippCodeTransmission,SippCodeAC,newVehicleName,newImageName,  
  pickupLatitude,pickupLongitude,contractCode,dropoffCity,PickupLocInfoCode,DropoffState,DropoffCountry,PickupState,PickupCountry  
 FROM  
 (  
  SELECT ROW_NUMBER() OVER  
   (  
    PARTITION BY carresponsedetail.carVendorKey, vw_sabreCarResponse.CarCompanyname, MIN (carresponsedetail.minRate),CarContent.dbo.SabreVehicles.VehicleName   
    ORDER BY CarContent.dbo.SabreVehicles.VehicleName DESC  
   ) AS Row,   
   carresponsedetail.carResponseDetailKey,carresponsedetail.carVendorKey as carCompany, vw_sabreCarResponse.CarCompanyname,   
   MIN (carresponsedetail.minRate)as LowestPrice, carresponsedetail.carCategoryCode as carClassCat,   
   CarContent.dbo.SippCodes.SippCodeDescription as carClassDes, carresponsedetail.NoOfDays,   
   ISNull(CarContent.dbo.SabreVehicles.ImageName,'no-image.jpg') as ImageName, CarContent.dbo.SabreVehicles.VehicleName,   
   CarResponse.carLocationCode,vw_sabreCarResponse.pickupCity, vw_sabreCarResponse.pickupLocationAddress, vw_sabreCarResponse.dropoffLocationAddress,   
   vw_sabreCarResponse.pickupDate, vw_sabreCarResponse.TotalChargeAmt, vw_sabreCarResponse.carResponseKey,   
   CarContent.dbo.SabreVehicles.PsgrCapacity as passenger, CarContent.dbo.SabreVehicles.baggage as baggage,   
   MIN (carresponsedetail.minRateTax )as minRateTax, CarContent.dbo.SippCodes.SippCodeTransmission, CarContent.dbo.SippCodes.SippCodeAC,   
   CarContent.dbo.SabreVehicles.VehicleName as newVehicleName, 'no-image.jpg' as newImageName, vw_sabreCarResponse.pickupLatitude,   
   vw_sabreCarResponse.pickupLongitude, carresponsedetail.contractCode, vw_sabreCarResponse.dropoffCity, carresponse.PickupLocInfoCode,   
   vw_sabreCarResponse.dropoffState as DropoffState, vw_sabreCarResponse.dropoffCountry as DropoffCountry, vw_sabreCarResponse.pickupState as PickupState,   
   vw_sabreCarResponse.pickupCountry as PickupCountry  
  FROM vw_sabreCarResponse   
   inner join carresponsedetail on carresponsedetail.carresponsekey=vw_sabreCarResponse.carResponseKey      
   inner join CarResponse on CarResponse.carResponseKey=CarResponseDetail.carResponseKey      
  ...
  group by carresponsedetail.carVendorKey, vw_sabreCarResponse.CarCompanyname,carresponsedetail.carCategoryCode,   
   ...
   carresponsedetail.carResponseDetailKey,CarResponseDetail.minRate,carresponsedetail.NoOfDays,CarResponse.carLocationCode,  
   vw_sabreCarResponse.pickupCity,vw_sabreCarResponse.pickupLocationAddress, vw_sabreCarResponse.dropoffLocationAddress,vw_sabreCarResponse.pickupDate,  
   vw_sabreCarResponse.TotalChargeAmt,vw_sabreCarResponse.carResponseKey,CarContent.dbo.SabreVehicles.PsgrCapacity, CarContent.dbo.SabreVehicles.baggage,     
   CarContent.dbo.SippCodes.SippCodeTransmission, CarContent.dbo.SippCodes.SippCodeAC,  
