...
-- =============================================
CREATE PROCEDURE [dbo].[usp_GetBillingDetail] 
( 
...
BEGIN
	...
	FROM       dbo.Trip  INNER JOIN
                      dbo.TripAirResponse ON dbo.Trip.tripKey = dbo.TripAirResponse.tripKey 
                      Or (dbo.Trip.tripPurchasedKey = TripAirResponse.tripGUIDKey and dbo.Trip.tripPurchasedKey is not null)
                      INNER JOIN dbo.TripStatusLookup ON dbo.Trip.tripStatusKey = dbo.TripStatusLookup.tripStatusKey 
                      INNER JOIN dbo.TripPassengerInfo ON dbo.Trip.tripKey = dbo.TripPassengerInfo.tripKey  
                      left outer JOIN dbo.TripPassengerUDIDInfo ON ((dbo.Trip.tripKey = dbo.TripPassengerUDIDInfo.tripKey) AND CompanyUDIDNumber = 20) 
    WHERE (dbo.Trip.CreatedDate BETWEEN @fromDate AND @toDate) 
           AND siteKey = @siteKey
           AND Trip.tripStatusKey <> 17 
           AND (Trip.recordLocator Is Not Null or Trip.recordLocator <> '')
