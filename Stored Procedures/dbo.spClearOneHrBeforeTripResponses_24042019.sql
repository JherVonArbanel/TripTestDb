...
SET ANSI_NULLS ON
GO
CREATE   proc  [dbo].[spClearOneHrBeforeTripResponses_24042019]
@siteKey	INT
AS
BEGIN
...
PRINT 'INSERT TRIPREQUEST'
	...
	SELECT tripRequestKey FROM TripRequest WITH (NOLOCK) WHERE tripRequestCreated < DATEADD(MINUTE, -30, GETDATE())
	
	INSERT INTO @tblAirRequest 
	SELECT airRequestKey FROM airRequest WITH (NOLOCK) WHERE airRequestCreated < DATEADD(MINUTE, -5, @Latest_Date)
...
PRINT 'DELETE AIRRESPONSE'
	...
	PRINT 'DELETE AIRREDEEMCALENDARRESPONSE'
	...
	DELETE A FROM AirRedeemCalendarResponse A
		INNER JOIN @tblAirSubRequest ASR ON A.airsubrequestkey = ASR.airSubRequestKey
	SET @Row_Count=@@ROWCOUNT
	SET @Curr_DateTime=CONVERT(DATETIME,GETDATE(),103)
	exec Log..USP_InsertLogs 
