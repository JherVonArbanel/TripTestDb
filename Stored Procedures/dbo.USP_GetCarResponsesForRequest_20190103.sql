...
Create PROCEDURE [dbo].[USP_GetCarResponsesForRequest_20190103]      
( @CarRequestKey int ,      
...
Declare @sqlString varchar(2000)      
 ...
 SELECT *,CONVERT(VARCHAR(20), 'NONE') AS ReasonCode INTO #tmpCarResponseDetail  FROM CarResponseDetail WITH (NOLOCK) WHERE carResponseKey IN (SELECT CarResponsekey from CarResponse WITH (NOLOCK) WHERE carRequestKey=@CarRequestKey)  
  ...
  BEGIN  
   ...
   INSERT INTO @tblCarPolicy( MaxFareTotalCar, IsMaxFareTotalCar,  
          ...
          IsApproveHighFareTolCar,IsNotifyHighFareTolCar)   
   SELECT      CarSpendingCap,IsCarSpendingCap,   
          DomesticHighFareTol,IsDomesticHighFareTol,    
          IsApproveCarSpendingCap,NotifyDomesticHighFareCap  
   FROM      vault.dbo.[udf_GetPolicyDetailsForCar](@UserKey, @CompanyKey, @TripType,@UserGroupKey)  
  
     
   SELECT TOP 1 @MaxFareTotal = MaxFareTotalCar, @IsHideFare = IsMaxFareTotalCar,  @HighFareTotal = HighFareTolCar,@IsHighFareTotal = IsHighFareTolCar FROM @tblCarPolicy  
...
(  
 ...
 carresponsedetail.minRateTax as minRateTax,CarContent.dbo.SippCodes.SippCodeTransmission, CarContent.dbo.SippCodes.SippCodeDescription,             
  ...
  vw_sabreCarResponse.DropDistanceUnit,vw_sabreCarResponse.PickupAddress,vw_sabreCarResponse.DropAddress,vw_sabreCarResponse.RequestType, vw_sabreCarResponse.PickUpLatLong,  
     vw_sabreCarResponse.DropOffLatLong, carresponsedetail.ReasonCode,ROW_NUMBER() OVER (PARTITION BY carresponsedetail.carVendorKey,carresponsedetail.carCategoryCode ORDER BY carresponsedetail.carVendorKey) AS rn  
