...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[USP_GetAirResponsesMultiCityInternational_Backup_07112017]
(
--DECLARE	
	...
	@MaxNoofstops INT = 1,
	@MaxInternationalFareTotal FLOAT = 0,
...
-- SELECT @airSubRequestKey=921611,@airRequestTypeKey=2,@SuperSetairLines=N'AA,AB,AY,BA,CX,IB,LA,MH,QR,RJ',@allowedOperatingAirlines=N'AA,MH,BA,AB,CX,AY,IB,JL,LA,QF,RJ,S7,QR',@airLines=N'',@price=2147483647,@pageNo=0,@pageSize=30,@NoOfStops=N'-1',@selectedResponseKey='DAA4D819-67DC-4E4D-B4D5-351D42D61061',@drillDownLevel=N'1',@gdsSourcekey=2,@minTakeOffDate='2013-10-23 00:00:00',@maxTakeOffDate='2016-01-23 00:00:00',@minLandingDate='2013-10-23 00:00:00',@maxLandingDate='2016-01-23 00:00:00',@selectedFareType=N'',@isIgnoreAirlineFilter=N'False',@isTotalPriceSort=N'True',@siteKey=7,@matrixView=0
	...
	DECLARE @LastRec int
	DECLARE @isExcludeAirlinesPresent BIT = 0 , @isExcludeCountryPresent BIT = 0, @isLoggedinAirlinesPresent BIT = 0
...
--INSERT INTO #AirSegments ( airSegmentKey,airResponseKey,airLegNumber,airSegmentMarketingAirlineCode,airSegmentOperatingAirlineCode,airSegmentFlightNumber,airSegmentDuration,airSegmentEquipment,airSegmentMiles,airSegmentDepartureDate,airSegmentArrivalDate,airSegmentDepartureAirport,airSegmentArrivalAirport,airSegmentResBookDesigCode,airSegmentDepartureOffset,airSegmentArrivalOffset ,airSegmentSeatRemaining,airSegmentMarriageGrp ,airFareBasisCode ,airFareReferenceKey ,airSegmentOperatingFlightNumber,airsegmentCabin ,segmentOrder,airSegmentOperatingAirlineCompanyShortName,gdsSourceKey  )
	INSERT INTO #AirSegments ( airSegmentKey,airResponseKey,airLegNumber,airSegmentMarketingAirlineCode,airSegmentOperatingAirlineCode,airSegmentFlightNumber,airSegmentDuration,airSegmentEquipment,airSegmentMiles,airSegmentDepartureDate,airSegmentArrivalDate,airSegmentDepartureAirport,airSegmentArrivalAirport,airSegmentResBookDesigCode,airSegmentDepartureOffset,airSegmentArrivalOffset ,airSegmentSeatRemaining,airSegmentMarriageGrp ,airFareBasisCode ,airFareReferenceKey ,airSegmentOperatingFlightNumber,airsegmentCabin ,segmentOrder,airSegmentOperatingAirlineCompanyShortName,gdsSourceKey,airsegmentPricingKey,airSegmentFareCategory,airSegmentBrandName,airSegmentBrandID,airSegmentBaggage,airSegmentMealCode )
	(
		...
		AND   airRequestKey = @airRequestKey  AND ((ISNULL(resp.gdsSourceKey,2) =( CASE WHEN @gdssourcekey = 0 THEN ISNULL(resp.gdsSourceKey ,2) ELSE @gdssourcekey END )) OR resp.gdsSourceKey = 12) 
	)
--DELETE FROM #AirSegments WHERE airResponseKey in ( SELECT distinct airResponseKey FROM #AirSegments where airSegmentMarketingAirlineCode ='WN')
	DELETE FROM #AirSegments WHERE airResponseKey in ( SELECT distinct airResponseKey FROM #AirSegments where airSegmentMarketingAirlineCode ='WN' and gdsSourceKey <> 12)
	...
	DECLARE @tblMarketingAirlinesGroup AS TABLE ( airlineCode varchar(20), airlineGroupName varchar(100))	  
	...
	BEGIN 
		...
		--select '1',* from @tempResponseToRemove
		INSERT @tempResponseToRemove (airresponsekey )   
		(SELECT distinct S.airresponsekey FROM #AirSegments S WITH(NOLOCK) 
		INNER JOIN AirResponse AR WITH(NOLOCK) on S.airResponseKey = Ar.airResponseKey 
		INNER JOIN AirSubRequest SUB WITH(NOLOCK) ON Ar.airSubRequestKey = SUB.airSubRequestKey 
		  WHERE airRequestKey = @airRequestKey and airSegmentMarketingAirlineCode   in (SELECT * FROM @tblExcludedAirlines) and SUB.groupkey <> 4) 
		...
		airSegmentOperatingAirlineCode in (SELECT * FROM @tblExcludedAirlines) and subReq.groupKey<>4)
		...
		END
	END
	--select '3',* from @tempResponseToRemove
	--select @IsLoginedAirlineList
	-- Add responsekey to @tempResponseToRemove which contains Login excludes Airlines
	...
	BEGIN 
		...
		INSERT @tempResponseToRemove (airresponsekey )   
		(SELECT distinct airresponsekey FROM #AirSegments WHERE airSegmentMarketingAirlineCode   in (SELECT * FROM @tblLoggedinAirlines))  
		...
		(SELECT Distinct s.airResponseKey from AirSegments s WITH (NOLOCK) inner join AirResponse resp WITH (NOLOCK) on s.airResponseKey =resp.airResponseKey   
		inner join AirSubRequest subReq WITH (NOLOCK) on resp.airSubRequestKey =subReq.airSubRequestKey  where airRequestKey = @airRequestKey and   
		airSegmentOperatingAirlineCode in (SELECT * FROM @tblLoggedinAirlines))
		IF ( @isLoggedinAirlinesPresent = 0 ) 
		BEGIN
			IF((SELECT COUNT(Distinct s.airResponseKey)from AirSegments s WITH (NOLOCK) inner join AirResponse resp WITH (NOLOCK) on s.airResponseKey =resp.airResponseKey   
		inner join AirSubRequest subReq WITH (NOLOCK) on resp.airSubRequestKey =subReq.airSubRequestKey  where airRequestKey = @airRequestKey and   
		airSegmentOperatingAirlineCode in (SELECT * FROM @tblLoggedinAirlines))> 0)
			BEGIN
				SET @isLoggedinAirlinesPresent =  1 
			END	
		END
...
/****logic for calculating price for higher legs *****/
	...
	ELSE     
	BEGIN 
		INSERT into @tmpAirline(airlineCode)  
		SELECT distinct seg1.airSegmentMarketingAirlineCode FROM AirSegments seg1  WITH (NOLOCK) INNER JOIN AirResponse resp  WITH (NOLOCK) ON seg1.airResponseKey = resp.airResponseKey 
		INNER JOIN AirSubRequest subreq WITH (NOLOCK) ON resp.airSubRequestKey = subreq .airSubRequestKey 
		WHERE ( airRequestKey = @airRequestKey )
		INSERT into @tmpAirline (airLineCode ) values  ('Multiple Airlines')
	    --INSERT into @tmpAirline (airLineCode ) values  ('LATAM Airlines')
	END   
...
---- print (cast(getdate() AS time))
	...
	BEGIN 
		...
		IF ( @gdssourcekey =0  or @gdssourcekey <> 9 ) 
			...
			airOnePriceBaseTotal,airOnePriceTaxTotal,airOnePriceBaseDisplay, airOnePriceTaxDisplay,legConnections,airOnePriceBaseInfantWithSeat,airOnePriceTaxInfantWithSeat,agentwareQueryID,agentwareItineraryID)
			...
			airPriceBaseSenior,airPriceTaxSenior,airPriceBaseChildren,airPriceTaxChildren,airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth,airPriceTaxYouth,AirPriceBaseTotal,AirPriceTaxTotal,airPriceBaseDisplay, airPriceTaxDisplay,airLegConnections,resp.airPriceBaseInfantWithSeat,resp.airPriceTaxInfantWithSeat,resp.agentwareQueryID,resp.agentwareItineraryID 
			...
			--select * from #AllOneWayResponses where airSegmentMarketingAirlineCode like '%wn%'
			if @airRequestTripType = 1 
			BEGIN  
			update #AllOneWayResponses set otherLegPrice = 0 , otherLegTax = 0 
			END 
			--select @airRequestTripType
		END 
		...
		BEGIN
				...
				airPriceBaseSenior,airPriceTaxSenior,airPriceBaseChildren,airPriceTaxChildren,airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth,airPriceTaxYouth,AirPriceBaseTotal,AirPriceTaxTotal,airPriceBaseDisplay, airPriceTaxDisplay,airLegConnections,resp.airPriceBaseInfantWithSeat,resp.airPriceTaxInfantWithSeat
				FROM NormalizedAirResponses n WITH (NOLOCK) INNER JOIN AirResponse resp  WITH (NOLOCK) ON n.airresponsekey = resp.airResponseKey 
				INNER JOIN AirSubRequest subRequest WITH (NOLOCK) ON resp .airSubRequestKey = subRequest .airSubRequestKey 
				WHERE  airRequestKey = @airRequestKey AND airLegNumber = @airRequestTypeKey
	...
	BEGIN  
			...
			BEGIN
				...
				SET @flxairlines  = (SELECT distinct airSegmentMarketingAirlineCode FROM AirSegments WHERE airResponseKey = @selectedResponseKey)
				   ...
				   r.airPriceTaxInfantWithSeat + @airPriceTaxInfantWithSeatForAnotherLeg 
...
--select '@noOfLegsForRequest',@noOfLegsForRequest
...
--    INSERT INTO @tblMarketingAirlinesGroup
--SELECT seg.airSegmentMarketingAirlineCode, seg.airresponsekey,
--			CASE WHEN COUNT(distinct seg.airSegmentMarketingAirlineCode ) > 1 THEN dbo.udf_GetAirlineGroupName(seg.airSegmentMarketingAirlineCode)  ELSE MIN(seg.airSegmentMarketingAirlineCode) END 
--			FROM 
...
--			GROUP BY seg.airResponseKey,result.airOnePriceBaseDisplay ,gdssourcekey  ,result .airOnePriceTaxDisplay , result.airsubRequestkey ,result.cabinClass ,result.otherlegprice,otherLegTax ,legConnections,result.otherlegAirlines ,result.noOfOtherlegairlines,seg.airSegmentMarketingAirlineCode
--   SELECT * FROM @tblMarketingAirlinesGroup
 
  INSERT INTO @tblMarketingAirlinesGroup
  SELECT AV.AirlineCode, AG.AirVendorGroupName FROM Trip..AirVendorLookup AV
  INNER JOIN Trip..AirVendorGroup AG ON AV.AirVendorGroupKey = AG.AirVendorGroupKey
  GROUP BY  AV.AirlineCode, AG.AirVendorGroupName
  ...
  otherlegsAirlinesCount int 
       ...
       INSERT @secondLegDetails ( otherlegsAirlinesCount ,responsekey ,otherLegAirlines )
		SELECT COUNT(DISTINCT airSegmentMarketingAirlineCode) , seg.airResponseKey ,
			...
			MIN (airSegmentMarketingAirlineCode ) END ) 
		From airsegments seg WITH (NOLOCK)  INNER JOIN airresponse r WITH ( NOLOCK)
	...
	SELECT ROW_NUMBER() over (order by (case when @isTotalPriceSort = 0 then airOnePriceBaseDisplay  else airOnePriceBaseDisplay + airOnePriceTaxDisplay end )  ) AS airOneIdent , * FROM #AllOneWayResponses  
     ...
     --select '#tempOneWayResponses',* from #tempOneWayResponses where airSegmentMarketingAirlineCode like '%WN%'
	...
	(
		...
		(
			SELECT seg.airresponsekey,result.airOnePriceBaseDisplay 
				, CASE WHEN resp.gdsSourceKey = 12 THEN seg.airSegmentStops ELSE (CASE WHEN COUNT(seg.airresponsekey )-1  > 1 THEN (CASE WHEN @MaxNoofstops=2 THEN 2 ELSE 1 END) ELSE  COUNT(seg.airresponsekey )-1 END) END
				,COUNT(distinct seg.airSegmentMarketingAirlineCode ),MIN(airSegmentDepartureDate ) ,MAX(airSegmentArrivalDate ),
			...
			AirSegments seg  WITH (NOLOCK)  ON result .airOneResponsekey = seg.airResponseKey 
			WHERE airLegNumber = @airRequestTypeKey
			GROUP BY seg.airResponseKey,result.airOnePriceBaseDisplay ,resp.gdssourcekey  ,result .airOnePriceTaxDisplay , result.airsubRequestkey ,result.cabinClass ,result.otherlegprice,otherLegTax ,legConnections,result.otherlegAirlines ,result.noOfOtherlegairlines,airSegmentStops
		 )
		--select 'normalizedResultSet',* from  #normalizedResultSet where airsubrequetkey = 964966
		--return
--SELECT 'testing' test, * FROM #normalizedResultSet
    ...
    -- SELECT * FROM #normalizedResultSet
	 ...
	 END
 		...
 		,airSuperSaverTax,airEconSaverTax,airFirstFlexTax,airCorporateTax,airEconFlexTax,airEconUpgradeTax,airPriceBaseInfantWithSeat,airPriceTaxInfantWithSeat,agentwareQueryID,agentwareItineraryID,airsegmentPricingKey,airSegmentFareCategory,airSegmentBrandName,airSegmentBrandID,airSegmentBaggage,airSegmentMealCode)  
		   ...
		   otherlegPrice ,refundable, isBrandedFare, normalized.cabinclass ,fareType,segmentOrder ,seg.airsegmentCabin,(isnull(normalized.airPriceBase,0) + ISNULL (normalized.airpriceTax,0) ),seg.airSegmentOperatingFlightNumber,otherlegtax, airPriceBaseSenior,airPriceTaxSenior,airPriceBaseChildren,airPriceTaxChildren,airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth,airPriceTaxYouth,AirPriceBaseTotal,AirPriceTaxTotal,airPriceBaseDisplay, airPriceTaxDisplay, seg.airSegmentOperatingAirlineCompanyShortName,
		    ...
		    ,airSuperSaverTax,airEconSaverTax,airFirstFlexTax,airCorporateTax,airEconFlexTax,airEconUpgradetax,
		     resp.airPriceBaseInfantWithSeat, resp.airPriceTaxInfantWithSeat,agentwareQueryID,agentwareItineraryID,airsegmentPricingKey,airSegmentFareCategory,airSegmentBrandName,airSegmentBrandID,airSegmentBaggage,airSegmentMealCode
	...
	IF (@MaxInternationalFareTotal != 0)
	BEGIN
		...
		WHERE airresponsekey IN (SELECT A.airResponseKey from #airResponseResultset A WHERE (case when @isTotalPriceSort= 0 then A.airPrice  else A.totalcost end) > @MaxInternationalFareTotal)
...
---- print ( cast(getdate() AS time )  )
  ...
  /**MAIN RESULTSET FOR LIST STARTS HERE**/
	SELECT rowNum,air.*, airSegmentArrivalOffset, coalesce(departureAirport .CityName, departureAirportNonCommercial.CityName) AS DepartureAirPortCityName , coalesce(departureAirport.StateCode,departureAirportNonCommercial.StateCode) AS DepartureAirportStateCode , coalesce(departureAirport .AirportName,departureAirportNonCommercial.AirportName) AS DepartureAirportName , coalesce(departureAirport.CountryCode, departureAirportNonCommercial.CountryCode) AS DepartureAirportCountryCode, 
	coalesce(arrivalAirport .CItyName, arrivalAirportNonCommercial.CItyName) AS ArrivalAirPortCityName , coalesce(arrivalAirport .StateCode,arrivalAirportNonCommercial.StateCode) AS ArrivalAirportStateCode , coalesce(arrivalAirport.AirportName,arrivalAirportNonCommercial.AirportName) AS ArrivalAirportName , coalesce(arrivalAirport .CountryCode, arrivalAirportNonCommercial.CountryCode)  AS ArrivalAirportCountryCode,
	...
	CD.CountryName AS DepartureAirportCountryName, CA.CountryName AS ArrivalAirportCountryName,issmartFare,airPriceBaseInfantWithSeat,airPriceTaxInfantWithSeat
	...
	LEFT OUTER JOIN AirportLookupNonCommercial departureAirportNonCommercial WITH (NOLOCK)  ON air .airSegmentDepartureAirport = departureAirportNonCommercial .AirportCode 
	LEFT OUTER JOIN AirportLookupNonCommercial arrivalAirportNonCommercial  WITH (NOLOCK)  ON air .airSegmentArrivalAirport =arrivalAirportNonCommercial .AirportCode 	
  ...
  /****MIN-MAX PRICE FOR FILTERS ***/
   ...
   /* Stops for Slider END*/
	...
	BEGIN 
	    SELECT LowestPrice, MarketingAirlineName, airSegmentMarketingAirlineCode FROM (
	    SELECT LowestPrice, MarketingAirlineName, airSegmentMarketingAirlineCode, ROW_NUMBER() OVER(PARTITION BY MarketingAirlineName ORDER BY LowestPrice) AS RowNumber FROM (
	    SELECT LowestPrice, (CASE WHEN MG.airlineGroupName IS NULL  THEN MarketingAirlineName ELSE MG.airlineGroupName END) AS MarketingAirlineName, airSegmentMarketingAirlineCode FROM (
		...
		GROUP BY airlineName ,ShortName ) M
		LEFT OUTER JOIN @tblMarketingAirlinesGroup MG ON MG.airlineCode = M.airSegmentMarketingAirlineCode ) G )  R WHERE R.RowNumber =1
	...
	BEGIN
			...
			BEGIN
				...
				if(@superSetAirlines != '')
				BEGIN
					SELECT LowestPRice, airSegmentMarketingAirlineCode, NoOFSegments, MarketingAirlineName, noOFFLights,isSameAirlinesItin FROM (
					SELECT LowestPRice, airSegmentMarketingAirlineCode, NoOFSegments, MarketingAirlineName, SUM(noOFFLights) AS noOFFLights ,isSameAirlinesItin,  ROW_NUMBER() OVER(PARTITION BY MarketingAirlineName ORDER BY LowestPrice) AS RowNumber FROM (
                    SELECT LowestPRice, airSegmentMarketingAirlineCode, NoOFSegments, (CASE WHEN MG.airlineGroupName IS NULL  THEN MarketingAirlineName ELSE MG.airlineGroupName END) AS MarketingAirlineName, noOFFLights,isSameAirlinesItin FROM (
					...
					GROUP BY airvend .ShortName,airSegmentMarketingAirlineCode ,NoOFSegments ) M
					LEFT OUTER JOIN @tblMarketingAirlinesGroup MG ON MG.airlineCode = M.airSegmentMarketingAirlineCode ) G ) R WHERE R.RowNumber = 1
				END
				ELSE
				BEGIN	
     ...
     --               SELECT LowestPrice,NoOFSegments,airSegmentMarketingAirlineCode,noOFFLights,(CASE WHEN MG.airlineGroupName IS NULL  THEN MarketingAirlineName ELSE MG.airlineGroupName END) AS MarketingAirlineName FROM (
					--SELECT ( case when @isTotalPriceSort = 0 then  MIN(airPrice ) else min ( airprice + airpricetax) end )AS LowestPrice ,NoOfSTOPs AS NoOFSegments ,airlineName AS airSegmentMarketingAirlineCode,COUNT(distinct air.airResponseKey ) noOFFLights ,
					--ISNULL (ShortName,airlineName)AS MarketingAirlineName From #airResponseResultset air  
					--LEFT OUTER JOIN AirVendorLookup vendor  WITH (NOLOCK) ON air.airlineName = vendor .AirlineCode   
					--GROUP BY airlineName ,ShortName ,NoOfSTOPs )M
					--LEFT OUTER JOIN @tblMarketingAirlinesGroup MG ON MG.airlineCode = M.airSegmentMarketingAirlineCode
					...
					SELECT LowestPrice, airSegmentMarketingAirlineCode, NoOFSegments, MarketingAirlineName, noOFFLights,isSameAirlinesItin FROM (
					SELECT LowestPrice, airSegmentMarketingAirlineCode, NoOFSegments, MarketingAirlineName ,isSameAirlinesItin, SUM(noOFFLights)  OVER(PARTITION BY MarketingAirlineName) AS noOFFLights, ROW_NUMBER() OVER(PARTITION BY MarketingAirlineName ORDER BY LowestPrice) AS RowNumber FROM (										
					SELECT LowestPrice,NoOFSegments,airSegmentMarketingAirlineCode,noOFFLights,(CASE WHEN MG.airlineGroupName IS NULL  THEN MarketingAirlineName ELSE MG.airlineGroupName END) AS MarketingAirlineName,isSameAirlinesItin
					FROM @MatrixResult M
					LEFT OUTER JOIN @tblMarketingAirlinesGroup MG ON MG.airlineCode = M.airSegmentMarketingAirlineCode
					 )G ) R WHERE R.RowNumber = 1
    ...
    (
        -- Use a sub query for child elements.
         SELECT ROW_NUMBER()  over ( order by segmentOrder)RowID,
             coalesce(depart.CityName,departureAirportNonCommercial.CityName) DepartCityName ,
             CAST (coalesce(depart.Latitude,departureAirportNonCommercial.Latitude) as VARCHAR(200)) "DepartAirport/@Lattitude" ,
             CAST (coalesce(depart.Longitude,departureAirportNonCommercial.Longitude) as VARCHAR(200))"DepartAirport/@Longitude",
             CAST (coalesce(depart.StateCode,departureAirportNonCommercial.StateCode) as VARCHAR(2))"DepartAirport/@StateCode" ,
             coalesce(DEPART.AirportCode,departureAirportNonCommercial.AirportCode) as DepartAirport  ,
             
             coalesce(arrival.CityName,arrivalAirportNonCommercial.CityName) ArrivalCityName ,
             CAST (coalesce(arrival.Latitude,arrivalAirportNonCommercial.Latitude) as VARCHAR(200)) "ArrivalAirport/@Lattitude" ,
             CAST (coalesce(Arrival.Longitude,arrivalAirportNonCommercial.Longitude) as VARCHAR(200))"ArrivalAirport/@Longitude"  ,
             CAST (coalesce(Arrival.StateCode,arrivalAirportNonCommercial.StateCode) as VARCHAR(2))"ArrivalAirport/@StateCode" ,
             coalesce(Arrival.AirportCode,arrivalAirportNonCommercial.AirportCode) as ArrivalAirport  
             
        FROM
        #airResponseResultset result  LEFT OUTER JOIN AirportLookup  depart WITH (NOLOCK)
                                      on result.airSegmentDepartureAirport = depart.AirportCode
                                      --added by kailash rathod 
                                      LEFT OUTER JOIN AirportLookupNonCommercial departureAirportNonCommercial WITH (NOLOCK)  
                                      ON result.airSegmentDepartureAirport = departureAirportNonCommercial .AirportCode  
                                      LEFT OUTER JOIN AirportLookup  arrival WITH (NOLOCK) 
                                      on result.airSegmentArrivalAirport  = arrival.AirportCode 
                                      LEFT OUTER JOIN AirportLookupNonCommercial arrivalAirportNonCommercial WITH (NOLOCK)  
                                      ON result.airSegmentArrivalAirport = arrivalAirportNonCommercial .AirportCode  
        WHERE
