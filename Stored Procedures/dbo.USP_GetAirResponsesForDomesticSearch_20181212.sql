...
CREATE PROCEDURE [dbo].[USP_GetAirResponsesForDomesticSearch_20181212](  
--DECLARE
...
SELECT TOP  1 @TripRequestkey = Triprequestkey, 
	  @Type = 'StoredProcedure', @WSName = 'DomesticSearch Start',
	  @XmlData = '', @Event = 'DomesticSearch Start', @Details = 'DomesticSearch Start',
	  ...
	  @SingleBookThreadId = '',
	  @GroupBookThreadId =''
FROM trip..TripRequest_air  TRA
INNER JOIN TRIP..AirSubRequest AR ON TRA.airRequestKey=AR.airRequestKey
where  AR.airSubRequestKey=@airSubRequestKey
 ...
 --       ,ERROR_MESSAGE() AS ErrorMessage;
	...
	CREATE NONCLUSTERED INDEX [IDX_tempairResponseID] ON #AirResponse([airResponseID]) 
...
SELECT TOP  1 @TripRequestkey = Triprequestkey, 
	  @Type = 'StoredProcedure', @WSName = 'DomesticSearch End NoofRecordsFetched:' + Cast(@NofRecord as Varchar),
	  @XmlData = '', @Event = 'DomesticSearch End NoofRecordsFetched:' + Cast(@NofRecord as Varchar), 
	  @Details = 'DomesticSearch End NoofRecordsFetched:' + Cast(@NofRecord as Varchar),
	  @ExceptionMessage = ERROR_MESSAGE(),
	  @StackTrace = ERROR_STATE(),
	  @SessionId = ERROR_NUMBER(),
	  @LogLevelKey = ERROR_LINE(),
