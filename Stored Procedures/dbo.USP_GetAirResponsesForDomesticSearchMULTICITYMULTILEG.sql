...
CREATE PROCEDURE [dbo].[USP_GetAirResponsesForDomesticSearchMULTICITYMULTILEG]
	(  
	...
	-------------------------------------------------
	------------------------------------------------
	---------------------------------------------- Ashima only 1 row fetching so currently returning only 
...
select * from @multiLegPrice
		...
		--  SET @anotherLegAirlines = (SELECT Top 1 N.airlines FROM NormalizedAirResponses N Inner join @multiLegPrice M on N.airresponsekey = M.airresponsekey )
				...
				--select * from @SecondLegs_CTE
				-----------------------------------
				------------------------------------------------------Question two times Group By why seg.airResponseKey
...
--return
	...
	------------------------------------ Ashima checking whether the selected response is RT OR OW
	...
	IF ( @airRequestTypeKey = 1 )    ---- Ashima First Leg --------
	BEGIN   
		INSERT #AllOneWayResponses (airOneResponsekey,airOnePriceBase,airOnePriceBaseSenior,airOnePriceTaxSenior,airOnePriceBaseChildren,airOnePriceTaxChildren,airOnePriceBaseInfant,airOnePriceTaxInfant,airOnePriceBaseYouth,airOnePriceTaxYouth,airOnePriceBaseTotal,airOnePriceTaxTotal,airOnePriceBaseDisplay, airOnePriceTaxDisplay,airSegmentFlightNumber,airSegmentMarketingAirlineCode,airsubRequestkey,airOnePriceTax ,airpriceTotal,otherLegprice,cabinclass  ,otherlegtax ,legConnections,airOnePriceBaseInfantWithSeat,airOnePriceTaxInfantWithSeat)  
		...
		(airPriceBaseInfantWithSeat + ISNULL(@airPriceInfantWithSeatForAnotherLeg,0)  ),(airPriceTaxInfantWithSeat + ISNULL(@airPriceTaxInfantWithSeatForAnotherLeg,0))
		...
		print('remove')
		---- Ashima Published Fare Also--------------------
		if ( @airRequestType =1  ) --------------------- Ashima If OW --------------------------
		BEGIN
		INSERT #AllOneWayResponses (airOneResponsekey,airOnePriceBase,airOnePriceBaseSenior,airOnePriceTaxSenior,airOnePriceBaseChildren,airOnePriceTaxChildren,airOnePriceBaseInfant,airOnePriceTaxInfant,airOnePriceBaseYouth,airOnePriceTaxYouth,airOnePriceBaseTotal,airOnePriceTaxTotal,airOnePriceBaseDisplay, airOnePriceTaxDisplay,airSegmentFlightNumber,airSegmentMarketingAirlineCode,airsubRequestkey,airOnePriceTax ,airpriceTotal,otherLegprice,cabinclass  ,otherlegtax ,legConnections ,airOnePriceBaseInfantWithSeat,airOnePriceTaxInfantWithSeat)  
		SELECT resp.airresponsekey, (airPriceBase + ISNULL(@airPriceForAnotherLeg,0)  ),
		(airPriceBaseSenior + ISNULL(@airPriceSeniorForAnotherLeg,0)  ),(airPriceTaxSenior + ISNULL(@airPriceTaxSeniorForAnotherLeg,0)),
	...
	BEGIN  
		...
		(airPriceTaxInfantWithSeat + ISNULL(@airPriceTaxInfantWithSeatForAnotherLeg,0))
		From NormalizedAirResponses n WITH (NOLOCK) inner join AirResponse resp WITH (NOLOCK) on n.airresponsekey = resp.airResponseKey WHERE resp.airSubRequestKey = @airSubRequestKey   
		--and gdsSourceKey <> 9  
		END
		END  
	...
	IF @airRequestTypeKey > 1 ------------- Leg 2 Onwards-----------------
	BEGIN
	...
	INNER JOIN @tempResponseToRemove T  ON P.airOneResponsekey = T.airresponsekey  ---------------- Delete Responses which are not to be considered--------------------
	...
	result.airOnePriceBaseInfantWithSeat,result.airOnePriceTaxInfantWithSeat
	FROM   
	...
	,airSuperSaverTax,airEconSaverTax,airFirstFlexTax,airCorporateTax,airEconFlexTax,airEconUpgradetax,resp.airPriceBaseInfantWithSeat,resp.airPriceTaxInfantWithSeat
