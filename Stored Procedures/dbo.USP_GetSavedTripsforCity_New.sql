...
--USP_GetSavedTripsforCity_NEW 'DFW','From' ,5,9
 CREATE Procedure [dbo].[USP_GetSavedTripsforCity_New] 
(
@cityCode varchar(20) ,
...
if   (SELECT COUNT(*)  from @Tripdetails) < 6 
BEGIN 
...
update @Tripdetails SET  tripTotal = (isnull(DTAP.tripAdultBase,Dtap.tripSeniorBase ) + ISNULL(Dtap.tripAdultTax ,Dtap.tripSeniorTax)) , tripAirsavings = 
...
--
update  @Tripdetails SET tripTotal = ISNULL(triptotal,0) + (DTR.hotelTotalPrice/2) , triphotelsavings = (( DTR.hotelTotalPrice - THR.hotelTotalPrice ) / 2)
 FROM TripHotelResponse THR inner join @Tripdetails TD on thr.tripGUIDKey =td.tripsavedKey 
inner join @deal  D on tD.tripKey = d.tripkey 
inner join TripSavedDeals TSD on tsd.TripSavedDealKey = d.dealId 
inner join TripHotelResponse DTR 
ON DTR.hotelResponseKey = tsd.responseKey 

--tripTotal =ISNULL(triptotal,0) + ( DTR.SearchCarPrice+ DTR.searchCarTax),
update  @Tripdetails SET tripTotal = ISNULL(triptotal,0)+( DTR.SearchCarPrice+ DTR.searchCarTax) ,  tripcarsavings =   ( DTR.SearchCarPrice+ DTR.searchCarTax)   - ( THR.SearchCarPrice+ THR.searchCarTax) 
...
SELECT t.* ,FA.CityName as fromCity , FA.StateCode fromState , fa.CountryCode fromCountry ,TA.CityName as ToCity , TA.StateCode ToState , Ta.CountryCode ToCountry  FROM @Tripdetails  T
left outer join 
AirportLookup FA on 
