...
CREATE PROCEDURE [dbo].[USP_GetAirResponsesForDomesticSearch_2803201_19](  
--DECLARE
...
/*
   ...
   -- SELECT * INTO #AirSubRequest  FROM AirSubRequest WHERE airSubRequestKey IN (SELECT airSubRequestKey FROM AirSubRequest WITH(NOLOCK) WHERE airrequestKey = @airRequestKey)
    SELECT * INTO #AirSubRequest  FROM AirSubRequest WHERE  airrequestKey = @airRequestKey
	OPTION (MAXDOP 1)
	CREATE CLUSTERED INDEX [IDX_tempAirSubRequestKey] ON #AirSubRequest([airSubRequestKey]) 
...
--	INSERT INTO #AirResponse
	SELECT [airResponseKey]
      ...
      ,[isAvailable]
      ,[isReturnFare] 
	INTO #AirResponse	FROM AirResponse WITH(NOLOCK) WHERE airSubRequestKey IN (SELECT airSubRequestKey FROM #AirSubRequest)
		OPTION(MAXDOP 1)
		CREATE CLUSTERED INDEX [PX_airSubRequestKey] ON #AirResponse([airResponseKey]) 
	...
	SELECT [airSegmentKey]
	  ...
	  INTO #Airsegments
		 ...
		 OPTION(MAXDOP 1 )
		  CREATE NONCLUSTERED INDEX [PK_airSegmentKey]	ON #Airsegments ([airSegmentKey]) 
	...
	CREATE NONCLUSTERED INDEX [IDX_airResponseKey]	ON #Airsegments ([airResponseKey],[airLegNumber],[airSegmentDepartureAirport],[airSegmentArrivalAirport])INCLUDE([airSegmentArrivalDate],[airSegmentMarketingAirlineCode],[AirSegmentDepartureOffset],[AirSegmentArrivalOffset],[segmentOrder])
    ...
    SELECT *,CONVERT(VARCHAR(20), '') AS airlineCode INTO #NormalizedAirResponses FROM NormalizedAirResponses
	 WHERE airsubrequestkey IN (SELECT airSubRequestKey FROM #AirSubRequest)
	 OPTION(MAXDOP 1)
	 CREATE CLUSTERED INDEX [PK_NormalizedAirResponses]	ON #NormalizedAirResponses ([airsubrequestkey]) 
	 
    IF (@isMultiBrand=1)
    BEGIN
	
		SELECT AB.* INTO #AirResponseMultiBrand FROM AirResponseMultiBrand AB
		inner join #AirSubRequest AR ON  AB.airSubRequestKey=AR.airSubRequestKey
		OPTION(MAXDOP 1)
	--	WHERE airSubRequestKey IN (SELECT airSubRequestKey FROM #AirSubRequest)
		...
		SELECT ASMB.* INTO #AirSegmentsMultiBrand FROM AirSegmentsMultiBrand ASMB WITH(NOLOCK) 
			INNER  JOIN (SELECT AB.* FROM AirResponseMultiBrand AB
		inner join #AirSubRequest AR WITH (INDEX(IDX_tempAirSubRequestKey)) ON  AB.airSubRequestKey=AR.airSubRequestKey) t ON ASMB.airResponseKey = t.airResponseKey 
			OPTION(MAXDOP 1)
			print getdate()
		...
		OPTION (MAXDOP 1)
			CREATE CLUSTERED INDEX [IX_airSegmentMultiBrandKey_PK] ON #AirSegmentsMultiBrand(airResponseKey)
			CREATE NONCLUSTERED INDEX [IX_airSegmentMultiBrandKey] ON #AirSegmentsMultiBrand(airSegmentMultiBrandKey)
	CREATE NONCLUSTERED INDEX [IDX_airResponseKey_SegmentOrder_airLegNumber] ON #AirSegmentsMultiBrand ([airResponseKey])
		INCLUDE ([segmentOrder], [airLegNumber]) 
	CREATE NONCLUSTERED INDEX [IX_airResponseMultiBrandKey] ON #AirSegmentsMultiBrand([airResponseMultiBrandKey] ASC,[airLegNumber])
	
ALTER TABLE #AirResponseMultiBrand ADD PRIMARY KEY (airresponseMultiBrandkey)
CREATE NONCLUSTERED INDEX [IDX_airResponseMultiBrandKey] ON #NormalizedAirResponsesMultiBrand 
([airresponseMultiBrandkey])INCLUDE(flightNumber,airlines,airSubRequestKey,airLegBookingClasses,airLegConnections,cabinClass,airLegBrandName,airLegNumber)
		...
		--select '@isMultiBrandSelectedOnPreviousLeg',@isMultiBrandSelectedOnPreviousLeg
			...
			BEGIN  
				...
				BEGIN 
					...
					BEGIN
					DECLARE @selectedBrandNameOnLeg1 AS VARCHAR(20)
					DECLARE @selectedFlightNumberOnLeg1 AS VARCHAR(20)
					DECLARE @selectedAirlinesOnLeg1 AS VARCHAR(20)
					SELECT @selectedBrandNameOnLeg1 = airLegBrandName,@selectedFlightNumberOnLeg1 = flightNumber,@selectedAirlinesOnLeg1 = airlines FROM #NormalizedAirResponsesMultiBrand WHERE airresponseMultiBrandkey = @SelectedResponseMultiBrandKey and airLegNumber =1
						...
						BEGIN
							...
							inner join #AirResponseMultiBrand AR WITH(NOLOCK) on n.AirResponseMultiBrandKey = AR.airResponseMultiBrandKey
							 WHERE airlegnumber =1
							and  airSubRequestLegIndex = 1 and flightNumber = @selectedFlightNumberOnLeg1 AND AIRLINES = @selectedAirlinesOnLeg1 
							AND airLegBrandName = @selectedBrandNameOnLeg1
							AND N.airsubrequestkey <> @airBundledRequest AND N.airsubrequestkey <> @airMultiCabinBundledRequest)
							order by (airPriceBaseDisplay + airPriceTaxDisplay) asc
						END
						ELSE
						...
						BEGIN
							...
							BEGIN
								...
								inner join #AirResponse AR WITH(NOLOCK) on n.airresponsekey = AR.airResponseKey
								 WHERE airlegnumber =1
								and  airSubRequestLegIndex = 1 and flightNumber = @selectedFlightNumberOnLeg1  AND AIRLINES = @selectedAirlinesOnLeg1
								 AND UPPER(airLegBrandName) = @selectedBrandNameOnLeg1
								 AND N.airsubrequestkey <> @airBundledRequest AND N.airsubrequestkey <> @airMultiCabinBundledRequest ) 
								order by (airPriceBaseDisplay + airPriceTaxDisplay) asc
							END
							ELSE
			...
			BEGIN   
				...
				BEGIN 
					...
					BEGIN
						...
						--select '444',* from @upSellLegPrice
			...
			BEGIN  
				...
				BEGIN
					...
					on resp .airResponseKey = SELECTed .responsekey
					END
				--select '2',* from @SELECTedResponse
				--select '1',* from @multiLegPrice
...
--return
	...
	CREATE NONCLUSTERED INDEX [IDX_airOneResponseKey] ON #AllOneWayResponses ([airOneResponsekey])INCLUDE([gdsSourceKey])
	...
	BEGIN
		...
		-- Deletion to make the ResultSet Small
				--select * from #AllOneWayResponses_Leg2 where airSegmentFlightNumber = '2199,1402' and airLegBrandName = 'Select'
		Delete P  
		...
		-- Commented because No Return flight also needs to be retained for SameDayReturn
		...
		--select * from #AllOneWayResponses_Leg1 order by airGroupId asc
		--select *,airSegmentFlightNumber,airSegmentMarketingAirlineCode from #AllOneWayResponses_Leg2 order by airGroupId asc
        ...
        CREATE CLUSTERED INDEX IX_airGroupId ON #AllOneWayResponses_Leg2_1(airGroupId)
		--select * from #AllOneWayResponses_Leg2_1 order by ID_Loop 
		...
		--select * from #AllOneWayResponses_Legs_Merge order by airOneIdent
		...
		--select top 100 airOneIdent,airLegBrandName,isRefundable from #AllOneWayResponses_Legs_Merge
		select airOneIdent,airLegBrandName,isRefundable from #AllOneWayResponses_Legs_Merge
		where airlegnumber=1 

		--Drop table #DataProcess
		--select * into ##Temp_Working_org from ##Temp_Working
		...
		BEGIN
		Select @airOneIdent=airOneIdent,@airLegBrandName=airLegBrandName,@isRefundable=isRefundable 
		from #DataProcess where Row_id=@START_LOOP_Merge

		--IF @airLegBrandName='Main' and @isRefundable=0
		--BEGIN
		--select top 1 
		--	Case when leg1.airLegBrandName='Main' and leg1.isRefundable=0 THEN
		--	airPriceTaxLowest_Main_L END
		--	--@airPriceLowest_Main_L=airPriceLowest_Main_L,@airPriceTaxLowest_Main_L=airPriceTaxLowest_Main_L
		--from ##Temp_Working leg2
		--where leg2.airlegnumber=2 
		--and leg2.airOneIdent>358
		--and exists(select 1 from ##Temp_Working leg1
		--where airOneIdent=358 and airlegnumber=1 and Datediff(minute,Leg1.legDateTime,Leg2.legDateTime)>=60)
		--order by 1
		...
		--select @airPriceLowest,@airPriceTaxLowest
		...
		--select airOnePriceBase,airOnePriceTax,otherLegprice,otherlegtax,isValid,* from #AllOneWayResponses_Legs_Merge where airLegNumber = 1 order by airOneIdent

		--return
		--select * from #AllOneWayResponses_Leg1
		--select * from #AllOneWayResponses_Leg2
	...
	BEGIN   
		...
		IF(@airRequestType <> 1 AND @airRequestTypeKey != @isSameDayReturnOWLogicToApply)
		BEGIN
			IF (@isCabinUniquification = 0)
			BEGIN
...
IF(@isMultiBrand = 1)
	BEGIN
		...
		BEGIN
			...
			IF(@isSameDayReturnOWLogicToApply = 1)
			BEGIN
				delete #AllOneWayResponses  
				FROM #AllOneWayResponses t,  
				(  
		...
		BEGIN 
		 ...
		 BEGIN
			...
			WHERE t.airSegmentFlightNumber = derived.airSegmentFlightNumber AND t.airSegmentMarketingAirlineCode =derived .airSegmentMarketingAirlineCode AND t.airLegBookingClasses = derived.airLegBookingClasses 
			AND (airOnePriceBase + airOnePriceTax) >  minPrice  --AND airOneIdent > minIdent 
			and t.airsubRequestkey = @airMultiCabinRequest
	...
	BEGIN
		...
		INSERT INTO #AdditionalFares(airResponseKey,airresponseMultiBrandkey,TotalPriceToDisplay,airLegBrandName,TotalAllPaxPriceToDisplay,childresponsekey,isRefundable,airResBookDesigCode,airRequestKey)
			SELECT t.airOneResponsekey,t.airResponseMultiBrandKey,(t.airOnePriceBaseDisplay + t.airOnePriceTaxDisplay),t.airLegBrandName,(t.airOnePriceBaseTotal + t.airOnePriceTaxTotal),t.childResponsekey,t.isRefundable,t.airLegBookingClasses,t.airsubRequestkey
			From #AllOneWayResponses t 
