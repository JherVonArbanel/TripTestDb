...
/***********************************                 
 ...
 Updated proximity search using getDistance function to calculate distance of search lat,long vs hotel lat,long.
  ...
  Updated - Manoj on 28-02-2019 14:29  
 Added HotelBeds
 ...
 **********************************/                
CREATE PROCEDURE [dbo].[USP_GetHotelResponsesForRequest2_MarketPlace]      
(       
--declare       
 @hotelRequestKey  INT,      
...
BEGIN         
 SET NOCOUNT ON;   
 DECLARE @TripType VARCHAR(50) , @tripTypeKey INT, @IsPolicyApplicable BIT= 0,
...
begin try
   exec USP_UpdateHotelAvgRate @hotelRequestKey
end try
begin catch
   print  'Error'
end catch
 ...
 (     
  ...
  ,CompanyContractApplied [varchar](50) NULL
  ,isAvgRateUpdated bit
  ,pricelineNetRate float NULL
  ,hotelbedsNetRate float NULL
  ,atMerchant bit
  ,imageUrl varchar(500) NULL
 )    
...
WHERE (HR.proximityDistance = 0  or HR.proximityDistance is NULL) AND HR.hotelRequestKey=@hotelRequestKey
 ...
 AND supplierId = 'Priceline'  AND atMerchant = 1 AND minRate <= @maxPrice
 ...
 --WHERE supplierId = 'HotelBeds' AND minRate <= @maxPrice   
 WHERE TH.hotelId NOT IN    
 (      
  SELECT hotelId    
  FROM #FinalHotelResponse    
 )    
 AND supplierId = 'HotelBeds'  AND minRate <= @maxPrice --AND AH.Miles <=40 
 ...
 INNER JOIN #TmpHotelResponse THR    
 ON THR.hotelId = FHR.hotelId    
 AND THR.supplierId = 'Priceline' AND THR.atMerchant=0
 ...
 AND supplierId = 'Priceline'  AND atMerchant=0  AND minRate <= @maxPrice --AND AH.Miles <=40 
...
BEGIN    
  ...
  ,ZipCode,PhoneNumber,FaxNumber, CityCode,CountryName,distance,checkInDate,checkOutDate,HotelDescription,ChainName,minRateTax,ImageURL,         
        ...
        ,promoId, eanBarRate, touricoCalculatedBarRate, touricoNetRate, touricoCostBasisRate, marketPlaceVariableId, isNonRefundable,proximityDistance,proximityUnit,corporateRate,payLaterRate,payNowRate,geoSequence,hasGovRate,airportDistance,cityCenterDistance,CompanyContractApplied,isAvgRateUpdated,hotelbedsNetRate,pricelineNetRate    
 )    
   (      
  SELECT FHR.hotelResponseKey,FHR.supplierHotelKey,FHR.hotelRequestKey,FHR.SupplierId,FHR.minRate,HT.HotelName,HT.Rating,HT.RatingType, FHR.corporateCode, FHR.preferenceOrder,    
   ...
   FHR.minRateTax,    
   (CASE WHEN (ISNULL(HI.SupplierImageURL,'')='') THEN FHR.imageUrl ELSE HI.SupplierImageURL END) AS SupplierImageURL,'','',HT.reviewRating as tripAdvisorRating ,'','',''    
   ...
   ,FHR.promoId, FHR.eanBarRate, FHR.touricoCalculatedBarRate, FHR.touricoNetRate, FHR.touricoCostBasisRate, FHR.marketPlaceVariableId, FHR.isNonRefundable, FHR.proximityDistance, FHR.proximityUnit,FHR.corporateRate,FHR.payLater,FHR.payNowRate,FHR.geoSequence,FHR.hasGovRate
	...
	--, ISNULL(FHR.proximityDistance, ISNULL(HCM.Miles,0))
	 FHR.CompanyContractApplied,FHR.isAvgRateUpdated,FHR.hotelbedsNetRate,FHR.pricelineNetRate   
