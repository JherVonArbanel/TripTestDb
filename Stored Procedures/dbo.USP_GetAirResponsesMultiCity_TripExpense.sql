...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[USP_GetAirResponsesMultiCity_TripExpense]  
(  
 ...
 From AirSegments seg INNER JOIN AirResponse  resp ON seg .airResponseKey = resp.airresponsekey   
   INNER JOIN AirSubRequest airSub  on resp.airSubRequestKey = airsub.airSubRequestKey 
 ...
 WHERE 
airRequestKey = @airRequestKey  ---airLegNumber = @airRequestTypeKey    AND (airSubRequestKey = @airBundledRequest or airSubRequestKey =@airSubRequestKey )    
 ...
 IF ( @superSetAirlines <> '' AND @superSetAirlines is not null )  
  ...
  -- To remove affiliate airline results from @tempResponseToRemove
	IF (@affiliateAirlines is not null AND @affiliateAirlines <> '')   ---- Exp. @affiliateAirlines = AY-BE-Nordic,SA-BE-USA
	BEGIN		
		DECLARE @tblAffiliate TABLE (RowID INT NOT NULL IDENTITY(1,1), strAffiliate VARCHAR(100))		
		DECLARE @rowID int = 1 , @rowCount int = 0 

		INSERT INTO @tblAffiliate SELECT String FROM vault .dbo.ufn_CSVToTable (@affiliateAirlines) 
		SET @rowCount = @@ROWCOUNT
		IF @rowCount > 0
		BEGIN	
			WHILE @rowID <= @rowCount  
			BEGIN  
				DECLARE @marketingAir VARCHAR(3), @operatingAir VARCHAR(3), @zone VARCHAR(50)
				DECLARE @strAffiliate VARCHAR(100), @index INT
				
				SELECT @strAffiliate = strAffiliate FROM @tblAffiliate WHERE RowID = @rowID
				
				-- get marketing air
				SET @index = CHARINDEX('-', @strAffiliate, 0)         
				SET @marketingAir = SUBSTRING(@strAffiliate, 0, @index)	
				
				set @strAffiliate = SUBSTRING(@strAffiliate, @index + 1, LEN(@strAffiliate))
				set @index = CHARINDEX('-', @strAffiliate, 0) 				
				
				-- get operating air				
				IF @index > 0	---IF Zone is specified (MK-OP-ZONE)
				BEGIN					
					set @operatingAir = SUBSTRING(@strAffiliate, 0, @index)	        
					-- get Zone (Region)
					set @strAffiliate = SUBSTRING(@strAffiliate, @index + 1, LEN(@strAffiliate))		
					set @zone = @strAffiliate
				END	
				ELSE			--ELSE No Zone specified (MK-OP)
				BEGIN					
					set @operatingAir = @strAffiliate
					set @zone = NULL
				END
								
				DELETE FROM @tempResponseToRemove where airresponsekey in
				...
				inner join AirportLookup AL1 on S.airSegmentDepartureAirport = AL1.AirportCode and AL1.Zone = ISNULL(@zone,AL1.Zone)
				inner join AirportLookup AL2 on S.airSegmentArrivalAirport = AL2.AirportCode  and AL2.Zone = ISNULL(@zone,AL2.Zone)
				where airRequestKey = @airRequestKey -- AND groupKey =2 
				and airSegmentMarketingAirlineCode = @marketingAir
				and airSegmentOperatingAirlineCode = @operatingAir)
...
Delete P  
 ...
 AirPriceBaseTotal,AirPriceTaxTotal,airPriceBaseDisplay, airPriceTaxDisplay,airSegmentOperatingAirlineCompanyShortName)  
