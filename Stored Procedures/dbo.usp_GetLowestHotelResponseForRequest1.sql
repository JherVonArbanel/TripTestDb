...
CREATE PROCEDURE [dbo].[usp_GetLowestHotelResponseForRequest1] (@hotelRequestID INT , @hotelRating FLOAT , @regionId INT, @hotelGroupID INT = 0    
 ,@noOfResults INT = 1    
 )        
AS         
  BEGIN        
...
DECLARE @hotelResponse AS TABLE (hotelResponseKey  UNIQUEIDENTIFIER , ID INT identity (1,1))        
 ...
 SET @newCount = @noOfResults -  ( SELECT COUNT(*) FROM @hotelResponse )
	   ...
	   SELECT TOP  (@newCount)  *  FROM #tmpResponse  HR         
	   inner join HotelContent.dbo.Hotels AS HT ON HT.HotelId = HR.HotelId         
 ...
 SELECT   DISTINCT top (@noOfResults )       
       HR.hotelResponseKey, HR.supplierHotelKey, HR.hotelRequestKey, HR.supplierId, HR.minRate, HT.HotelName, HT.Rating, HT.RatingType, HT.ChainCode, HT.HotelId,       
                      ...
                      HT.richMediaUrl    ,ID
 FROM         dbo.HotelResponse AS HR   inner join @hotelResponse  VW ON VW.hotelResponseKey = HR.hotelResponseKey INNER JOIN      
          
                      HotelContent.dbo.SupplierHotels1 AS SH ON SH.SupplierHotelId = HR.supplierHotelKey AND SH.SupplierFamily = HR.supplierId INNER JOIN      
                      HotelContent.dbo.Hotels AS HT ON SH.HotelId = HT.HotelId LEFT OUTER JOIN      
