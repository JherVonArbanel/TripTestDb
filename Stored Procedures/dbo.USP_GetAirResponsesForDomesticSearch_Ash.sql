...
CREATE PROCEDURE [dbo].[USP_GetAirResponsesForDomesticSearch_Ash](  
--DECLARE
	...
	DECLARE @isSameDayReturnOWLogicToApply AS BIT =0
	...
	SELECT @isSameDaySearch = CASE WHEN DATEDIFF(MINUTE,CONVERT(date,airRequestArrivalDate),CONVERT(date,airRequestDepartureDate)) = 0 THEN 1 ELSE 0 END FROM #AirSubRequest WITH(NOLOCK) where airSubRequestKey = @airBundledRequest
	IF(@isSameDaySearch = 0 AND @IsDomesticRegionTravel = 0)
	BEGIN
		SELECT @isSameDaySearch = CASE WHEN DATEDIFF(MINUTE,CONVERT(date,airRequestArrivalDate),CONVERT(date,airRequestDepartureDate)) <= 2 THEN 1 ELSE 0 END FROM #AirSubRequest WITH(NOLOCK) where airSubRequestKey = @airBundledRequest
	END
  ...
  BEGIN
		...
		--select @SelectedResponseArrivalAirportDate,@SelectedResponseArrivalAirport
		--select * from #Airsegments
		--select * from @tempResponseToRemove
		--select @SelectedResponseArrivalAirport,@SelectedResponseArrivalAirportDate
		--select @airMultiCabinRequest,@airSubRequestLeg2
		INSERT @tempResponseToRemove (airresponsekey)
		(SELECT DISTINCT seg.airResponseKey  FROM #AirSegments seg WITH(NOLOCK) 
		INNER JOIN #AirResponse resp ON seg.airResponseKey = resp.airResponseKey   
		INNER JOIN #AirSubRequest subReq ON resp.airSubRequestKey =subReq.airSubRequestKey
		...
		AND subreq.airSubRequestKey IN (@airSubRequestLeg2,@airMultiCabinRequest))
		--select '2',* from @tempResponseToRemove
		INSERT @tempResponseToRemove (airresponsekey) 
		(SELECT DISTINCT seg.airResponseKey  FROM #AirSegments seg WITH(NOLOCK) 
		INNER JOIN #AirResponse resp ON seg.airResponseKey = resp.airResponseKey   
		INNER JOIN #AirSubRequest subReq ON resp.airSubRequestKey =subReq.airSubRequestKey
		...
		--select * FROM #AirSegments seg WITH(NOLOCK) 
		--INNER JOIN #AirResponse resp ON seg.airResponseKey = resp.airResponseKey   
		--INNER JOIN #AirSubRequest subReq ON resp.airSubRequestKey =subReq.airSubRequestKey
		--WHERE seg.airLegNumber = 2 
		--AND segmentOrder = 1
		--AND subreq.airSubRequestKey IN (@airSubRequestLeg2,@airMultiCabinRequest)
		--order by seg.airSegmentDepartureDate desc
		--INSERT @tempResponseToRemove (airresponsekey) 
		--(SELECT DISTINCT seg.airResponseKey  FROM #AirSegments seg WITH(NOLOCK) 
		--INNER JOIN #AirResponse resp ON seg.airResponseKey = resp.airResponseKey   
		--INNER JOIN #AirSubRequest subReq ON resp.airSubRequestKey =subReq.airSubRequestKey
...
--return
	...
	BEGIN
         ...
         --OW Insert 
	        IF(@isMultiBrand = 1 AND @isSameDayReturnOWLogicToApply = 0)
				...
				BEGIN
				select 'before OW', * from #AllOneWayResponses 
	...
	BEGIN
		...
		where P.airResponseMultiBrandKey IS NOT NULL AND P.isMultiBrandFare = 1
	END
	--select '1',* from #AllOneWayResponses where airSegmentMarketingAirlineCode like '%AA%' and airLegBrandName = 'Main'
	IF(@isMultiBrand = 1 AND @isCabinUniquification = 1)
	BEGIN
	-- Both the legs should have the same cabin
		DELETE #AllOneWayResponses  
...
IF(@isMultiBrand = 1)
	BEGIN
		...
		BEGIN
			...
			--select '2',* from #AllOneWayResponses where airSegmentMarketingAirlineCode like '%AA%' and airLegBrandName = 'Main'
			...
			and (t.airsubRequestkey = @airMultiCabinRequest OR t.airsubRequestkey = @airMultiCabinBundledRequest)
			 ...
			 select '1.1',* from @Temp_AllOneWayResponses where airSegmentMarketingAirlineCode like '%AA%' --and airLegBrandName = 'Main'
			INSERT INTO @Temp_AllOneWayResponses(airOneResponsekey,airOnePriceBase,airOnePriceBaseSenior ,airOnePriceTaxSenior, airOnePriceBaseChildren ,airOnePriceTaxChildren ,airOnePriceBaseInfant, airOnePriceTaxInfant,airOnePriceBaseYouth, airOnePriceTaxYouth, airOnePriceBaseTotal, airOnePriceTaxTotal,airOnePriceBaseDisplay, airOnePriceTaxDisplay,airSegmentFlightNumber,airSegmentMarketingAirlineCode,airsubRequestkey,airOnePriceTax ,airpriceTotal ,cabinclass  ,legConnections,airOnePriceBaseInfantWithSeat, airOnePriceTaxInfantWithSeat,  airLegBrandName,airLegBookingClasses,airResponseMultiBrandkey,isMultiBrandFare,gdsSourceKey,childResponsekey,isRefundable)
			SELECT t.airOneResponsekey, t.airOnePriceBase ,t.airOnePriceBaseSenior,t.airOnePriceTaxSenior, t.airOnePriceBaseChildren ,t.airOnePriceTaxChildren ,t.airOnePriceBaseInfant, t.airOnePriceTaxInfant,t.airOnePriceBaseYouth, t.airOnePriceTaxYouth, t.airOnePriceBaseTotal, t.airOnePriceTaxTotal,t.airOnePriceBaseDisplay, t.airOnePriceTaxDisplay,t.airSegmentFlightNumber,t.airSegmentMarketingAirlineCode,t.airsubRequestkey,t.airOnePriceTax ,t.airpriceTotal ,t.cabinclass  ,t.legConnections,t.airOnePriceBaseInfantWithSeat, t.airOnePriceTaxInfantWithSeat,  t.airLegBrandName,t.airLegBookingClasses,t.airResponseMultiBrandkey,t.isMultiBrandFare,t.gdsSourceKey,t.childResponsekey,t.isRefundable
			FROM #AllOneWayResponses t,
			(
			...
			--select * from #AllOneWayResponses where airSegmentFlightNumber = '99'
			--select * from @Temp_AllOneWayResponses where airSegmentFlightNumber = '99'
			...
			--select '5',* from #AllOneWayResponses where airSegmentMarketingAirlineCode like '%AA%'	
			...
			--select '@Temp_AllOneWayResponses',* from #AllOneWayResponses where airSegmentMarketingAirlineCode like '%AA%' and airLegBrandName = 'Main'	
			-- Temporarily Disabled because of Main Not Coming for AA
			--delete P 
			--FROM @Temp_AllOneWayResponses P
			--where P.isMultiBrandFare = 0

			--select '@Temp_AllOneWayResponses 1',* from #AllOneWayResponses where airSegmentMarketingAirlineCode like '%AA%'	and airLegBrandName = 'Main'
			select '1',* from @Temp_AllOneWayResponses where airSegmentFlightNumber = '99' 
			UPDATE @Temp_AllOneWayResponses   
			SET airOneResponsekey = S.airOneResponsekey 
			FROM #AllOneWayResponses S 
			LEFT OUTER JOIN @Temp_AllOneWayResponses Temp
			...
			select '2',* from @Temp_AllOneWayResponses where airSegmentFlightNumber = '99' 
			delete @Temp_AllOneWayResponses  
			FROM @Temp_AllOneWayResponses t,  
			(  
			SELECT min(airOnePriceBase + airOnePriceTax) AS minPrice,MIN(airOneIdent )  AS minIdent,   airSegmentFlightNumber,airSegmentMarketingAirlineCode,airLegBookingClasses,isRefundable    
	...
	BEGIN
		...
		INSERT INTO #AdditionalFares(airResponseKey,airresponseMultiBrandkey,TotalPriceToDisplay,airLegBrandName,TotalAllPaxPriceToDisplay,childresponsekey,isRefundable,airResBookDesigCode)
			SELECT t.airOneResponsekey,t.airResponseMultiBrandKey,(t.airOnePriceBaseDisplay + t.airOnePriceTaxDisplay),t.airLegBrandName,(t.airOnePriceBaseTotal + t.airOnePriceTaxTotal),t.childResponsekey,t.isRefundable,t.airLegBookingClasses
			From #AllOneWayResponses t 
		...
		BEGIN
			...
			INSERT INTO #AdditionalFares(airResponseKey,airresponseMultiBrandkey,TotalPriceToDisplay,airLegBrandName,TotalAllPaxPriceToDisplay,childresponsekey,isRefundable,airResBookDesigCode)
			SELECT TM.airOneResponsekey,TM.airResponseMultiBrandKey,(TM.airOnePriceBaseDisplay + TM.airOnePriceTaxDisplay),TM.airLegBrandName,(TM.airOnePriceBaseTotal + TM.airOnePriceTaxTotal),TM.childResponsekey,TM.isRefundable,tm.airLegBookingClasses
			From #AllOneWayResponses t
...
END
	...
	SELECT airresponseMultiBrandkey,airLegBrandName,TotalPriceToDisplay,childresponsekey,isRefundable,ReasonCode,airResBookDesigCode
	FROM #AdditionalFares A
