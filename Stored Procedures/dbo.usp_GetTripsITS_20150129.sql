...
-- =============================================                         
CREATE PROCEDURE [dbo].[usp_GetTripsITS_20150129]
...
BEGIN 
	...
	--declare @p11 int
	--set @p11=NULL
	-- SELECT  @pageName=N'CurrentTripName',@pageNo=1,@userkey=1614,@tripKey=NULL,@fromDate=N'1900-1-1',@toDate=N'9999-12-31',@traveler=NULL, @status=NULL,@companyKey=159,@TripCompType=N'Trip',@totalRecords=@p11,@siteKey=12,@sortField=N'Traveler',@sortDirection=N'Ascending'
...
--SELECT GETDATE() AS [0.4] 
	...
	BEGIN                            
		...
		FROM HotelContent.dbo.Hotels AS HT WITH (NOLOCK) 
			...
			INNER JOIN dbo.TripHotelResponse AS HR WITH (NOLOCK) ON SH.SupplierHotelId = HR.supplierHotelKey ON HT.HotelId = SH.HotelId AND SH.SupplierFamily = HR.supplierId 
			INNER JOIN dbo.Trip WITH (NOLOCK) ON HR.tripGUIDKey = dbo.Trip.tripPurchasedKey 
			INNER JOIN #tmpTrip tmp ON tmp.tripKey = Trip.tripKey 
		...
		FROM Trip WITH (NOLOCK) 
			INNER JOIN TripAirResponse resp WITH (NOLOCK) ON trip.tripPurchasedKey = resp.tripGUIDKey 
