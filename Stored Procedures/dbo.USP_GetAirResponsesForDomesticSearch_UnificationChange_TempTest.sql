...
CREATE PROCEDURE [dbo].[USP_GetAirResponsesForDomesticSearch_UnificationChange_TempTest]
	(  
			...
			BEGIN  
				...
				BEGIN  
					...
					BEGIN
						...
						) AND AIRLINES = (SELECT airlines FROM NormalizedAirResponsesMultiBrand WHERE airresponseMultiBrandkey = @SelectedResponseMultiBrandKey and airLegNumber =1 ) AND N.airsubrequestkey <> @airBundledRequest )	
...
--return
			...
			BEGIN    
				...
				BEGIN
					...
					--select 'multileg',* from @multiLegPrice
				END
			END   
			ELSE   
			BEGIN  
...
--return
	...
	BEGIN   
		...
		BEGIN
			...
			INSERT #AllOneWayResponses (airOneResponsekey,airOnePriceBase,airOnePriceBaseSenior ,airOnePriceTaxSenior, airOnePriceBaseChildren ,airOnePriceTaxChildren ,airOnePriceBaseInfant, airOnePriceTaxInfant,airOnePriceBaseYouth, airOnePriceTaxYouth, airOnePriceBaseTotal, airOnePriceTaxTotal,airOnePriceBaseDisplay, airOnePriceTaxDisplay,airSegmentFlightNumber,airSegmentMarketingAirlineCode,airsubRequestkey,airOnePriceTax ,airpriceTotal ,cabinclass  ,legConnections,airOnePriceBaseInfantWithSeat, airOnePriceTaxInfantWithSeat,  airLegBrandName,airLegBookingClasses,airResponseMultiBrandkey,isMultiBrandFare)  
			SELECT resp.airresponsekey, (airPriceBase   ),
			airPriceBaseSenior,airPriceTaxSenior,airPriceBaseChildren,airPriceTaxChildren,airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth,airPriceTaxYouth,AirPriceBaseTotal,AirPriceTaxTotal, airPriceBaseDisplay, airPriceTaxDisplay,flightNumber,airlines,resp .airSubRequestKey ,(airPriceTax ) ,airPriceBase + airPriceTax ,n.cabinclass    ,n.airLegConnections ,airPriceBaseInfantWithSeat,airPriceTaxInfantWithSeat,n.airLegBrandName,n.airLegBookingClasses,resp.airResponseMultiBrandKey,1
	...
	END 
		...
		BEGIN
			...
			--BEGIN
			--	--select 'Before Delete',* from @valid
			--	Delete P  
			--	FROM @valid  P  
			--	INNER JOIN AirResponse T  ON P.airResponsekey = T.airresponsekey 
			--	WHERE (airPriceBaseDisplay + airPriceTaxDisplay < @selectedLeg1RoundTripFare)
			--	--select 'After Delete',* from @valid
			--END
			...
			SELECT resp.AirResponsekey, (airPriceBase    ) ,airPriceBaseSenior,airPriceTaxSenior,airPriceBaseChildren,airPriceTaxChildren,airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth,airPriceTaxYouth,AirPriceBaseTotal,AirPriceTaxTotal,airPriceBaseDisplay, airPriceTaxDisplay,N.flightNumber ,N.airlines,resp.airSubRequestKey,airPriceTax ,airPriceBase + airPriceTax ,n.cabinclass ,n.airLegConnections ,airPriceBaseInfantWithSeat,airPriceTaxInfantWithSeat,n.airLegBrandName,n.airLegBookingClasses,resp.airresponseMultiBrandkey,1
			FROM AirResponseMultiBrand resp  WITH (NOLOCK) 
			INNER JOIN NormalizedAirResponsesMultiBrand n  WITH (NOLOCK) ON resp.airresponsekey = n.airresponsekey  
			INNER JOIN @valid valid ON resp.airResponseKey = valid .airResponsekey  
			AND ISNULL(resp.gdsSourceKey,2) =( Case WHEN @gdssourcekey = 0 THEN ISNULL(resp.gdsSourceKey,2) ELSE @gdssourcekey END ) AND N .airLegNumber = @airRequestTypeKey  
...
--return
	...
	BEGIN  
		IF(@isMultiBrand = 1)
		BEGIN
			INSERT #AllOneWayResponses (airOneResponsekey,airOnePriceBase,airOnePriceBaseSenior,airOnePriceTaxSenior,airOnePriceBaseChildren,airOnePriceTaxChildren,airOnePriceBaseInfant,airOnePriceTaxInfant,airOnePriceBaseYouth,airOnePriceTaxYouth,airOnePriceBaseTotal,airOnePriceTaxTotal,airOnePriceBaseDisplay, airOnePriceTaxDisplay,airSegmentFlightNumber,airSegmentMarketingAirlineCode,airsubRequestkey,airOnePriceTax ,airpriceTotal,otherLegprice,cabinclass  ,otherlegtax ,legConnections,airOnePriceBaseInfantWithSeat,airOnePriceTaxInfantWithSeat,airLegBrandName,airLegBookingClasses)  
			...
			(airPriceBaseInfantWithSeat + ISNULL(@airPriceInfantWithSeatForAnotherLeg,0)  ),(airPriceTaxInfantWithSeat + ISNULL(@airPriceTaxInfantWithSeatForAnotherLeg,0)),n.airLegBrandName,n.airLegBookingClasses
			...
			INSERT #AllOneWayResponses (airOneResponsekey,airOnePriceBase,airOnePriceBaseSenior,airOnePriceTaxSenior,airOnePriceBaseChildren,airOnePriceTaxChildren,airOnePriceBaseInfant,airOnePriceTaxInfant,airOnePriceBaseYouth,airOnePriceTaxYouth,airOnePriceBaseTotal,airOnePriceTaxTotal,airOnePriceBaseDisplay, airOnePriceTaxDisplay,airSegmentFlightNumber,airSegmentMarketingAirlineCode,airsubRequestkey,airOnePriceTax ,airpriceTotal,otherLegprice,cabinclass  ,otherlegtax ,legConnections,airOnePriceBaseInfantWithSeat,airOnePriceTaxInfantWithSeat,airLegBrandName, airLegBookingClasses,airResponseMultiBrandkey,isMultiBrandFare)  
			SELECT resp.airresponsekey, (airPriceBase + ISNULL(@airPriceForAnotherLeg,0)  ),
			(airPriceBaseSenior + ISNULL(@airPriceSeniorForAnotherLeg,0)  ),(airPriceTaxSenior + ISNULL(@airPriceTaxSeniorForAnotherLeg,0)),
			(airPriceBaseChildren + ISNULL(@airPriceChildrenForAnotherLeg,0)  ),(airPriceTaxChildren + ISNULL(@airPriceTaxChildrenForAnotherLeg,0)),
			(airPriceBaseInfant + ISNULL(@airPriceInfantForAnotherLeg,0)  ),(airPriceTaxInfant + ISNULL(@airPriceTaxInfantForAnotherLeg,0)),
			...
			flightNumber,airlines,resp .airSubRequestKey ,(airPriceTax + ISNULL(@airPriceTaxForAnotherLeg,0)),(airPriceBase + ISNULL(@airPriceForAnotherLeg,0)  )+(airPriceTax + ISNULL
			(@airPriceTaxForAnotherLeg,0)), case when @isTotalPriceSort = 0 THEN  ISNULL(@airPriceForAnotherLeg,0) else( isnull(@airPriceForAnotherLeg ,0) + isnull(@airPriceTaxForAnotherLeg,0) ) END ,N.cabinclass ,isnull(@airPriceTaxForAnotherLeg,0)  ,n.airLegConnections,
			(airPriceBaseInfantWithSeat + ISNULL(@airPriceInfantWithSeatForAnotherLeg,0)  ),(airPriceTaxInfantWithSeat + ISNULL(@airPriceTaxInfantWithSeatForAnotherLeg,0)),n.airLegBrandName,n.airLegBookingClasses,resp.airResponseMultiBrandKey,1
...
--return
		...
		BEGIN
			INSERT #AllOneWayResponses (airOneResponsekey,airOnePriceBase,airOnePriceBaseSenior ,airOnePriceTaxSenior, airOnePriceBaseChildren ,airOnePriceTaxChildren ,airOnePriceBaseInfant, airOnePriceTaxInfant,airOnePriceBaseYouth, airOnePriceTaxYouth, airOnePriceBaseTotal, airOnePriceTaxTotal,airOnePriceBaseDisplay, airOnePriceTaxDisplay,
			airSegmentFlightNumber,airSegmentMarketingAirlineCode,airsubRequestkey,airOnePriceTax ,airpriceTotal,otherLegprice ,cabinclass ,otherlegtax  ,legConnections,airOnePriceBaseInfantWithSeat, airOnePriceTaxInfantWithSeat,agentwareQueryID,agentwareItineraryID,airLegBrandName,airLegBookingClasses)  
			SELECT resp.airresponsekey, (airPriceBase + ISNULL(@airPriceForAnotherLeg,0)  ),
			...
			(airPriceTaxInfantWithSeat + ISNULL(@airPriceTaxInfantWithSeatForAnotherLeg,0)),agentwareQueryID,agentwareItineraryID,n.airLegBrandName,n.airLegBookingClasses
			From NormalizedAirResponses n WITH (NOLOCK) inner join AirResponse resp WITH (NOLOCK) on n.airresponsekey = resp.airResponseKey WHERE resp.airSubRequestKey = @airSubRequestKey   
			...
			flightNumber,airlines,resp .airSubRequestKey ,(airPriceTax + ISNULL(@airPriceTaxForAnotherLeg,0)),(airPriceBase + ISNULL(@airPriceForAnotherLeg,0)  )+(airPriceTax + ISNULL(@airPriceTaxForAnotherLeg,0)) , case when @isTotalPriceSort = 0 THEN  ISNULL(@airPriceForAnotherLeg,0) else( isnull(@airPriceForAnotherLeg ,0) + isnull(@airPriceTaxForAnotherLeg,0) ) END,n .cabinclass ,isnull(@airPriceTaxForAnotherLeg,0)  ,n.airLegConnections,
			(airPriceBaseInfantWithSeat + ISNULL(@airPriceInfantWithSeatForAnotherLeg,0)  ),
			(airPriceTaxInfantWithSeat + ISNULL(@airPriceTaxInfantWithSeatForAnotherLeg,0)),n.airLegBrandName,n.airLegBookingClasses,resp.airResponseMultiBrandKey,1
			From NormalizedAirResponsesMultiBrand n WITH (NOLOCK) inner join AirResponseMultiBrand resp WITH (NOLOCK) on n.airresponsekey = resp.airResponseKey WHERE resp.airSubRequestKey = @airSubRequestKey 
		END
		ELSE
		BEGIN
...
--select * from #AllOneWayResponses
	...
	BEGIN
		...
		BEGIN
			...
			WHERE airOneResponsekey in 
			(SELECT t.airOneResponsekey
			FROM #AllOneWayResponses t,  
			(  
			SELECT min(airOnePriceBase + airOnePriceTax) AS minPrice,MIN(airOneIdent )  AS minIdent,   airSegmentFlightNumber,airSegmentMarketingAirlineCode  ,cabinclass    
			FROM #AllOneWayResponses m  
		...
		BEGIN
			...
			Delete P  
			FROM #AllOneWayResponses  P  
			WHERE (airOnePriceBaseDisplay + airOnePriceTaxDisplay <= @selectedLeg1RoundTripFare)
			...
			--SELECT '3',* from @Temp_AllOneWayResponses
		...
		--PRINT 'LEg2'
		-- Leg 2 Filteration
		-- Step 1 ) First Remove All Responses whose Price is less than Selected Roundtrip Fare (Irrespective of OW or RT Selected)
			--DELETE FROM #AllOneWayResponses 
			--Where (airOnePriceBaseDisplay + airOnePriceTaxDisplay) < @selectedLeg1RoundTripFare
		-- Step 2) /****KEEP ALL DUPLICATE WITH LOWEST PRICE AND DELETE HIGHER PRICE In Primary Responses ****/
			--Inserting Response Keys in INSERT @tempResponseToRemove (airresponsekey ) 
			--INSERT @tempResponseToRemove (airresponsekey )
			--(SELECT t.airOneResponsekey
			--FROM #AllOneWayResponses t,  
			--(  
			...
			-- Now Deleting the previous Selected Response Keys
			--DELETE FROM #AllOneWayResponses  
			--WHERE airOneResponsekey in 
			--(SELECT t.airOneResponsekey
			--FROM #AllOneWayResponses t,  
			...
			--AND (airOnePriceBase + airOnePriceTax) >  minPrice  --AND airOneIdent >= minIdent 
			--AND t.isMultiBrandFare = 0)
		-- Step 3 /****KEEP ONLY ONE LOWEST PRICE ,REMOVE ALL OTHER WITH SAME PRICE In Primary Responses ****/ 	
			...
			--Step 4 Now Remove the Newly Inserted Response Keys in @tempResponseToRemove From #AllOneWayResponses
			--Delete P  
			--FROM #AllOneWayResponses  P  
			--INNER JOIN @tempResponseToRemove T  ON P.airOneResponsekey = T.airresponsekey 
		...
		--FROM #AllOneWayResponses t,  
		--(  
		--SELECT min(airOnePriceBase + airOnePriceTax) AS minPrice,MIN(airOneIdent )  AS minIdent,   airSegmentFlightNumber,airSegmentMarketingAirlineCode  ,cabinclass , airLegBookingClasses   
		--FROM #AllOneWayResponses m  
		--GROUP BY   airSegmentFlightNumber,airSegmentMarketingAirlineCode  ,cabinclass, airLegBookingClasses   
		--having count(1) > 1  
		--) AS derived  
		--WHERE t.airSegmentFlightNumber = derived.airSegmentFlightNumber AND t.airSegmentMarketingAirlineCode =derived .airSegmentMarketingAirlineCode AND isnull(t.cabinclass,'') =isnull(derived .cabinclass ,'') AND isnull(t.airLegBookingClasses,'') =isnull(derived .airLegBookingClasses ,'')
		--AND (airOnePriceBase + airOnePriceTax) >  minPrice  --AND airOneIdent >= minIdent 
		...
		--DELETE 
		--FROM #AllOneWayResponses 
		--WHERE  NOT IN ( airOneResponsekey  = (SELECT airOneResponsekey FROM 
		--		AND isMultiBrandFare = 1
...
--return
	...
	BEGIN
		If((@airRequestTypeKey = 1) OR (@airRequestTypeKey = 2 AND @isMultiBrandSelectedOnPreviousLeg = 0))
		BEGIN
			INSERT INTO #AdditionalFares(airResponseKey,airresponseMultiBrandkey,TotalPriceToDisplay,airLegBrandName)
			SELECT t.airOneResponsekey,t.airResponseMultiBrandKey,(t.airOnePriceBaseDisplay + t.airOnePriceTaxDisplay),t.airLegBrandName
			From #AllOneWayResponses t 
			WHERE t.isMultiBrandFare = 1 AND (t.airsubRequestkey = @airBundledRequest OR t.airsubRequestkey = @airSubRequestKey)
			--order by 
			
			--INSERT INTO #AdditionalFares(airResponseKey,airresponseMultiBrandkey,TotalPriceToDisplay,airLegBrandName)
			--SELECT t.airOneResponsekey,t.airResponseMultiBrandKey,
			--((t.airOnePriceBaseDisplay + ISNULL(@airPriceDisplayForAnotherLeg,0)  ) + (t.airOnePriceTaxDisplay + ISNULL(@airPriceTaxDisplayForAnotherLeg,0)  )),
			--t.airLegBrandName
			--From #AllOneWayResponses t 
			--WHERE t.isMultiBrandFare = 1 AND t.airsubRequestkey = @airSubRequestKey
		...
		BEGIN
			...
			INSERT INTO #AdditionalFares(airResponseKey,airresponseMultiBrandkey,TotalPriceToDisplay,airLegBrandName)
			SELECT TM.airOneResponsekey,TM.airResponseMultiBrandKey,(TM.airOnePriceBaseDisplay + TM.airOnePriceTaxDisplay),TM.airLegBrandName
			From #AllOneWayResponses t
			INNER JOIN @Temp_AllOneWayResponses TM
			ON t.airOneResponsekey = TM.airOneResponsekey 
			WHERE (TM.airsubRequestkey = @airBundledRequest OR TM.airsubRequestkey = @airSubRequestKey)
			...
			--select 'Normalize',* from #normalizedResultSet
			--return
		---- Additional Fares data insertion and deleted not used to be removed from  #normalizedResultSet
		--	INSERT  #normalizedResultSet (airresponsekey ,airPriceBase,noOFSTOPs ,noOfAirlines ,takeoffdate ,landingdate ,airlinecode ,gdssourcekey ,airpricetax ,airsubrequetkey ,otherlegprice,cabinclass,otherlegtax ,  airPriceBaseSenior, airPriceTaxSenior, airPriceBaseChildren,airPriceTaxChildren, airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth, airPriceTaxYouth, airPriceBaseTotal,airPriceTaxTotal, airPriceBaseDisplay,airPriceTaxDisplay,noOfOtherlegairlines ,otherlegAirlines,legConnections,actualNoOFStops,isSameAirlinesItin,airPriceBaseInfantWithSeat,airPriceTaxInfantWithSeat,airLegBrandName)  
		--	(  
		--	SELECT seg.airresponsekey,result.airOnePriceBaseDisplay ,CASE WHEN COUNT(seg.airresponsekey )-1  > 1 THEN (CASE WHEN @MaxNoofstops=2 THEN 2 ELSE 1 END) ELSE  COUNT(seg.airresponsekey )-1 END ,COUNT(distinct seg.airSegmentMarketingAirlineCode ),MIN(airSegmentDepartureDate ) ,MAX(airSegmentArrivalDate ), 
...
--select * from #normalizedResultSet
	...
	--IF(@isMultiBrand = 1)
	--BEGIN
	-- -- Preparing Upsell XML For Leg1 AND Removing Those Responses From Main Result Set
	--	IF(@airRequestTypeKey = 1)
	--	BEGIN
	--		INSERT INTO #AdditionalFares(airResponseKey,airresponseMultiBrandkey,TotalPriceToDisplay,airLegBrandName)
	--		SELECT respResult.airResponseKey,t.airResponseMultiBrandKey,(respResult.airPriceBaseDisplay + respResult.airPriceTaxDisplay)
	--		From #normalizedResultSet respResult 
	--		INNER JOIN #AllOneWayResponses t ON respResult.airresponsekey = t.airOneResponsekey
...
--SELECT 'Normalized',* from #normalizedResultSet
	...
	,airSuperSaverTax,airEconSaverTax,airFirstFlexTax,airCorporateTax,airEconFlexTax,airEconUpgradetax,resp.airPriceBaseInfantWithSeat,resp.airPriceTaxInfantWithSeat,agentwareQueryID,agentwareItineraryID,airsegmentPricingKey,airSegmentFareCategory,normalized.airLegBrandName,seg.airSegmentBrandName,seg.airSegmentBrandID,seg.airSegmentBaggage,seg.airSegmentMealCode,normalized.airresponseMultiBrandkey
