...
CREATE PROCEDURE [dbo].[USP_GetAirResponsesForDomesticSearch_20180920](  
--DECLARE
	...
	IF(@IsSameDayReturnOWAllowed = 1 AND @IsDomesticRegionTravel = 1 AND @isSameDaySearch = 1)
	BEGIN
  ...
  BEGIN
		SELECT @SelectedResponseArrivalAirport = airSegmentarrivalairport, @SelectedResponseArrivalAirportDate= airSegmentArrivalDate FROm #AirSegments WITH(NOLOCK) where airResponseKey = @SelectedResponseKey 
		AND segmentOrder = (select max(segmentOrder) from #AirSegments A WHERE A.airResponseKey = @SelectedResponseKey AND A.airLegNumber = 1)
