...
CREATE PROCEDURE [dbo].[USP_Report_TransactionCounter_WithRawPrice] ( 
		--DECLARE  
		 @pageNo As int = 1,      
		 @pageSize As int = 25,      
...
--	,@sortDirection=NULL,@TotalRecords=0,@siteKey=1
		...
		AdvPurch int,    
		agentStatus int,    
		RawOriginalUSDPrice FLOAT,
		RawOriginalEURPrice FLOAT, 
...
--SELECT GETDATE() AS [5]
		...
		Insert into #tripItinerary (tripkey, PassengerName, Booked, Travel, City, Vendor, VendorCode, AdvPurch, agentStatus, Price, appliedDiscount, 
			meetingCode, tripStatus, tripStatusName, fareType, bookingClass, farebasisCode, creditCardVendorCode, recordlocator, countryCode, ArrivalCountryCode, tripType
...
UPDATE #tripItinerary SET	RawOriginalUSDPrice = CASE WHEN currencyCode = 'USD' THEN Price ELSE 0 END
							,RawOriginalEURPrice = CASE WHEN CurrencyCode = 'EUR' THEN Price ELSE 0 END
