...
--exec [USP_GetCarResponsesForRequest] 106052,'','','','',9999999999.99 - 3E5CB5B7-C1A0-420A-BBB4-D2665BEF667C    
    
CREATE PROCEDURE [dbo].[USP_tmpGetCarResponsesForRequest]    
( @CarRequestKey int ,    
...
ISNULL((SELECT TOP 1 N.CarImage FROM CarContent.dbo.NewCarImages N WHERE N.VendorCode = carresponsedetail.carVendorKey  AND N.CarCode = carresponsedetail.carCategoryCode), 
ISNULL((SELECT TOP 1 N.CarImage FROM CarContent.dbo.NewCarImages N WHERE N.VendorCode = carresponsedetail.carVendorKey  AND LEFT(N.CarCode, 1) = LEFT(carresponsedetail.carCategoryCode, 1)), 'no-image.jpg')))) as ImageName, 
...
vw_sabreCarResponse.carResponseKey,CarContent.dbo.SabreVehicles.PsgrCapacity as passenger, CarContent.dbo.SabreVehicles.baggage as baggage,MIN (carresponsedetail.minRateTax )as minRateTax,CarContent.dbo.SippCodes.SippCodeTransmission,           
   CarContent.dbo.SippCodes.SippCodeAC,(CASE WHEN CarContent.dbo.SabreVehicles.ImageName IS NULL
			THEN 
				CASE WHEN (SELECT TOP 1 N.CarName FROM CarContent.dbo.NewCarImages N WHERE N.VendorCode = carresponsedetail.carVendorKey  AND N.CarCode = carresponsedetail.carCategoryCode) IS NULL 
					THEN CASE WHEN (SELECT TOP 1 N.CarName FROM CarContent.dbo.NewCarImages N WHERE N.VendorCode = carresponsedetail.carVendorKey  AND LEFT(N.CarCode, 1) = LEFT(carresponsedetail.carCategoryCode, 1)) IS NULL
						THEN CarContent.dbo.SabreVehicles.VehicleName
						ELSE (SELECT TOP 1 N.CarName FROM CarContent.dbo.NewCarImages N WHERE N.VendorCode = carresponsedetail.carVendorKey  AND LEFT(N.CarCode, 1) = LEFT(carresponsedetail.carCategoryCode, 1))
						END
				ELSE (SELECT TOP 1 N.CarName FROM CarContent.dbo.NewCarImages N WHERE N.VendorCode = carresponsedetail.carVendorKey  AND N.CarCode = carresponsedetail.carCategoryCode) 
				END
