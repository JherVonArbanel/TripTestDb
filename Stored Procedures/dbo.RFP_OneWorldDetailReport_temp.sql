...
CREATE procedure [dbo].[RFP_OneWorldDetailReport_temp]    
(    
 @userKey int,    
 @currency varchar(5),    
 @meetingCode int,    
...
*/
 ...
 Select trip.tripKey,trip.recordLocator , tar.CurrencyCodeKey ,trip.ISSUEDATE , tar.actualAirPrice , TTI.TotalFare ,CONVERT(DATE, trip.CreatedDate, 103) as bookedDate, CONVERT(DATE, trip.startDate, 103) as TravelDate,       
 ...
 --) as tripTotalBaseCost      
	 ...
	 (
	Case when trip.tripStatusKey = 12 then  isnull(TTI.TotalFare,0) / 
			 (CASE 
	 
			WHEN  @currency  = 'USD'
				then(
						Case when Tar.CurrencyCodeKey = 'USD' then 1 
						else Tman.dba.fn_xchange_rate(tar.CurrencyCodeKey,trip.ISSUEDATE ) 
						end
					) 
			else 
			(
						Case when Tar.CurrencyCodeKey  = @currency then 1 
						else Tman.dba.fn_xchange_rate(tar.CurrencyCodeKey,trip.ISSUEDATE ) 
						end
					)
					end )
	...
	else isnull(TAR.actualAirPrice,0) /
		 (CASE 
			...
			(
						Case when Tar.CurrencyCodeKey  = @currency then 1 
						else Tman.dba.fn_xchange_rate(@currency,trip.ISSUEDATE ) 
						end
					)
					end ) end)
  as tripTotalBaseCost ,Tman.dba.fn_xchange_rate(tar.CurrencyCodeKey,trip.ISSUEDATE ) 
 from trip       
