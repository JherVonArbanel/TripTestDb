...
CREATE PROCEDURE [dbo].[USP_Report_TransactionCounter_Backup]  
(      
 @pageNo As int = 1,    
 @pageSize As int = 25,    
...
update @tblSegmentDtl  set VendorCode ='Multiple',Vendor ='Mulitple Airlines' where VendorCode  like '%,%'--   
...
(case  when Pax.PassengerLocale <> 'USD' Then  vault.dbo.ufn_Currency_Converter_New(Pax.PassengerLocale, isnull(TAR.actualAirPrice ,0),T.CreatedDate) else   
...
,envUdidValue,cashUdidValue  ,(case  when Pax.PassengerLocale <> 'USD' Then  vault.dbo.ufn_Currency_Converter_New(Pax.PassengerLocale, isnull(TAR.actualAirPrice ,0),T.CreatedDate) else   
...
update T set totalPrice =  addCollectFare ,price = 0 ,appliedDiscount=0   from @tripItinerary T inner join TripTicketInfo ticket on T.tripKey =ticket.tripKey where t.tripStatus = 12 and isExchanged =1
