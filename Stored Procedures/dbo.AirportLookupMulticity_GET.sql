...
CREATE PROCEDURE [dbo].[AirportLookupMulticity_GET]
( 
...
BEGIN    
	...
	BEGIN      
		...
		BEGIN 
			...
			SELECT AirportName, CityName, StateCode, CountryCode, AirportCode, AirPriority     
			FROM AirportLookup     
			WHERE AirportCode <> @prefixText and (CityName LIKE (@prefixText +'%')) 
				AND CityCode = @prefixText AND AirStatus = 0 
	...
	BEGIN 
		...
		BEGIN    
			...
			SELECT CountryCode FROM AirportLookup 
			WHERE (AirportName LIKE ('%' + @prefixText + '%') OR CityName LIKE ('%'+ @prefixText +'%')) AND AirStatus = 0  
			ORDER BY AirPriority ASC 
