 ...
 **********************************/              
CREATE PROCEDURE [dbo].[USP_GetHotelResponsesForMultipleRequest2_MarketPlace]    
(     
--declare     
 @hotelRequestKey  INT,    
...
BEGIN       
 ...
 /*Start  Code added by SunilK on 03-01-2019 for SLA report*/
 DECLARE @TripRequestKey BIGINT=(SELECT TriprequestKey FROM Trip..TripRequest_hotel WHERE hotelRequestKey=@hotelRequestKey)
 DECLARE @StartDate_SP DATETIME=GETDATE()
 DECLARE @EndDate_SP DATETIME=NULL
 /*End*/
...
--END    
  ...
  DROP TABLE #TmpHotelResponse
	...
	SET @EndDate_SP=GETDATE()

	EXEC [Log].[dbo].USP_InsertAuditPerformanceLogs 
	 @TripRequestkey =@TripRequestkey
	,@SLACategory ='Shopping'	
	,@SLASubCategory ='Hotel'
	,@Page ='HotelList'	
	,@APIName='USP_GetHotelResponsesForMultipleRequest2_MarketPlace'	
