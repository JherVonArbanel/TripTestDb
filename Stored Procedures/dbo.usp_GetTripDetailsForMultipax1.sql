...
/*Exec usp_GetTripDetailsForMultipax1 6301,0*/      
CREATE PROCEDURE [dbo].[usp_GetTripDetailsForMultipax1]              
(          
 @tripID   int   ,            
 @tripRequestID Int = 0            
)          
...
--,TPR.CreatedOn            
 ...
 from TripAirSegments  segments               
  ...
  inner join Trip t on AirResp.tripGUIDKey  = t.tripPurchasedKey             
Inner join @tblTrip tt on tt.tripKey = t.tripKey             
...
TAVP.Active,      
TAVP.AirsegmentKey      
 , TPI.PassengerFirstName,TPI.PassengerLastName, TPI.PassengerLocale,TPI.PassengerEmailID ,TPI.tripKey,TPI.TripPassengerInfoKey  from TripPassengerInfo TPI             
...
Select * from vw_TripCruiseResponse cruise    
 inner join trip t on cruise.tripGuidkey =t.tripPurchasedKey             
 Inner join @tblTrip tt on tt.tripKey = t.tripKey             
    Order by t.tripKey     
