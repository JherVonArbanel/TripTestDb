...
CREATE PROCEDURE [dbo].[USP_GetAirResponsesForDomesticSearch_1]
	(  
	...
	DECLARE @legTwoLowestFare AS FLOAT
    ...
    SELECT * INTO #AirResponse FROM AirResponse WHERE airSubRequestKey IN (SELECT airSubRequestKey FROM AirSubRequest WHERE airRequestKey = @airRequestKey )
    SELECT * INTO #Airsegments FROM AirSegments WHERE airResponseKey in (SELECT airResponseKey FROM #AirResponse)
    SELECT * INTO #NormalizedAirResponses FROM NormalizedAirResponses WHERE airsubrequestkey IN (SELECT airSubRequestKey FROM AirSubRequest WHERE airRequestKey = @airRequestKey )
    
    IF (@isMultiBrand=1)
    BEGIN
		SELECT * INTO #AirResponseMultiBrand FROM AirResponseMultiBrand WHERE airSubRequestKey IN (SELECT airSubRequestKey FROM AirSubRequest WHERE airRequestKey = @airRequestKey )
		...
		SELECT * INTO #NormalizedAirResponsesMultiBrand FROM NormalizedAirResponsesMultiBrand WHERE airsubrequestkey IN (SELECT airSubRequestKey FROM AirSubRequest WHERE airRequestKey = @airRequestKey )
    END
	...
	BEGIN 
		...
		begin
			SET @airAgentWareWNRequest = (SELECT TOP 1 AirSubRequestKey FROM AirSubRequest WITH(NOLOCK) WHERE airRequestKey = @airRequestKey AND airSubRequestLegIndex = 2 AND groupKey = 4)   
		end
	END 
	ELSE 
	BEGIN 
	...
	BEGIN
		...
		BEGIN 
			IF(@airRequestTypeKey =1)
			BEGIN
				IF EXISTS(SELECT 1 FROM AirSubRequest WITH(NOLOCK) WHERE airRequestKey = @airRequestKey AND airSubRequestLegIndex = 1  AND groupKey = 5 )
				SET @airMultiCabinRequest = (SELECT TOP 1 AirSubRequestKey FROM AirSubRequest WITH(NOLOCK) WHERE airRequestKey = @airRequestKey AND airSubRequestLegIndex = 1  AND groupKey = 5)
			END
			ELSE
			BEGIN
				IF EXISTS(SELECT 1 FROM AirSubRequest WITH(NOLOCK) WHERE airRequestKey = @airRequestKey AND airSubRequestLegIndex = 2  AND groupKey = 5 )
				SET @airMultiCabinRequest = (SELECT TOP 1 AirSubRequestKey FROM AirSubRequest WITH(NOLOCK) WHERE airRequestKey = @airRequestKey AND airSubRequestLegIndex = 2 AND groupKey = 5)   
			END
		END 
		ELSE 
		BEGIN 
	...
	SET @airResponseKey = (SELECT Top 1 airresponsekey FROM #AirResponse r WITH (NOLOCK)  WHERE  (r.airSubRequestKey = @airBundledRequest or  r.airSubRequestKey = @airSubRequestKey or r.airSubRequestKey = @airMultiCabinRequest))
	...
	BEGIN 
		...
		INNER JOIN #AirResponse resp WITH(NOLOCK) ON s.airResponseKey =resp.airResponseKey   
		INNER JOIN AirSubRequest subReq WITH(NOLOCK) ON resp.airSubRequestKey =subReq.airSubRequestKey  
		WHERE airRequestKey = @airRequestKey and airSegmentOperatingAirlineCode in (SELECT * FROM @tblLoggedinAirlines) 
		)
	...
	-- Remove oneway reponses of southwest(NW) airline, if its not ONEWAY  ***Bug:7431 Display only RT repponses for southwest
		...
		WHERE airRequestKey = @airRequestKey and airSubRequestLegIndex <> -1 and resp.gdsSourceKey = 2 and seg.airSegmentMarketingAirlineCode = 'WN')
	END
	...
	END
	select A.* from @tempResponseToRemove T inner join AirResponse A on T.airresponsekey = A.airResponseKey inner join AirSegments S on A.airResponseKey = S.airResponseKey
and s.airSegmentMarketingAirlineCode = 'WN'
	...
	BEGIN
		...
		WHERE airRequestKey = @airRequestKey AND airLegNumber =@airRequestTypeKey AND segmentOrder = 1 
		...
		BEGIN
			...
			BEGIN
					...
					BEGIN
						...
						INNER JOIN #AirResponse resp WITH(NOLOCK) ON s.airResponseKey =resp.airResponseKey   
						INNER JOIN AirSubRequest subReq WITH(NOLOCK) ON resp.airSubRequestKey =subReq.airSubRequestKey  
						WHERE airRequestKey = @airRequestKey and Upper(airsegmentCabin) NOT IN (SELECT UPPER(cabin) FROM @tblCabinGroup))
	...
	BEGIN   
			IF ( @airLines = '' or @airLines = 'Multiple Airlines' )   
				BEGIN   
					...
					from #AirResponse  Resp WITH (NOLOCK)
					inner join AirSubRequest subReq WITH(NOLOCK) on resp.airSubRequestKey = subReq.airSubRequestKey   
					WHERE airRequestKey  = @airRequestKey  and airSubRequestLegIndex > 1 and resp.gdsSourceKey = 12  
					and airResponseKey not in ( select airresponsekey  from @tempResponseToRemove )   
					--order by Resp. AirSubRequestkey , (airPriceBase + airPriceTax)asc	
					) 
				END   
				...
				SET @airPriceForAnotherLeg = (SELECT SUM(Airpricebase) FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceTaxForAnotherLeg = (SELECT SUM(airPriceTax) FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceSeniorForAnotherLeg = (SELECT SUM(AirpricebaseSenior) FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceTaxSeniorForAnotherLeg = (SELECT SUM(airPriceTaxSenior) FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceChildrenForAnotherLeg = (SELECT SUM(AirpricebaseChildren) FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceTaxChildrenForAnotherLeg = (SELECT SUM(airPriceTaxChildren) FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceInfantForAnotherLeg = (SELECT SUM(AirpricebaseInfant) FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceTaxInfantForAnotherLeg = (SELECT SUM(airPriceTaxInfant) FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceYouthForAnotherLeg = (SELECT SUM(AirpricebaseYouth) FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceTaxYouthForAnotherLeg = (SELECT SUM(airPriceTaxYouth) FROM @multiLegPrice where gdsSourceKey = 2 )
				SET @airPriceTotalForAnotherLeg = (SELECT SUM(AirPriceBaseTotal) FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceTaxTotalForAnotherLeg = (SELECT SUM(AirPriceTaxTotal) FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceDisplayForAnotherLeg = (SELECT SUM(AirpricebaseDisplay) FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceTaxDisplayForAnotherLeg = (SELECT SUM(airPriceTaxDisplay) FROM @multiLegPrice where gdsSourceKey = 2 )
				SET @airPriceInfantWithSeatForAnotherLeg = (SELECT SUM(airPriceBaseInfantWithSeat) FROM @multiLegPrice where gdsSourceKey = 2 )  
				SET @airPriceTaxInfantWithSeatForAnotherLeg = (SELECT SUM(airPriceTaxInfantWithSeat) FROM @multiLegPrice where gdsSourceKey = 2 )
			...
			BEGIN  
				...
				BEGIN 
					...
					BEGIN
						...
						AND N.airsubrequestkey <> @airBundledRequest )	
						...
						--select '@selectedLeg1RoundTripFare',@selectedLeg1RoundTripFare
						--SELECT '@selectedLeg1RoundTripFare', * from @selectedLeg1RoundTripFare
					END
 				END   
...
--return
	...
	BEGIN   
		...
		BEGIN
			...
			--INSERT #AllOneWayResponses (airOneResponsekey,airOnePriceBase,airOnePriceBaseSenior ,airOnePriceTaxSenior, airOnePriceBaseChildren ,airOnePriceTaxChildren ,airOnePriceBaseInfant, airOnePriceTaxInfant,airOnePriceBaseYouth, airOnePriceTaxYouth, airOnePriceBaseTotal, airOnePriceTaxTotal,airOnePriceBaseDisplay, airOnePriceTaxDisplay,airSegmentFlightNumber,airSegmentMarketingAirlineCode,airsubRequestkey,airOnePriceTax ,airpriceTotal ,cabinclass  ,legConnections,airOnePriceBaseInfantWithSeat, airOnePriceTaxInfantWithSeat,  airLegBrandName,airLegBookingClasses,airResponseMultiBrandkey,isMultiBrandFare,gdsSourceKey,childResponsekey)  
			--SELECT resp.airresponsekey, (airPriceBase   ),
			--airPriceBaseSenior,airPriceTaxSenior,airPriceBaseChildren,airPriceTaxChildren,airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth,airPriceTaxYouth,AirPriceBaseTotal,AirPriceTaxTotal, airPriceBaseDisplay, airPriceTaxDisplay,flightNumber,airlines,resp .airSubRequestKey ,(airPriceTax ) ,airPriceBase + airPriceTax ,n.cabinclass    ,n.airLegConnections ,airPriceBaseInfantWithSeat,airPriceTaxInfantWithSeat,n.airLegBrandName,n.airLegBookingClasses,resp.airResponseMultiBrandKey,1,ISNULL(resp.gdsSourceKey,2) ,resp.airresponsekey
		...
		BEGIN
			...
			SELECT resp.airresponsekey, (airPriceBase + ISNULL((SELECT SUM(Airpricebase) FROM @multiLegPrice where gdsSourceKey = 12),0)  ),
			(airPriceBaseSenior + ISNULL((SELECT SUM(AirpricebaseSenior) FROM @multiLegPrice where gdsSourceKey = 12),0) ),
			(airPriceTaxSenior + ISNULL((SELECT SUM(airPriceTaxSenior) FROM @multiLegPrice where gdsSourceKey = 12),0)),
			(airPriceBaseChildren + ISNULL((SELECT SUM(AirpricebaseChildren) FROM @multiLegPrice where gdsSourceKey = 12),0) ),
			(airPriceTaxChildren + ISNULL((SELECT SUM(airPriceTaxChildren) FROM @multiLegPrice where gdsSourceKey = 12),0)),
			(airPriceBaseInfant + ISNULL((SELECT SUM(AirpricebaseInfant) FROM @multiLegPrice where gdsSourceKey = 12),0) ),
			(airPriceTaxInfant + ISNULL((SELECT SUM(airPriceTaxInfant) FROM @multiLegPrice where gdsSourceKey = 12),0)),
			(airPriceBaseYouth + ISNULL((SELECT SUM(AirpricebaseYouth) FROM @multiLegPrice where gdsSourceKey = 12),0) ),
			(airPriceTaxYouth + ISNULL((SELECT SUM(airPriceTaxYouth) FROM @multiLegPrice where gdsSourceKey = 12),0)),
			(AirPriceBaseTotal + ISNULL((SELECT SUM(AirPriceBaseTotal) FROM @multiLegPrice where gdsSourceKey = 12),0) ),
			(AirPriceTaxTotal + ISNULL((SELECT SUM(AirPriceTaxTotal) FROM @multiLegPrice where gdsSourceKey = 12),0)),
			(airPriceBaseDisplay + ISNULL((SELECT SUM(AirpricebaseDisplay) FROM @multiLegPrice where gdsSourceKey = 12),0) ),
			(airPriceTaxDisplay + ISNULL((SELECT SUM(airPriceTaxDisplay) FROM @multiLegPrice where gdsSourceKey = 12),0)),
			flightNumber,airlines,resp .airSubRequestKey ,
			(airPriceTax + ISNULL(( SELECT SUM(airpriceTax ) FROM @multiLegPrice where gdsSourceKey = 12),0)),
			(airPriceBase + ISNULL((SELECT SUM(Airpricebase) FROM @multiLegPrice where gdsSourceKey = 12),0)  )+(airPriceTax + ISNULL(( SELECT SUM(airpriceTax ) FROM @multiLegPrice where gdsSourceKey = 12),0)), 
			case when @isTotalPriceSort = 0 THEN  ISNULL((SELECT SUM(Airpricebase) FROM @multiLegPrice where gdsSourceKey = 12),0) else( isnull((SELECT SUM(Airpricebase) FROM @multiLegPrice where gdsSourceKey = 12) ,0) + isnull(( SELECT SUM(airpriceTax ) FROM @multiLegPrice where gdsSourceKey = 12),0) ) END ,
			N.cabinclass ,isnull(( SELECT SUM(airpriceTax ) FROM @multiLegPrice where gdsSourceKey = 12),0)  ,
			n.airLegConnections,
			(airPriceBaseInfantWithSeat + ISNULL((SELECT SUM(airPriceBaseInfantWithSeat) FROM @multiLegPrice where gdsSourceKey = 12),0)  ),
			(airPriceTaxInfantWithSeat + ISNULL((SELECT SUM(airPriceTaxInfantWithSeat) FROM @multiLegPrice where gdsSourceKey = 12),0)),
			...
			BEGIN
				...
				SELECT resp.airresponsekey, (resp.airPriceBase + ISNULL((SELECT SUM(Airpricebase) FROM @multiLegPrice where gdsSourceKey = 12),0)  ),
				(resp.airPriceBaseSenior + ISNULL((SELECT SUM(AirpricebaseSenior) FROM @multiLegPrice where gdsSourceKey = 12),0)  ),
				(resp.airPriceTaxSenior + ISNULL((SELECT SUM(airPriceTaxSenior) FROM @multiLegPrice where gdsSourceKey = 12),0)),
				(resp.airPriceBaseChildren + ISNULL((SELECT SUM(AirpricebaseChildren) FROM @multiLegPrice where gdsSourceKey = 12),0)  ),
				(resp.airPriceTaxChildren + ISNULL((SELECT SUM(airPriceTaxChildren) FROM @multiLegPrice where gdsSourceKey = 12),0)),
				(resp.airPriceBaseInfant + ISNULL((SELECT SUM(AirpricebaseInfant) FROM @multiLegPrice where gdsSourceKey = 12),0)  ),
				(resp.airPriceTaxInfant + ISNULL((SELECT SUM(airPriceTaxInfant) FROM @multiLegPrice where gdsSourceKey = 12),0)),
				(resp.airPriceBaseYouth + ISNULL((SELECT SUM(AirpricebaseYouth) FROM @multiLegPrice where gdsSourceKey = 12),0)  ),
				(resp.airPriceTaxYouth + ISNULL((SELECT SUM(airPriceTaxYouth) FROM @multiLegPrice where gdsSourceKey = 12),0)),
				(resp.AirPriceBaseTotal + ISNULL((SELECT SUM(AirPriceBaseTotal) FROM @multiLegPrice where gdsSourceKey = 12),0)  ),
				(resp.AirPriceTaxTotal + ISNULL((SELECT SUM(AirPriceTaxTotal) FROM @multiLegPrice where gdsSourceKey = 12),0)),
				(resp.airPriceBaseDisplay + ISNULL((SELECT SUM(AirpricebaseDisplay) FROM @multiLegPrice where gdsSourceKey = 12),0)  ),
				(resp.airPriceTaxDisplay + ISNULL((SELECT SUM(airPriceTaxDisplay) FROM @multiLegPrice where gdsSourceKey = 12),0)),
				flightNumber,airlines,resp .airSubRequestKey ,
				(resp.airPriceTax + ISNULL((SELECT SUM(airpriceTax ) FROM @multiLegPrice where gdsSourceKey = 12),0)),
				(resp.airPriceBase + ISNULL((SELECT SUM(Airpricebase) FROM @multiLegPrice where gdsSourceKey = 12),0)  )+(resp.airPriceTax + ISNULL	((SELECT SUM(airpriceTax ) FROM @multiLegPrice where gdsSourceKey = 12),0)), 
				case when @isTotalPriceSort = 0 THEN  ISNULL((SELECT SUM(Airpricebase) FROM @multiLegPrice where gdsSourceKey = 12),0) 
					else( isnull((SELECT SUM(Airpricebase) FROM @multiLegPrice where gdsSourceKey = 12) ,0) + isnull(( SELECT SUM(airpriceTax ) FROM @multiLegPrice where gdsSourceKey = 12),0) ) END ,
				N.cabinclass ,	isnull(( SELECT SUM(airpriceTax ) FROM @multiLegPrice where gdsSourceKey = 12),0)  ,
				n.airLegConnections,
				(resp.airPriceBaseInfantWithSeat + ISNULL((SELECT SUM(airPriceBaseInfantWithSeat) FROM @multiLegPrice where gdsSourceKey = 12),0)  ),
				(resp.airPriceTaxInfantWithSeat + ISNULL((SELECT SUM(airPriceTaxInfantWithSeat) FROM @multiLegPrice where gdsSourceKey = 12),0)),
	...
	END 
		...
		BEGIN
			...
			--INNER JOIN @valid valid ON resp.airResponseKey = valid .airResponsekey  
			--AND ISNULL(resp.gdsSourceKey,2) =( Case WHEN @gdssourcekey = 0 THEN ISNULL(resp.gdsSourceKey,2) ELSE @gdssourcekey END ) AND N .airLegNumber = @airRequestTypeKey  
			--select '2',* from #AllOneWayResponses
		END
		ELSE
		BEGIN 
			INSERT #AllOneWayResponses (airOneResponsekey,airOnePriceBase,airOnePriceBaseSenior ,airOnePriceTaxSenior, airOnePriceBaseChildren ,airOnePriceTaxChildren ,airOnePriceBaseInfant, airOnePriceTaxInfant,airOnePriceBaseYouth, airOnePriceTaxYouth, airOnePriceBaseTotal, airOnePriceTaxTotal,airOnePriceBaseDisplay, airOnePriceTaxDisplay,airSegmentFlightNumber,airSegmentMarketingAirlineCode,airsubRequestkey,airOnePriceTax ,airpriceTotal ,cabinclass ,legConnections ,airOnePriceBaseInfantWithSeat, airOnePriceTaxInfantWithSeat,airLegBrandName)  
...
--return
	...
	--select 'All',* from #AllOneWayResponses
	 --return
	...
	BEGIN
		...
		BEGIN
			...
			-- need to see whether one unique cabin needs to be pick for one particular flight number, airline code
			...
			FROM #AllOneWayResponses t,  
			(  
			SELECT min(airOnePriceBase + airOnePriceTax) AS minPrice,MIN(airOneIdent )  AS minIdent,   airSegmentFlightNumber,airSegmentMarketingAirlineCode,airLegBookingClasses   
			FROM #AllOneWayResponses m 
			GROUP BY   airSegmentFlightNumber,airSegmentMarketingAirlineCode,airLegBookingClasses  
			having count(1) > 1  
			) AS derived  
			...
			FROM #AllOneWayResponses m  
			where m.isMultiBrandFare = 0 AND (m.airsubRequestkey = @airBundledRequest OR m.airsubRequestkey = @airPublishedFareRequest OR m.airsubRequestkey = @airSubRequestKey OR m.airsubRequestkey = @airMultiCabinRequest)
			) AS derived 
			WHERE t.airSegmentFlightNumber = derived.airSegmentFlightNumber AND t.airSegmentMarketingAirlineCode =derived .airSegmentMarketingAirlineCode
			...
			WHEN airLegBrandName = 'Business Select' THEN 'Main'
		...
		BEGIN 
			...
			WHERE (S.airsubRequestkey = @airBundledRequest OR S.airsubRequestkey = @airPublishedFareRequest OR S.airsubRequestkey = @airSubRequestKey OR S.airsubRequestkey = @airMultiCabinRequest)
			...
			WHERE t.airSegmentFlightNumber = derived.airSegmentFlightNumber AND t.airSegmentMarketingAirlineCode =derived .airSegmentMarketingAirlineCode and t.airSegmentMarketingAirlineCode =derived .airSegmentMarketingAirlineCode AND t.airLegBookingClasses =derived.airLegBookingClasses
			AND (airOnePriceBase + airOnePriceTax) >  minPrice
...
--return
	UPDATE  A  SET otherlegAirlines = case when a.gdsSourceKey = 12 then 'WN' else S.otherLegAirlines end ,--S.otherLegAirlines  
			noOfOtherlegairlines = otherlegsAirlinesCount  FROM #AllOneWayResponses A inner join @secondLegDetails S on A.airOneResponsekey = S.responsekey and airsubRequestkey = @airBundledRequest 
	...
	BEGIN
		...
		BEGIN 
			...
			From #AllOneWayResponses t 
			WHERE t.isMultiBrandFare = 1 AND (t.airsubRequestkey = @airBundledRequest OR t.airsubRequestkey = @airSubRequestKey OR t.airsubRequestkey = @airAgentWareWNRequest OR t.airsubRequestkey = @airMultiCabinRequest)
		...
		BEGIN
			...
			WHERE (TM.airsubRequestkey = @airBundledRequest OR TM.airsubRequestkey = @airSubRequestKey OR TM.airsubRequestkey = @airPublishedFareRequest OR TM.airsubRequestkey = @airAgentWareWNRequest OR TM.airsubRequestkey = @airMultiCabinRequest)
	...
	BEGIN
		...
		BEGIN
			...
			WHERE airLegNumber = @airRequestTypeKey AND (ARMB.airSubRequestKey = @airBundledRequest OR ARMB.airSubRequestKey = @airSubRequestKey OR ARMB.airSubRequestKey = @airPublishedFareRequest OR ARMB.airSubRequestKey = @airMultiCabinRequest)
	...
	SELECT airresponseMultiBrandkey,airLegBrandName,TotalPriceToDisplay,childresponsekey,isRefundable, 
			(SELECT distinct segmentOrder, airSegmentKey, airSegmentSeatRemaining FROM #AirSegmentsMultiBrand 
					where airresponseMultiBrandkey = A.airresponseMultiBrandkey FOR XML PATH('Segment'), TYPE) as Segments
