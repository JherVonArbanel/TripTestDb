...
SET ANSI_NULLS ON
GO
--exec [usp_Auction_GetTripDetailsById_cybage] 4141  -- 4944
--exec [usp_Auction_GetTripDetailsById] 4141  -- 4944
CREATE PROC [dbo].[usp_Auction_GetTripDetailsById]
(
-- Declare
	@TripKey int
) AS 
...
------------ AIR INFO ------------
	SELECT  seg.airSegmentDepartureAirport 'Origin', seg.airSegmentArrivalAirport 'Destination'
		...
		, seg.airSegmentMarketingAirlineCode 'AirlineCode', resp.actualAirPrice BaseCost
		--, NULL Disc_Price
...
--SELECT airSegmentDepartureAirport 'Origin',airSegmentArrivalAirport 'Destination',departuredate 'DepatureDateTime',BookingType = ''
--		,NumberOfPassengers = 1,AdultCount = 1,ChildCount = 0,VendorCode 'AirlineCode' ,BaseCost =basecost,Disc_Price = (basecost-0.10*basecost)
--		,Tax = tax FROM	 TripAirResponse --vw_TripDetails WHERE	 tripGUIDKey = @tripSavedKey --And [Type] = 'air'
...
------------ CAR INFO ------------
	...
	--	,PickupCity,PickupState,PickupCountry,DropoffAirport = dropoffCity,DropoffCity, DropoffState, DropoffCountry, OneWayRental = ''
	--	,PickUpDate  -- dbo.TripCarResponse.PickUpdate, DropOutDate, CarCategoryCode,PrefferedVendorCode = '',CarType = ''
	--	,CarTransmission = SippCodeTransmission,DemandStatus = '', ModifiedBy  = userKey, ModifiedDate = GETDATE()
