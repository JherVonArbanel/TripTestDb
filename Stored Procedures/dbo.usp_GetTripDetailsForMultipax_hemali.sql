...
GO
		...
		CREATE PROCEDURE [dbo].[usp_GetTripDetailsForMultipax_hemali]                          
		(                      
			@tripID   int   ,                        
			@tripRequestID Int = 0                        
		)                      
...
--SELECT @tripId=30989, @tripRequestId=735461
	...
	SET @LogLastDateTime = GETDATE(); 
		...
		BEGIN                        
		 INSERT Into @tblTrip                        
		 Select  @tripID,   tripRequestKey, tripStatusKey, tripPurchasedKey  from Trip WITH(NOLOCK) where tripKey  = @tripID 
		 --ORDER BY tripKey DESC
		END                        
		ELSE                         
		BEGIN                        
		 INSERT Into @tblTrip                        
		 Select  tripKey ,  tripRequestKey, tripStatusKey, tripPurchasedKey  from Trip WITH(NOLOCK) where tripRequestKey  = @tripRequestID 
		 --ORDER BY tripKey DESC
		END                        
...
WHERE     TR.SupplierId <> 'Sabre' AND TR.carVendorKey = 'ZT'
		 ...
		 Inner join @tblTrip tt ON tt.tripKey  = Td.tripKey       
		 ...
		 Inner join @tblTrip tt ON td.tripguidkey = tt.tripPurchasedKey   
		 Inner Join vault.dbo.GDSSourceLookup GL WITH(NOLOCK) On GL.GDSName = TD.SupplierId                        
		 ORDER BY tT.tripKey                
