...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[AirportLookup_GET_PAR_var]
(
...
BEGIN
	...
	BEGIN		
		...
		BEGIN
			...
			FROM AirportLookup 
			WHERE (CityName LIKE (@prefixText + '%') OR AirportCode = @prefixText) AND AirStatus = 0 
	...
	BEGIN
		...
		BEGIN
			SELECT CountryCode 
			FROM AirportLookup 
			WHERE  AirportCode = @prefixText or (AirportName LIKE ('%'+ @prefixText +'%') OR CityName LIKE ('%'+ @prefixText +'%')) and AirStatus =0  order by AirPriority asc	
		END
		ELSE
		BEGIN
