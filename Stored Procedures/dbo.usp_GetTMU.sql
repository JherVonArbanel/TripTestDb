...
--exec usp_GetTMU @cityCode=NULL,@siteKey=5,@cityType=N'To',@resultCount=20,@tripComponentType=0,@page=1,@tripKey=0,@startDate=NULL,@theme=0,@sortfield=N'',@friendOption=N'',@hotelClass=0,@loggedInUserKey=560799,@FromIndex=1,@ToIndex=20,@CarClass=N'',@AirClass=N'',@AirType=N'',@HashTag=N'',@TrendingPeople=N'0',@IsCrowdEvent=0,@IsSliderFilterApplied=1,@MinFollowed=0,@MaxFollowed=50,@MinSavings =0,@maxSavings=0,@MinBestPrice=0,@MaxBestPrice=0,@IsFilterApplied=0, @IsDiscoverFilter =1 , @CrowdMyFollowing =0 , @CrowdLeadersOnly= 1, @CrowdSize = 0, @CrowdPrice = 0, @CrowdDiscounts=0, @StateCode = 'MIA'


-- EXEC Trip..usp_GetTMU @cityCode=NULL,@siteKey=5,@cityType=N'To',@resultCount=20,@tripComponentType=0,@page=16,@tripKey=0,@startDate=NULL,@theme=0,@sortfield=N'',@friendOption=N'',@hotelClass=0,@loggedInUserKey=560799,@FromIndex=1,@ToIndex=20,@CarClass=N'',@AirClass=N'',@AirType=N'',@HashTag='#hotel,#flight,#car'
--EXEC Trip..usp_GetTMU @cityCode=NULL,@siteKey=5,@cityType=N'To',@resultCount=100,@tripComponentType=0,@page=16,@tripKey=0,@startDate=NULL,@theme=0,@sortfield=N'',@friendOption=N'',@hotelClass=0,@loggedInUserKey=560799,@FromIndex=1,@ToIndex=100,@CarClass=N'',@AirClass=N'',@AirType=N'',@HashTag=''

--EXEC Trip..usp_GetTMU @cityCode=NULL,@siteKey=5,@cityType=N'To',@resultCount=20,@tripComponentType=0,@page=16,@tripKey=0,@startDate=NULL,@theme=0,@sortfield=N'',@friendOption=N'',@hotelClass=0,@loggedInUserKey=560799,@FromIndex=1,@ToIndex=20,@CarClass=N'',@AirClass=N'',@AirType=N'',@HashTag=''
            
CREATE PROC [dbo].[usp_GetTMU] (            
@cityCode varchar(20) = NULL ,                                
 ...
 @IsDiscoverFilter bit = 0, --Mobile Filter
 @CrowdMyFollowing bit = 0,
 @CrowdLeadersOnly bit = 0,
 @CrowdSize int = 0 ,
 @CrowdPrice int = 0,
 @CrowdDiscounts int = 0,
 @StateCode varchar(1000) = null ,
 @parentLoggedInUser BIGINT = 0
)            
AS             
BEGIN            
 SET NOCOUNT ON;            
 ...
 -- ##### TABLE DELCARATION ##### --             
 IF OBJECT_ID('tempdb..#tblSavedTripSerial') IS NOT NULL            
  DROP TABLE #tblSavedTripSerial 
             
 CREATE TABLE #tblSavedTripSerial 
 (             
  TripKey INT, 
  SavedTripKey UNIQUEIDENTIFIER,
  UserKey INT
 )            
 ...
 (                              
  ...
  NoOfStops VARCHAR(20),            --uncommented by pradeep for TFS 17817
 ...
 IF OBJECT_ID('tempdb..#TripdetailsFinal') IS NOT NULL        
 DROP TABLE #TripdetailsFinal            
...
#################################################################################### */            
 ...
 BEGIN              
  ...
  SELECT              
   ...
   TD.HotelResponseKey , 
   (SELECT COUNT(*) FROM TripSaved WHERE CrowdId IN(SELECT CrowdId FROM TripDetails WHERE tripKey = t1.tripKey) AND userKey = @parentLoggedInUser) AS InCrowd,
   TD.LatestHotelId               
...
/*  */
 ...
 BEGIN             
  ...
  AND TD.tripStartDate > DATEADD(D,2, GetDate()) -- DOD STOPS BEFORE TWO DAYS OF TRIP START DATE ....              
  AND T1.isUserCreatedSavedTrip =1 -- only user created save trip not system generated.  
  AND T1.tripSavedKey IS NOT NULL   -- Added to skip those trips which have tripsavedkey is null 30-08-2016
...
/*   */                
 ...
 --PRINT 'discover'            
  ...
  Insert Into @HashTags (HashTag,IsMain)            
  Select rtrim(ltrim(CASE WHEN Charindex('#', string) != 1 Then '#' + string Else string End)) , 0 
  From dbo.ufn_DelimiterToTable(rtrim(ltrim(@HashTag)),',') 
  --Where String not in ('#air','#car','#hotel','#package')

--********** Update ismain in @Hashtags for first record only
	Update top (1) @HashTags Set IsMain = 1 

--print 'hashtge is '+ @HashTag 
--select * from @HashTags
--SELECT 'delete it', @tripComponentType tripComponentType
  If @HashTag is null
	SET @HashTag = ''
...
--PRINT @compType		
	...
	END	
	IF CharIndex('#flights', @HashTag) > 0
	BEGIN            
		SET @compType =  1 
		SET @HashTag = Replace(@HashTag,'#flights','')
		
	END
	IF CharIndex('#flight', @HashTag) > 0
	BEGIN            
		SET @compType =  1 
		SET @HashTag = Replace(@HashTag,'#flight','')
...
--************** If StateCode is present then get all friendlyname/cityname associated with statecode
--************** This is done to fix the issue of generating hashtags with statecode e.g. austintx or similar
	IF @StateCode is not null -- StateCode will be set only for Trendingcrowds in home page from mobile
	BEGIN
		Insert Into @HashTags (HashTag,IsMain)            
		Select Distinct '#' + Replace(FriendlyName ,' ',''), 1 From cms.dbo.CustomHotelGroup Where AirportCode IN (
		Select string 
		From dbo.ufn_DelimiterToTable(@StateCode,',') ) And '#' + FriendlyName Not In (Select HashTag From @HashTags) 
		  
		Insert Into @HashTags (HashTag,IsMain)            
		Select Distinct '#' + Replace(CityName,' ','') , 1 From Trip.dbo.AirportLookup Where AirportCode IN (
		Select string 
		From dbo.ufn_DelimiterToTable(@StateCode,',') ) And '#' + CityName Not In (Select HashTag From @HashTags) 
	END

--Select * from @HashTags

INSERT INTO @HashTagTrips 
SELECT Distinct HashTag, TripKey From TripHashTagMapping WITH(NOLOCK) WHERE HashTag NOT IN (SELECT HashTag FROM @HashTags Where IsMain=1)
--SELECT Distinct HashTag, TripKey From TripHashTagMapping WITH(NOLOCK) WHERE HashTag NOT IN (SELECT TOP 1 HashTag FROM @HashTags)


--Select * from @HashTagTrips
INSERT INTO @HashTagTrips1 
SELECT Distinct HashTag, TripKey From TripHashTagMapping WHERE HashTag IN (SELECT HashTag FROM @HashTags Where IsMain = 1)
--SELECT Distinct HashTag, TripKey From TripHashTagMapping WHERE HashTag = (SELECT Top 1 HashTag FROM @HashTags )

DECLARE db_cursor CURSOR LOCAL FAST_FORWARD FOR  SELECT HashTag FROM @HashTags where HashTag NOT IN (SELECT HashTag FROM @HashTags Where Ismain = 1)
--DECLARE db_cursor CURSOR LOCAL FAST_FORWARD FOR  SELECT HashTag FROM @HashTags where HashTag NOT IN (SELECT top 1 HashTag FROM @HashTags )
...
OPEN db_cursor 
	...
	BEGIN 
		...
		DELETE FROM @HashTagTrips1 WHERE TripKey NOT IN (SELECT TripKey FROM @HashTagTrips  WHERE HashTag = @HName)
...
---------------------------------------------------------------------------------------------------------------------------------------------------- 
  ...
  FROM             
   TripDetails TD WITH (NOLOCK)                     
  INNER JOIN Trip T1 WITH (NOLOCK) ON TD.tripKey = T1.tripKey 
  INNER JOIN TripSaved TS WITH (NOLOCK) ON TS.tripSavedKey = T1.tripSavedKey
  INNER JOIN Vault..[User] UI WITH (NOLOCK) ON TD.userKey = UI.userKey  
  ...
  where  
  T1.tripStatusKey <> 17                      
  AND t1.tripKey <> @tripKey   and T1.PrivacyType <>2          
  AND T1.IsWatching = 1            
  --AND TD.userKey = @loggedInUserKey   
...
--and T1.tripComponentType>= case when  @OldcompType =0 then 1 when @compType >0 and @compType <7 then @compType else @compType end --added by pradeep as zarir's requirment      
 and T1.tripComponentType= CASE WHEN @compType > 0 THEN @compType ELSE T1.tripComponentType END
...
ELSE IF (@page = 17) --(Alert/Account page) Get extended trip details by trip for mobile             
 BEGIN            
...
---------------------------------------------------------------------------------------------------------------------------------------------------- 
  ...
  where  
  --T1.tripStatusKey <> 17                      
   t1.tripKey = @tripKey   --and T1.PrivacyType <>2          
  --AND T1.IsWatching = 1            
  --AND TD.userKey = @loggedInUserKey   
  --And TD.userKey = Case When @TrendingPeople > 0 Then @TrendingPeople Else TD.UserKey End         
  --AND TD.tripStartDate > DATEADD(D,2, GetDate()) -- DOD STOPS BEFORE TWO DAYS OF TRIP START DATE ....              
 ...
 */            
 ...
 --DELETE      --Comment this code due to implement the user story #17808       
 --FROM #Tripdetails            
 ...
 */            
 ...
 LatestHotelChainCode ,              
   ...
   INNER JOIN Trip T1 WITH (NOLOCK) ON TD.tripKey = T1.tripKey             
   INNER JOIN Vault..[User] UI WITH (NOLOCK) ON TD.userKey = UI.userKey                   
   --INNER JOIN #PreferredCityList PCL ON (CASE WHEN @cityType = 'From' THEN TD.tripFrom ELSE TD.tripTo END) = PCL.CityCode      --commented this for TFS #19259        
   LEFT JOIN Loyalty..UserMap UM WITH (NOLOCK) ON UI.userKey = UM.UserId 
   --LEFT JOIN Vault..AirPreference AP WITH (NOLOCK) ON UI.userKey = AP.userKey  
  -- LEFT JOIN TripSaved TS WITH (NOLOCK) ON TD.tripSavedKey = TS.tripSavedKey              
   where  T1.tripStatusKey <> 17      
...
-- ################ DELETE STATEMENTS AFTER BACK FILL RESULT SET PREPARATION ################ --            
    ...
    */            
    ...
    IF (@page != 17) -- added to get full tmu for purchased trip while viewing from third party page
    BEGIN
		DELETE             
...
--WHERE TD.UserName IS NULL
    ...
    --WHERE tripKey IN (SELECT max(tripKey) FROM #TripdetailsBackFill GROUP BY tripTo)  --commented this for TFS #19259               
...
-- UPDATING FASTEST TRENDING  ......                
 ...
 IF (@page = 1 OR @page= 9 OR  @page = 11 OR @page = 12 OR @page = 16 OR @page = 17)            
 /*            
  1 = HOME PAGE             
  9 = HOTEL SECTION LANDING PAGE             
  11 = FLIGHT SECTION LANDING PAGE            
...
*/             
 IF (@page = 1 OR @page= 9 OR  @page = 11 OR @page = 12 OR @page = 15 OR @page = 16 OR @page = 17)             
 /*            
  1 = HOME PAGE             
  9 = HOTEL SECTION LANDING PAGE             
  11 = FLIGHT SECTION LANDING PAGE            
...
#################################################################################### */            
 ...
 --    END
	...
	--select @MaxBestPriceTotal= CEILING(MAX(LatestDealAirPriceTotal+LatestDealHotelPriceTotal+LatestDealCarPriceTotal)) from #TripdetailsTemp	
   	...
   	select @MinFollowedTotal= MIN(CEILING(WatchersCount)) from #TempFilterDetails --having CEILING(MIN(WatchersCount)) >0
	select @MaxFollowedTotal=  MAX(CEILING(WatchersCount)) from #TempFilterDetails 

	select @MinSavingsTotal= MIN(CEILING(MostSavingsPrice)) from #TempFilterDetails --having CEILING(MIN(MostSavingsPrice)) >0
	select @MaxSavingsTotal= MAX(CEILING(MostSavingsPrice)) from #TempFilterDetails
	
	select @MinBestPriceTotal= MIN(CEILING(BestPrice)) from #TempFilterDetails --having CEILING(MIN(BestPrice))>0
	select @MaxBestPriceTotal= MAX(CEILING(BestPrice)) from #TempFilterDetails	
   
   

	IF @IsFilterApplied =0 AND @IsSliderFilterApplied = 0 AND @cityCode IS NULL AND @tripComponentType=0 AND @page=1
	BEGIN
	--print 'updateing latest value'
		UPDATE #TripdetailsTemp            
	...
	BEGIN
		UPDATE #TripdetailsTemp            
		SET MaxFollowedTotal=CT.MaxFollowedTotal, MaxSavingsTotal=CT.MaxSavingsTotal, MaxBestPriceTotal=CT.MaxBestPriceTotal,MinFollowedTotal = CT.MinFollowedTotal, MinSavingsTotal=CT.MinSavingsTotal, MinBestPriceTotal=CT.MinBestPriceTotal
		from Trip..CachingTMU CT
	END
...
--print 'inside filter zone'
	...
	BEGIN
		INSERT INTO #TripdetailsFinal            
			...
			--AND TF.MostSavingsPrice >=@MinSavings and TF.MostSavingsPrice <=@maxSavings  --commented for TFS #17635
	...
	IF @loggedInUserKey > 0 
	BEGIN
		INSERT INTO #tblSavedTripSerial 
		SELECT tripKey, tripSavedKey, userKey 
		FROM #TripdetailsFinal 
	...
	FROM
	(
		SELECT ROW_NUMBER() OVER(PARTITION BY tripSavedKey ORDER BY tripKey) RN, tripKey, tripsavedKey, userKey 
		FROM #TripdetailsFinal
			WHERE tripsavedKey NOT IN (SELECT SavedTripKey FROM #tblSavedTripSerial)
	)t WHERE RN = 1
-----------------------------------------------------------------------
----added below line for testing
	...
	--select * from #TripdetailsFinal
		...
		SELECT DISTINCT 
		 ...
		 INNER JOIN #tblSavedTripSerial TS ON TF.tripsavedKey = TS.SavedTripKey AND TF.tripKey = TS.TripKey AND TF.userKey = TS.UserKey 
		...
		order by TF.TripScoring	desc --added for TFS #8729
...
--print 'inside friends filter'
		...
		AND TF.WatchersCount <= (CASE WHEN @MaxFollowed>0 THEN @MaxFollowed else 1000 END)
		--AND TF.MostSavingsPrice >=@MinSavings and TF.MostSavingsPrice <=@maxSavings  --commented by pradeep for TFS 17635
		AND TF.BestPrice >=@MinBestPrice AND  TF.BestPrice<=@MaxBestPrice 
...
ELSE if @IsDiscoverFilter = 1 --if discover filter applied
BEGIN
			
			Declare @followings table
			(followingsId int)
			--If filter to show only followings users tmu then we will insert the followings users data in temp table
			If @CrowdMyFollowing = 1
			BEGIN
            ...
            BEGIN
				Insert Into @followings
				(followingsId)		
			    Select FollowerId From loyalty.dbo.UserFollowers Where userId = @loggedInUserKey       
            END
			...
			INNER JOIN #TempFilterDetails TF on TD.tripKey = TF.tripKey
			WHERE 
			--TD.WatchersCount > Case When @CrowdLeadersOnly = 1 Then 100 else  -1 END
			TD.WatchersCount > Case When @CrowdSize > 0 Then @CrowdSize Else -1 END
			AND TF.BestPrice < Case When @CrowdPrice > 0 Then @CrowdPrice Else TF.BestPrice+1 END
			AND (TF.MostSavingsPrice/(CASE WHEN TD.originalTotalPrice =0 THEN 1 ELSE TD.originalTotalPrice END) * 100) > Case When @CrowdDiscounts <=0 Then -999 Else @CrowdDiscounts END
			AND 1 =	CASE WHEN @CrowdMyFollowing = 0 THEN 1 ELSE  
			(
				SELECT top 1 1 FROM @followings F WHERE TD.userKey = F.followingsId 
					
			) END
			AND 1 =	CASE WHEN @CrowdLeadersOnly = 0 THEN 1 ELSE  
			(
				SELECT top 1 1 FROM @followings F WHERE TD.userKey = F.followingsId AND F.followingsId <> @loggedInUserKey
		...
		---- Added by Gopal	  
			...
			UPDATE x
			SET x.RowNumber = x.RowNumberNew
			FROM (
					SELECT RowNumber, ROW_NUMBER() OVER (ORDER BY [RowNumber]) AS RowNumberNew
					FROM #TripdetailsFinal
				 ) x
		-----------------------------------------------------------------------	
...
-----------------------------------------------------------------------
	...
	--ORDER BY  TF.TripKey--tm.[Order],tm.DiffDate
	order by TF.TripScoring	desc  --added for TFS #8729
...
BEGIN
	...
	WHERE userKey = @loggedInUserKey 
END
--SELECT * FROM #tblSavedTripSerial--ROHIT
...
--SELECT * FROM #tblSavedTripSerial Order By SavedTripKey 
--SELECT SavedTripKey, COUNT(tripKey) cnt FROM #tblSavedTripSerial Group By SavedTripKey 
