...
CREATE Procedure [dbo].[USP_GetHotelResponseForSavedTripNew]      
(      
...
BEGIN       
select distinct hotelresponsekey ,supplierhotelkey,supplierID,hotelname,rating,ratingtype,chaincode,hotelid,latitude,longitude,address1,cityname,citycode,statecode,countrycode,zipcode,PhoneNumber, Faxnumber,Distance, checkindate,checkoutdate,hoteldescript
ion,chainName,searchhotelprice as minrate,searchhotelTax as minratetax , 1 as isactualshoppingCart,hoteldailyprice , hoteltaxRate,hotelTotalPrice,hotelrateplancode,guaranteeCode,hotelrateplancode,ratedescription,cancellationPolicy,roomAmenities,OriginalHotelTotalPrice FROM vw_Tr
ipHotelResponseNew where hotelResponseKey =@hotelresponseKey       
END       
ELSE       
BEGIN        
