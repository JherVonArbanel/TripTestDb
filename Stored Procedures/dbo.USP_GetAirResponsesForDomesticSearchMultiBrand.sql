...
CREATE PROCEDURE [dbo].[USP_GetAirResponsesForDomesticSearchMultiBrand]
	(  
  ...
  inner join AirSubRequest r WITH(NOLOCK) on n.airsubrequestkey = r.airSubRequestKey  WHERE airRequestKey = @airRequestKey AND airlegnumber =1
   and  airSubRequestLegIndex = 1 and flightNumber =(SELECT flightnumber FROM NormalizedAirResponses WHERE airresponsekey = @SELECTedResponseKey  and airLegNumber =1 
	...
	BEGIN   
		...
		INSERT #AllOneWayResponses (airOneResponsekey,airOnePriceBase,airOnePriceBaseSenior ,airOnePriceTaxSenior, airOnePriceBaseChildren ,airOnePriceTaxChildren ,airOnePriceBaseInfant, airOnePriceTaxInfant,airOnePriceBaseYouth, airOnePriceTaxYouth, airOnePriceBaseTotal, airOnePriceTaxTotal,airOnePriceBaseDisplay, airOnePriceTaxDisplay,airSegmentFlightNumber,airSegmentMarketingAirlineCode,airsubRequestkey,airOnePriceTax ,airpriceTotal ,cabinclass  ,legConnections,airOnePriceBaseInfantWithSeat, airOnePriceTaxInfantWithSeat,agentwareQueryID,agentwareItineraryID, airLegBrandName, isMultiBrandFare ) 
		SELECT resp.airresponsekey, (airPriceBase ),
		airPriceBaseSenior,airPriceTaxSenior,airPriceBaseChildren,airPriceTaxChildren,airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth,airPriceTaxYouth,AirPriceBaseTotal,AirPriceTaxTotal, airPriceBaseDisplay, airPriceTaxDisplay,flightNumber,airlines,resp .airSubRequestKey ,(airPriceTax ) ,airPriceBase + airPriceTax ,n.cabinclass    ,n.airLegConnections ,airPriceBaseInfantWithSeat,airPriceTaxInfantWithSeat,agentwareQueryID,agentwareItineraryID,n.airLegBrandName,0
	...
	/** Ashima : Inserting OneWay for OneWay AirSubRequestKey*/ 
		INSERT #AllOneWayResponses (airOneResponsekey,airOnePriceBase,airOnePriceBaseSenior,airOnePriceTaxSenior,airOnePriceBaseChildren,airOnePriceTaxChildren,airOnePriceBaseInfant,airOnePriceTaxInfant,airOnePriceBaseYouth,airOnePriceTaxYouth,airOnePriceBaseTotal,airOnePriceTaxTotal,airOnePriceBaseDisplay, airOnePriceTaxDisplay,airSegmentFlightNumber,airSegmentMarketingAirlineCode,airsubRequestkey,airOnePriceTax ,airpriceTotal,otherLegprice,cabinclass  ,otherlegtax ,legConnections,airOnePriceBaseInfantWithSeat,airOnePriceTaxInfantWithSeat,airLegBrandName, isMultiBrandFare)  
		...
		(airPriceBaseInfantWithSeat + ISNULL(@airPriceInfantWithSeatForAnotherLeg,0)  ),(airPriceTaxInfantWithSeat + ISNULL(@airPriceTaxInfantWithSeatForAnotherLeg,0)),n.airLegBrandName,0
	  ...
	  --Insert Responses for AgentWare
	  	IF ( @airRequestType =1  ) 
		BEGIN
			INSERT #AllOneWayResponses (airOneResponsekey,airOnePriceBase,airOnePriceBaseSenior,airOnePriceTaxSenior,airOnePriceBaseChildren,airOnePriceTaxChildren,airOnePriceBaseInfant,airOnePriceTaxInfant,airOnePriceBaseYouth,airOnePriceTaxYouth,airOnePriceBaseTotal,airOnePriceTaxTotal,airOnePriceBaseDisplay, airOnePriceTaxDisplay,airSegmentFlightNumber,airSegmentMarketingAirlineCode,airsubRequestkey,airOnePriceTax ,airpriceTotal,otherLegprice,cabinclass  ,otherlegtax ,legConnections ,airOnePriceBaseInfantWithSeat,airOnePriceTaxInfantWithSeat,agentwareQueryID,agentwareItineraryID,airLegBrandName, isMultiBrandFare)  
			SELECT resp.airresponsekey, (airPriceBase + ISNULL(@airPriceForAnotherLeg,0)  ),
			(airPriceBaseSenior + ISNULL(@airPriceSeniorForAnotherLeg,0)  ),(airPriceTaxSenior + ISNULL(@airPriceTaxSeniorForAnotherLeg,0)),
			(airPriceBaseChildren + ISNULL(@airPriceChildrenForAnotherLeg,0)  ),(airPriceTaxChildren + ISNULL(@airPriceTaxChildrenForAnotherLeg,0)),
			(airPriceBaseInfant + ISNULL(@airPriceInfantForAnotherLeg,0)  ),(airPriceTaxInfant + ISNULL(@airPriceTaxInfantForAnotherLeg,0)),
			...
			flightNumber,airlines,resp .airSubRequestKey ,(airPriceTax + ISNULL(@airPriceTaxForAnotherLeg,0)),(airPriceBase + ISNULL(@airPriceForAnotherLeg,0)  )+(airPriceTax + ISNULL
			(@airPriceTaxForAnotherLeg,0)), case when @isTotalPriceSort = 0 THEN  ISNULL(@airPriceForAnotherLeg,0) else( isnull(@airPriceForAnotherLeg ,0) + isnull(@airPriceTaxForAnotherLeg,0) ) END ,N.cabinclass ,isnull(@airPriceTaxForAnotherLeg,0)  ,n.airLegConnections,
			(airPriceBaseInfantWithSeat + ISNULL(@airPriceInfantWithSeatForAnotherLeg,0)  ),(airPriceTaxInfantWithSeat + ISNULL(@airPriceTaxInfantWithSeatForAnotherLeg,0)),agentwareQueryID,agentwareItineraryID,n.airLegBrandName,0
	...
	BEGIN  
		...
		(airPriceTaxInfantWithSeat + ISNULL(@airPriceTaxInfantWithSeatForAnotherLeg,0)),agentwareQueryID,agentwareItineraryID,n.airLegBrandName,0
		From NormalizedAirResponses n WITH (NOLOCK) inner join AirResponse resp WITH (NOLOCK) on n.airresponsekey = resp.airResponseKey WHERE resp.airSubRequestKey = @airSubRequestKey   
		--and gdsSourceKey <> 9  
		END
		END  
...
--return
	...
	result.airOnePriceBaseInfantWithSeat,result.airOnePriceTaxInfantWithSeat,result.airLegBrandName,result.isMultiBrandFare
	FROM   
...
--select * from #normalizedResultSet
	...
	BEGIN
		--RT
		INSERT  #normalizedResultSet (airresponsekey ,airPriceBase,noOFSTOPs ,noOfAirlines ,takeoffdate ,landingdate ,airlinecode ,gdssourcekey ,airpricetax ,airsubrequetkey ,otherlegprice,cabinclass,otherlegtax ,  airPriceBaseSenior, airPriceTaxSenior, airPriceBaseChildren,airPriceTaxChildren, airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth, airPriceTaxYouth, airPriceBaseTotal,airPriceTaxTotal, airPriceBaseDisplay,airPriceTaxDisplay,noOfOtherlegairlines ,otherlegAirlines,legConnections,actualNoOFStops,isSameAirlinesItin,airPriceBaseInfantWithSeat,airPriceTaxInfantWithSeat,isMultiBrandFare,respBrand.airResponseMultiBrandKey)  
		(  
		SELECT seg.airresponsekey,respBrand.airPriceBaseDisplay ,CASE WHEN COUNT(seg.airresponsekey )-1  > 1 THEN (CASE WHEN @MaxNoofstops=2 THEN 2 ELSE 1 END) ELSE  COUNT(seg.airresponsekey )-1 END ,COUNT(distinct seg.airSegmentMarketingAirlineCode ),MIN(airSegmentDepartureDate ) ,MAX(airSegmentArrivalDate ), 

		CASE WHEN COUNT(distinct seg.airSegmentMarketingAirlineCode ) > 1 THEN 'Multiple Airlines'  ELSE MIN(seg.airSegmentMarketingAirlineCode) END ,  
		respBrand.gdsSourceKey, respBrand.airPriceTaxDisplay,respBrand.airSubRequestKey ,otherlegprice , result.cabinclass,--respNormalizeBrand.cabinclass ,
		otherlegtax , respBrand.airPriceBaseSenior, respBrand.airPriceTaxSenior,respBrand.airPriceBaseChildren,respBrand.airPriceTaxChildren, respBrand.airPriceBaseInfant,respBrand.airPriceTaxInfant,respBrand.airPriceBaseYouth, respBrand.airPriceTaxYouth, respBrand.airPriceBaseTotal,respBrand.airPriceTaxTotal, respBrand.airPriceBaseDisplay,respBrand.airPriceTaxDisplay  ,result.noOfOtherlegairlines ,result.otherlegAirlines ,result.legConnections,COUNT(seg.airresponsekey )-1,1,
		respBrand.airPriceBaseInfantWithSeat,respBrand.airPriceTaxInfantWithSeat,1,respBrand.airResponseMultiBrandKey
		FROM   
		#AllOneWayResponses result  INNER JOIN     
		AirSegments seg WITH(NOLOCK) ON result .airOneResponsekey = seg.airResponseKey 
		INNER JOIN  
		AirResponseMultiBrand respBrand WITH(NOLOCK) ON result.airOneResponsekey = respBrand.airResponseKey   
		WHERE seg.airLegNumber = @airRequestTypeKey  AND respBrand.airSubRequestKey = @airBundledRequest
		GROUP BY seg.airResponseKey,respBrand.airPriceBaseDisplay ,respBrand.gdsSourceKey  ,respBrand.airPriceTaxDisplay , respBrand.airSubRequestKey ,otherlegprice ,result.cabinclass,--respNormalizeBrand.cabinclass ,
		otherlegtax,respBrand.airPriceBaseSenior, respBrand.airPriceTaxSenior, respBrand.airPriceBaseChildren,respBrand.airPriceTaxChildren, respBrand.airPriceBaseInfant,respBrand.airPriceTaxInfant,respBrand.airPriceBaseYouth, respBrand.airPriceTaxYouth, respBrand.airPriceBaseTotal,respBrand.airPriceTaxTotal, respBrand.airPriceBaseDisplay,respBrand.airPriceTaxDisplay,result.noOfOtherlegairlines ,result.otherlegAirlines ,legConnections,respBrand.airPriceBaseInfantWithSeat ,respBrand.airPriceTaxInfantWithSeat,respBrand.airResponseMultiBrandKey
		) 
		...
		INNER JOIN  
		AirResponseMultiBrand respBrand WITH(NOLOCK) ON result.airOneResponsekey = respBrand.airResponseKey   
		WHERE seg.airLegNumber = @airRequestTypeKey  AND respBrand.airSubRequestKey = @airPublishedFareRequest
		GROUP BY seg.airResponseKey,respBrand.airPriceBaseDisplay ,respBrand.gdsSourceKey  ,respBrand.airPriceTaxDisplay , respBrand.airSubRequestKey ,otherlegprice ,result.cabinclass,--respNormalizeBrand.cabinclass ,
		otherlegtax,respBrand.airPriceBaseSenior, respBrand.airPriceTaxSenior, respBrand.airPriceBaseChildren,respBrand.airPriceTaxChildren, respBrand.airPriceBaseInfant,respBrand.airPriceTaxInfant,respBrand.airPriceBaseYouth, respBrand.airPriceTaxYouth, respBrand.airPriceBaseTotal,respBrand.airPriceTaxTotal, respBrand.airPriceBaseDisplay,respBrand.airPriceTaxDisplay,result.noOfOtherlegairlines ,result.otherlegAirlines ,legConnections,respBrand.airPriceBaseInfantWithSeat ,respBrand.airPriceTaxInfantWithSeat,respBrand.airResponseMultiBrandKey
		) 
		...
		INSERT  #normalizedResultSet (airresponsekey ,airPriceBase,noOFSTOPs ,noOfAirlines ,takeoffdate ,landingdate ,airlinecode ,gdssourcekey ,airpricetax ,airsubrequetkey ,otherlegprice,cabinclass,otherlegtax ,  airPriceBaseSenior, airPriceTaxSenior, airPriceBaseChildren,airPriceTaxChildren, airPriceBaseInfant,airPriceTaxInfant,airPriceBaseYouth, airPriceTaxYouth, airPriceBaseTotal,airPriceTaxTotal, airPriceBaseDisplay,airPriceTaxDisplay,noOfOtherlegairlines ,otherlegAirlines,legConnections,actualNoOFStops,isSameAirlinesItin,airPriceBaseInfantWithSeat,airPriceTaxInfantWithSeat,isMultiBrandFare,respBrand.airResponseMultiBrandKey)  
		(  
		SELECT seg.airresponsekey, (respBrand.airPriceBaseDisplay + ISNULL(@airPriceForAnotherLeg,0)) ,CASE WHEN COUNT(seg.airresponsekey )-1  > 1 THEN (CASE WHEN @MaxNoofstops=2 THEN 2 ELSE 1 END) ELSE  COUNT(seg.airresponsekey )-1 END ,COUNT(distinct seg.airSegmentMarketingAirlineCode ),MIN(airSegmentDepartureDate ) ,MAX(airSegmentArrivalDate ), 

		CASE WHEN COUNT(distinct seg.airSegmentMarketingAirlineCode ) > 1 THEN 'Multiple Airlines'  ELSE MIN(seg.airSegmentMarketingAirlineCode) END ,  
		respBrand.gdsSourceKey,(respBrand.airPriceTaxDisplay + ISNULL(@airPriceTaxDisplayForAnotherLeg,0)) ,respBrand.airSubRequestKey ,otherlegprice , result.cabinclass,--respNormalizeBrand.cabinclass ,
		otherlegtax , (respBrand.airPriceBaseSenior + ISNULL(@airPriceSeniorForAnotherLeg,0)  ), (respBrand.airPriceTaxSenior + ISNULL(@airPriceTaxSeniorForAnotherLeg,0)),(respBrand.airPriceBaseChildren + ISNULL(@airPriceChildrenForAnotherLeg,0)  )
		,(respBrand.airPriceTaxChildren + ISNULL(@airPriceTaxChildrenForAnotherLeg,0)), 
		(respBrand.airPriceBaseInfant + ISNULL(@airPriceInfantForAnotherLeg,0)  ),(respBrand.airPriceTaxInfant + ISNULL(@airPriceTaxInfantForAnotherLeg,0)),
		(respBrand.airPriceBaseYouth + ISNULL(@airPriceYouthForAnotherLeg,0)  ), (respBrand.airPriceTaxYouth + ISNULL(@airPriceTaxYouthForAnotherLeg,0)), 
		(respBrand.airPriceBaseTotal + ISNULL(@airPriceTotalForAnotherLeg,0)  ),(respBrand.airPriceTaxTotal + ISNULL(@airPriceTaxTotalForAnotherLeg,0)), 
		(respBrand.airPriceBaseDisplay + ISNULL(@airPriceDisplayForAnotherLeg,0)  ),(respBrand.airPriceTaxDisplay + ISNULL(@airPriceTaxDisplayForAnotherLeg,0))  ,
		result.noOfOtherlegairlines ,result.otherlegAirlines ,result.legConnections,COUNT(seg.airresponsekey )-1,1,
		(respBrand.airPriceBaseInfantWithSeat + ISNULL(@airPriceInfantWithSeatForAnotherLeg,0)  ),(respBrand.airPriceTaxInfantWithSeat + ISNULL(@airPriceTaxInfantWithSeatForAnotherLeg,0)),1,
		respBrand.airResponseMultiBrandKey
		FROM   
		#AllOneWayResponses result  INNER JOIN   
		AirSegments seg WITH(NOLOCK) ON result .airOneResponsekey = seg.airResponseKey 
		INNER JOIN  
		AirResponseMultiBrand respBrand WITH(NOLOCK) ON result.airOneResponsekey = respBrand.airResponseKey 
		WHERE seg.airLegNumber = @airRequestTypeKey  AND respBrand.airSubRequestKey  = @airSubRequestKey
		GROUP BY seg.airResponseKey,respBrand.airPriceBaseDisplay ,respBrand.gdsSourceKey  ,respBrand.airPriceTaxDisplay , respBrand.airSubRequestKey ,otherlegprice ,result.cabinclass,--respNormalizeBrand.cabinclass ,
		otherlegtax,respBrand.airPriceBaseSenior, respBrand.airPriceTaxSenior, respBrand.airPriceBaseChildren,respBrand.airPriceTaxChildren, respBrand.airPriceBaseInfant,respBrand.airPriceTaxInfant,respBrand.airPriceBaseYouth, respBrand.airPriceTaxYouth, respBrand.airPriceBaseTotal,respBrand.airPriceTaxTotal, respBrand.airPriceBaseDisplay,respBrand.airPriceTaxDisplay,result.noOfOtherlegairlines ,result.otherlegAirlines ,legConnections,respBrand.airPriceBaseInfantWithSeat ,respBrand.airPriceTaxInfantWithSeat,respBrand.airResponseMultiBrandKey
		) 
		...
		SET #normalizedResultSet.CabinClass = M.CabinClass, #normalizedResultSet.airLegBrandName = M.airLegBrandName
		FROM  NormalizedAirResponsesMultiBrand M
		WHERE #normalizedResultSet.airresponsekey = M.airresponsekey 
		AND M.airLegNumber = @airRequestTypeKey 
		AND #normalizedResultSet.isMultiBrandFare = 1
...
--SELECT 'Normalized',* from #normalizedResultSet
	...
	,airSuperSaverTax,airEconSaverTax,airFirstFlexTax,airCorporateTax,airEconFlexTax,airEconUpgradeTax,airPriceBaseInfantWithSeat, airPriceTaxInfantWithSeat,agentwareQueryID,agentwareItineraryID,airsegmentPricingKey,airLegBrandName, isMultiBrandFare, airResponseMultiBrandKey,airSegmentBrandName,airSegmentBrandID,airSegmentBaggage,airSegmentMealCode)  
	...
	,airSuperSaverTax,airEconSaverTax,airFirstFlexTax,airCorporateTax,airEconFlexTax,airEconUpgradetax,resp.airPriceBaseInfantWithSeat,resp.airPriceTaxInfantWithSeat,agentwareQueryID,agentwareItineraryID,airsegmentPricingKey,normalized.airLegBrandName,normalized.isMultiBrandFare,normalized.airResponseMultiBrandKey,seg.airSegmentBrandName,seg.airSegmentBrandID,seg.airSegmentBaggage,seg.airSegmentMealCode
...
--return
	...
	UPDATE #airResponseResultset 
		SET #airResponseResultset.airSegmentBrandName = M.airSegmentBrandName, #airResponseResultset.airSegmentBrandID = M.airSegmentBrandID,
		#airResponseResultset.airSegmentBaggage = M.airSegmentBaggage,#airResponseResultset.airSegmentMealCode = M.airSegmentMealCode,
		#airResponseResultset.airSegmentResBookDesigCode = M.airSegmentResBookDesigCode
		FROM  AirSegmentsMultiBrand M WITH (NOLOCK)
		WHERE #airResponseResultset.airSegmentKey = M.airSegmentKey
		AND #airResponseResultset.isMultiBrandFare = 1
	...
	order by rowNum ,airlegnumber ,air.isMultiBrandFare ,segmentOrder, airSegmentDepartureDate,airResponseMultiBrandKey
	...
	SELECT (case when @isTotalPriceSort= 0 then( MIN (airPrice) ) else  min (totalcost) end ) AS LowestPrice , (case when @isTotalPriceSort= 0 then MAX(airPrice ) else max(totalcost) end ) AS HighestPrice FROM #airResponseResultset  result1    
	where result1.isMultiBrandFare = 0 
	END   
