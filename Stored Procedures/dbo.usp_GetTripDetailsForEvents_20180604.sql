...
Create PROCEDURE [dbo].[usp_GetTripDetailsForEvents_20180604] 
(                      
--DECLARE
 @tripID   int   ,                        
...
BEGIN                        
 INSERT Into @tblTrip                        
 Select  @tripID,   AttendeeGuid, tripStatusKey  from Trip WITH(NOLOCK) where tripKey  = @tripID and   tripStatusKey <>17                      
 --ORDER BY tripKey DESC
END                        
ELSE                         
BEGIN                        
 INSERT Into @tblTrip                        
 Select  tripKey ,  AttendeeGuid, tripStatusKey  from Trip WITH(NOLOCK) where AttendeeGuid  = @AttendeeGuid  and   tripStatusKey <>17                      
 --ORDER BY tripKey DESC
END                        
