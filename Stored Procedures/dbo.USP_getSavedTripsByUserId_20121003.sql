...
*/
CREATE PROCEDURE [dbo].[USP_getSavedTripsByUserId_20121003] 
(          
	@userId INT,
	@Limit	INT
)          
...
SELECT top (@Limit)* 
		...
		(
			...
			FROM Trip T
				INNER JOIN TRIPREQUEST TR ON T.tripRequestKey = TR.tripRequestKey AND T.userKey =  CONVERT(VARCHAR, 697) 
